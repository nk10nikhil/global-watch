...
export async function listAirportDelays(
  ...
  try {
    const meta = (await getCachedJson("seed-meta:aviation:faa", true)) as {
      fetchedAt?: number;
    } | null;
    ...
    if (
      seedData &&
      Array.isArray(seedData.alerts) &&
      (seedAge < SEED_FRESHNESS_MS || !process.env.SEED_FALLBACK_FAA)
    ) {
  ...
  if (!faaFromSeed) {
    try {
      ...
      console.warn(
        `[Aviation] FAA fetch failed: ${err instanceof Error ? err.message : "unknown"}`,
      );
    }
  }
  ...
  try {
    const cached = (await getCachedJson(INTL_CACHE_KEY)) as {
      alerts: AirportDelayAlert[];
    } | null;
    if (cached?.alerts) {
    ...
    console.warn(
      `[Aviation] Intl fetch failed: ${err instanceof Error ? err.message : "unknown"}`,
    );
  }
  ...
  if (notamResult) {
    ...
    const total =
      (notamResult.closedIcaos?.length ?? 0) +
      (notamResult.restrictedIcaos?.length ?? 0);
    if (total > 0) {
      console.warn(
