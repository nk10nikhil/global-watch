...
function normalizeFlights(flights: AVSFlight[], now: number): FlightInstance[] {
  return flights.map((f) => {
    ...
    return {
      ...
      estimatedDeparture:
        parseTs(f.departure?.estimated) || (schedDep ? schedDep + delayMs : 0),
...
function buildSimulatedFlights(
  ...
  const destinations = {
    IST: ["LHR", "FRA", "CDG", "AMS", "MAD"],
    LHR: ["IST", "JFK", "FRA", "SIN"],
    FRA: ["IST", "LHR", "CDG", "JFK"],
  };
  const origins = (destinations as Record<string, string[]>)[airport] ?? [
    "LHR",
    "FRA",
    "CDG",
  ];
  ...
  for (let i = 0; i < count; i++) {
    const isArr =
      direction === "FLIGHT_DIRECTION_ARRIVAL" ||
      (direction === "FLIGHT_DIRECTION_BOTH" && i % 2 === 0);
    ...
    flights.push({
      ...
      operatingCarrier: carrier,
      origin: isArr
        ? { iata: other, icao: "", name: other, timezone: "UTC" }
        : { iata: airport, icao: "", name: airport, timezone: "UTC" },
      destination: isArr
        ? { iata: airport, icao: "", name: airport, timezone: "UTC" }
        : { iata: other, icao: "", name: other, timezone: "UTC" },
...
export async function listAirportFlights(
  ...
  try {
    const result = await cachedFetchJson<{
      ...
      if (!apiKey) {
        return {
          flights: buildSimulatedFlights(airport, direction, limit, now),
          source: "simulated",
        };
      }
      ...
      const paramKey =
        direction === "FLIGHT_DIRECTION_ARRIVAL" ? "arr_iata" : "dep_iata";
      ...
      try {
        ...
        console.warn(
          `[Aviation] Flights fetch failed for ${airport}: ${err instanceof Error ? err.message : err}`,
        );
        return {
          flights: buildSimulatedFlights(airport, direction, limit, now),
          source: "simulated",
