...
async function fetchTp<T>(url: string, token: string): Promise<T | null> {
  try {
    ...
    console.warn(
      `[Travelpayouts] fetch error: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}
...
export async function searchPricesTravelpayouts(opts: {
  ...
  if (isDayPrecision) {
    ...
    const data = await cachedFetchJson<TpV3Ticket[]>(cacheKey, 3600, () =>
      fetchTp<TpV3Ticket[]>(
        `${BASE_V3}/prices_for_dates?${params}`,
        token,
      ).then((d) => d ?? []),
    );
    ...
    const data = await cachedFetchJson<TpMonthMatrixTicket[]>(
      cacheKey,
      7200,
      () =>
        fetchTp<TpMonthMatrixTicket[]>(
          `${BASE_V2}/month-matrix?${params}`,
          token,
        ).then((d) => d ?? []),
    );
    ...
    let rows = data ?? [];
    if (nonstopOnly)
      rows = rows.filter((r) => (r.number_of_changes ?? 0) === 0);
    quotes = rows
      .slice(0, maxResults)
    ...
    const data = await cachedFetchJson<TpLatestTicket[]>(cacheKey, 3600, () =>
      fetchTp<TpLatestTicket[]>(`${BASE_V2}/latest?${params}`, token).then(
        (d) => d ?? [],
      ),
    );
