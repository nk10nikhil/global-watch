...
export async function getAirportOpsSummary(
  ...
  try {
    const result = await cachedFetchJson<{ summaries: AirportOpsSummary[] }>(
      ...
      CACHE_TTL,
      async () => {
        const airports = MONITORED_AIRPORTS.filter((a) =>
          requested.includes(a.iata),
        );
        ...
        for (const airport of airports) {
          ...
          const cancelRate =
            totalFlights > 0 ? (cancelledFlights / totalFlights) * 100 : 0;
          ...
          if ((isClosed || isRestricted) && notamText)
            topDelayReasons.push(notamText.slice(0, 80));
