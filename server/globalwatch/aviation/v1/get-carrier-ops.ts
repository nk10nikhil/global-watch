  ...
  try {
    const result = await cachedFetchJson<{ carriers: CarrierOpsSummary[] }>(
      ...
      async () => {
        ...
        type FI =
          import("../../../../src/generated/server/globalwatch/aviation/v1/service_server").FlightInstance;
        const allFlights: FI[] = [];
        ...
        const groups = new Map<
          string,
          {
            carrier: import("../../../../src/generated/server/globalwatch/aviation/v1/service_server").Carrier;
            airport: string;
            flights: FI[];
          }
        >();
        ...
        for (const [, { carrier, airport, flights }] of groups) {
          ...
          carriers.push({
            ...
            avgDelayMinutes:
              delayed.length > 0 ? Math.round(totalDelay / delayed.length) : 0,
            delayPct:
              Math.round((delayed.length / flights.length) * 100 * 10) / 10,
            cancellationRate:
              Math.round((cancelled.length / flights.length) * 100 * 10) / 10,
            updatedAt: now,
          });
        }
        ...
        carriers.sort(
          (a, b) =>
            b.cancellationRate - a.cancellationRate || b.delayPct - a.delayPct,
        );
    ...
    return {
      carriers: (result?.carriers ?? []).filter(
        (c) => c.totalFlights >= minFlights,
      ),
    ...
    console.warn(
      `[Aviation] GetCarrierOps failed: ${err instanceof Error ? err.message : err}`,
    );
