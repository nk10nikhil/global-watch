...
interface AVSFlight {
  flight?: {
    iata?: string;
    codeshared?: Array<{ flight_iata?: string; airline_iata?: string }>;
  };
...
function normalizeFlight(f: AVSFlight, now: number): FlightInstance {
  const schedDep = f.departure?.scheduled
