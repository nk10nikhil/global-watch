...
function buildSimulatedPositions(
  ...
  const count =
    latSpan > 0 && lonSpan > 0 ? Math.floor(Math.random() * 16) + 15 : 10;
...
async function fetchOpenSkyAnonymous(
...
export async function trackAircraft(
      ...
      async () => {
        ...
        if (relayBase) {
          try {
            ...
            console.warn(
              `[Aviation] Relay failed: ${err instanceof Error ? err.message : err}`,
            );
          }
        }
        ...
        try {
          ...
          console.warn(
            `[Aviation] Direct OpenSky anonymous failed: ${err instanceof Error ? err.message : err}`,
          );
        }
  ...
  if (result) {
    ...
    if (req.icao24)
      positions = positions.filter((p) => p.icao24 === req.icao24);
    if (req.callsign)
