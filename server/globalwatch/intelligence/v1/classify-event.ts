...
export async function classifyEvent(
  ...
  const title =
    typeof req.title === "string" ? req.title.slice(0, MAX_TITLE_LEN) : "";
  if (!title) {
    markNoCacheResponse(ctx.request);
    return { classification: undefined };
  }
  ...
  let cached: { level: string; category: string; timestamp: number } | null =
    null;
  try {
    cached = await cachedFetchJson<{
      ...
      category: string;
      timestamp: number;
    }>(cacheKey, CLASSIFY_CACHE_TTL, async () => {
      try {
...
Return: {"level":"...","category":"..."}`;
        ...
        const level = VALID_LEVELS.includes(parsed.level ?? "")
          ? parsed.level!
          : null;
