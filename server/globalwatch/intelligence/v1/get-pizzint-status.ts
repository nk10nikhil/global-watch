import type {
  ...
  TrendDirection,
  DataFreshness,
} from "../../../../src/generated/server/globalwatch/intelligence/v1/service_server";
...
export async function getPizzintStatus(
  ...
  try {
    result = await cachedFetchJson<GetPizzintStatusResponse>(
      cacheKey,
      REDIS_CACHE_TTL,
      async () => {
          ...
          if (raw.success && raw.data) {
            ...
            const avgPop =
              openLocations.length > 0
                ? openLocations.reduce((s, l) => s + l.currentPopularity, 0) /
            ...
            const hasFresh = locations.some(
              (l) => l.dataFreshness === "DATA_FRESHNESS_FRESH",
            );
        ...
        if (req.includeGdelt) {
          try {
            ...
            if (resp.ok) {
              ...
              >;
              tensionPairs = Object.entries(raw).map(
                ([pairKey, dataPoints]) => {
                  ...
                  const prev =
                    dataPoints.length > 1
                      ...
                      : latest;
                  const change =
                    prev.v > 0 ? ((latest.v - prev.v) / prev.v) * 100 : 0;
