import type {
  ...
  ImageryScene,
} from "../../../../src/generated/server/globalwatch/imagery/v1/service_server";
...
function mapFeature(f: StacFeature): ImageryScene {
  ...
  const mode =
    props["sar:instrument_mode"] ??
    (satellite.includes("sentinel-2") ? "MSI" : "");
...
export async function searchImagery(
  ...
  try {
    const result = await cachedFetchJson<{
      ...
      const totalResults =
        data.numberMatched ?? data.context?.matched ?? scenes.length;
    ...
    console.warn(
      `[Imagery] Search failed: ${err instanceof Error ? err.message : "unknown"}`,
    );
    return { scenes: [], totalResults: 0, cacheHit: false };
  }
}
