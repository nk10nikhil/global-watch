...
} from "../../../../src/generated/server/globalwatch/prediction/v1/service_server";
      ...
      async () => {
        ...
        for (const event of data.events) {
          ...
          if (topMarket && topVol > KALSHI_VOLUME_THRESHOLD) {
            markets.push(
              mapKalshiMarket(topMarket, event.category || "", event.title),
            );
          }
        }
...
export const listPredictionMarkets: PredictionServiceHandler["listPredictionMarkets"] =
  async (
    _ctx: ServerContext,
    ...
    try {
      ...
      if (!query) {
        try {
          ...
          if (bootstrap) {
            ...
            const isFinance =
              !isTech && category && FINANCE_CATEGORY_TAGS.includes(category);
            ...
            if (variant && variant.length > 0) {
              const markets: PredictionMarket[] = variant
                .slice(0, limit)
                .map((m) => ({
                  id: m.url?.split("/").pop() || "",
                  title: m.title,
                  ...
                  source:
                    m.source === "kalshi"
                      ? MarketSource.MARKET_SOURCE_KALSHI
                      : MarketSource.MARKET_SOURCE_POLYMARKET,
                }));
              return { markets, pagination: undefined };
            }
          }
        } catch {
          /* bootstrap read failed, fall through */
        }
      }

      // Fallback: fetch from Gamma API and Kalshi API in parallel
      const kalshiFetch = includeKalshi
        ? fetchKalshiMarkets()
      ...
      const [gammaResult, kalshiResult] = await Promise.allSettled([
        gammaFetch,
        kalshiFetch,
      ]);

      const polymarketMarkets =
        gammaResult.status === "fulfilled" && gammaResult.value
          ? gammaResult.value
          : [];
      ...
      if (includeKalshi) {
        ...
        if (query && kalshiMarkets.length > 0) {
          const q = query.toLowerCase();
          filteredKalshi = kalshiMarkets.filter((m) =>
