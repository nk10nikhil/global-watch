...
import type {
  ...
  FireConfidence,
} from "../../../../src/generated/server/globalwatch/wildfire/v1/service_server";
...
export const listFireDetections: WildfireServiceHandler["listFireDetections"] =
  async (
    _ctx: ServerContext,
    ...
    try {
      const [seedData, seedMeta] = await Promise.all([
        getCachedJson(
          REDIS_CACHE_KEY,
          true,
        ) as Promise<ListFireDetectionsResponse | null>,
        getCachedJson("seed-meta:wildfire:fires", true) as Promise<{
          fetchedAt?: number;
        } | null>,
      ]);
    ...
    try {
      result = await cachedFetchJson<ListFireDetectionsResponse>(
        ...
        async () => {
          ...
          return fireDetections.length > 0
            ? { fireDetections, pagination: undefined }
            : null;
        },
      );
    } catch {
