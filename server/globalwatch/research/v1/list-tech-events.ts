...
import type {
  ...
  TechEvent,
  TechEventCoords,
} from "../../../../src/generated/server/globalwatch/research/v1/service_server";
...
async function fetchTextWithRelay(url: string): Promise<string | null> {
  ...
  try {
    ...
    console.warn(
      `[tech-events] Direct fetch ${url} failed: ${(e as Error).message}`,
    );
  }
  ...
  if (relayBase) {
    try {
      ...
      if (resp.ok) {
        ...
        if (text.length > 100) {
          console.log(
            `[tech-events] Relay fetch ${url}: success (${text.length} chars)`,
          );
          return text;
        }
      } else {
      ...
      console.warn(
        `[tech-events] Relay fetch ${url} failed: ${(e as Error).message}`,
      );
    }
  }
...
async function fetchTechEvents(
  ...
  if (icsText) {
    ...
    console.warn(
      `[tech-events] Techmeme ICS: no data (direct + relay both failed)`,
    );
  }
  ...
  if (rssText) {
    ...
    console.log(
      `[tech-events] dev.events RSS: ${devEvents.length} events parsed`,
    );
  } else {
    externalSourcesFailed++;
    console.warn(
      `[tech-events] dev.events RSS: no data (direct + relay both failed)`,
    );
  }
