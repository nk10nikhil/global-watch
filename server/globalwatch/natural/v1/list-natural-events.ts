import type {
  ...
  NaturalEvent,
} from "../../../../src/generated/server/globalwatch/natural/v1/service_server";
...
function classifyWind(kt: number): {
  category: number;
  classification: string;
} {
...
async function fetchGdacs(): Promise<NaturalEvent[]> {
  ...
  for (const f of features) {
    ...
    const category = normalizeNaturalCategory(
      GDACS_TO_CATEGORY[props.eventtype] || "manmade",
    );
    const alertPrefix =
...
async function fetchNhc(): Promise<NaturalEvent[]> {
  ...
  for (const { slot, points, cone, pastPts } of stormData) {
    const currentPt = points.features.find(
      (f: any) => f.properties?.tau === 0 || f.properties?.fcstprd === 0,
    );
    ...
    if (pastPts?.features?.length > 0) {
      const sorted = pastPts.features
        ...
        .sort(
          (a: any, b: any) => (a.properties.dtg || 0) - (b.properties.dtg || 0),
        );
  ...
  try {
    const [seedData, seedMeta] = await Promise.all([
      getCachedJson(
        REDIS_CACHE_KEY,
        true,
      ) as Promise<NaturalEventsCache | null>,
      getCachedJson("seed-meta:natural:events", true) as Promise<{
        fetchedAt?: number;
      } | null>,
    ]);
...
export const listNaturalEvents: NaturalServiceHandler["listNaturalEvents"] =
  async (
    _ctx: ServerContext,
    ...
    try {
      ...
      const result = await cachedFetchJson<ListNaturalEventsResponse>(
        ...
        REDIS_CACHE_TTL,
        async () => {
          const [eonetResult, gdacsResult, nhcResult] =
          ...
          if (eonetResult.status === "rejected")
            console.error("[EONET]", eonetResult.reason?.message);
          if (gdacsResult.status === "rejected")
            console.error("[GDACS]", gdacsResult.reason?.message);
          if (nhcResult.status === "rejected")
            console.error("[NHC]", nhcResult.reason?.message);
