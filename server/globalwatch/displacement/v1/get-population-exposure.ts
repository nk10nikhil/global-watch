...
const PRIORITY_COUNTRIES: Record<
  string,
  { name: string; pop: number; area: number }
> = {
...
export async function getPopulationExposure(
  ...
  if (req.mode === "exposure") {
    ...
    const info = bestMatch
      ? PRIORITY_COUNTRIES[bestMatch]!
      : { pop: 50000000, area: 500000 };
  ...
  const countries: CountryPopulationEntry[] = Object.entries(
    PRIORITY_COUNTRIES,
  ).map(([code, info]) => ({
    code,
    name: info.name,
