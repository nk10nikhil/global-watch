...
import type {
  ...
  GeoCoordinates,
} from "../../../../src/generated/server/globalwatch/displacement/v1/service_server";
...
async function fetchUnhcrYearItems(
  year: number,
): Promise<UnhcrRawItem[] | null> {
  const limit = 10000;
  ...
  for (let page = 1; page <= maxPageGuard; page++) {
    ...
    const pageItems: UnhcrRawItem[] = Array.isArray(data.items)
      ? data.items
      : [];
  ...
  try {
    ...
    const [seedData, seedMeta] = await Promise.all([
      getCachedJson(
        seedKey,
        true,
      ) as Promise<GetDisplacementSummaryResponse | null>,
        ...
        return {
          summary: {
            ...
            globalTotals: {
              ...
              total:
                totalRefugees + totalAsylumSeekers + totalIdps + totalStateless,
            },
