...
} from "../../../../src/generated/server/globalwatch/infrastructure/v1/service_server";
...
function redisCmd(
  cmd: string[],
): { url: string; token: string; body: string } | null {
  ...
  try {
...

    {
      ...
      for (const [type, sourceKey] of Object.entries(COUNT_SOURCE_KEYS)) {
        const data = (await getCachedJson(sourceKey)) as Record<
          string,
          unknown
        > | null;
        if (!data) continue;
        ...
        if (type === "news") {
          ...
          const fires = (data as { fireDetections?: unknown[] })
            ?.fireDetections;
          counts[type] = fires?.length ?? 0;
        }
      }

      const typesWithCounts = trackedTypes.filter(
        (t) => counts[t] !== undefined,
      );
      ...
      for (let i = 0; i < typesWithCounts.length; i++) {
        ...
        if (baseline && baseline.sampleCount >= MIN_SAMPLES) {
          const variance = Math.max(
            0,
            baseline.m2 / (baseline.sampleCount - 1),
          );
          const stdDev = Math.sqrt(variance);
          const zScore =
            stdDev > 0 ? Math.abs((count - baseline.mean) / stdDev) : 0;
        ...
        const prev: BaselineEntry = baseline || {
          mean: 0,
          m2: 0,
          sampleCount: 0,
          lastUpdated: "",
