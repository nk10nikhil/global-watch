  ...
  for (const warning of cableWarnings) {
    ...
    if (isFault) {
      signals.push({
        ...
        severity: 1.0,
        confidence:
          joinMethod === "name" ? 0.9 : Math.max(0.4, 0.8 - distanceKm / 500),
        ttlSeconds: 5 * 86400,
        kind: "operator_fault",
        evidence: [
          {
      ...
      signals.push({
        ...
        severity: 0.6,
        confidence:
          joinMethod === "name" ? 0.8 : Math.max(0.3, 0.7 - distanceKm / 500),
        ttlSeconds: 3 * 86400,
        kind: "cable_advisory",
        evidence: [
          { source: "NGA", summary: `Cable advisory: ${summaryText}`, ts },
        ],
      });
    }
...
export function computeHealthMap(
  signals: Signal[],
): Record<string, CableHealthRecord> {
  const now = Date.now();
  ...
  for (const [cableId, cableSignals] of Object.entries(byCable)) {
    const effectiveSignals: Array<
      Signal & { effective: number; recencyWeight: number }
    > = [];
    ...
    for (const sig of cableSignals) {
      ...
      const recencyWeight = Math.max(
        0,
        Math.min(1, 1 - ageSec / sig.ttlSeconds),
      );
    ...
    const topConfidence =
      effectiveSignals[0]!.confidence * effectiveSignals[0]!.recencyWeight;
...
export async function getCableHealth(
  ...
  try {
    const result = await cachedFetchJson<GetCableHealthResponse>(
      CACHE_KEY,
      CACHE_TTL,
      async () => {
