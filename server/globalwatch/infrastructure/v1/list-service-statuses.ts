  ...
  try {
    ...
    if (service.customParser === "gcp") {
      ...
      const active = Array.isArray(data)
        ? data.filter(
            (i: any) => i.end === undefined || new Date(i.end) > new Date(),
          )
        : [];
      if (active.length === 0) {
        return withStatus(
...
export async function listServiceStatuses(
  ...
  try {
    const results = await cachedFetchJson<ServiceStatus[]>(
      INFRA_CACHE_KEY,
      INFRA_CACHE_TTL,
      async () => {
    ...
    return {
      statuses: filterAndSortStatuses(fallbackStatusesCache?.data || [], req),
    };
  }
}
