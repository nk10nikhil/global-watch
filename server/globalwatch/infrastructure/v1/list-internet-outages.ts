...
export async function listInternetOutages(
  ...
  try {
    const [seedData, seedMeta] = await Promise.all([
      getCachedJson(
        REDIS_CACHE_KEY,
        true,
      ) as Promise<ListInternetOutagesResponse | null>,
      getCachedJson(SEED_FRESHNESS_KEY, true) as Promise<{
        fetchedAt?: number;
      } | null>,
    ]);
    if (seedData?.outages) {
      const isFresh =
        ...
        Date.now() - seedMeta!.fetchedAt! < SEED_MAX_AGE_MS;
      if (isFresh || !process.env.SEED_FALLBACK_OUTAGES) {
        return {
          outages: filterOutages(seedData.outages, req),
  ...
  try {
    const result = await cachedFetchJson<ListInternetOutagesResponse>(
      ...
      async () => {
        ...
        for (const raw of data.result?.annotations || []) {
          ...
          if (raw.outage?.outageCause)
            categories.push(raw.outage.outageCause.replace(/_/g, " "));
    ...
    return {
      outages: filterOutages(result?.outages || [], req),
      pagination: undefined,
    };
  } catch {
