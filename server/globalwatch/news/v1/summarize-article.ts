...
} from "../../../../src/generated/server/globalwatch/news/v1/service_server";
...
export async function summarizeArticle(
  ...
  const sanitizedGeoContext =
    typeof geoContext === "string"
      ? geoContext.slice(0, MAX_GEO_CONTEXT_LEN)
      : "";
  ...
  try {
    ...
    const { data: result, source } = await cachedFetchJsonWithMeta<{
      ...
      const { systemPrompt, userPrompt } = buildArticlePrompts(
        headlines,
        uniqueHeadlines,
        {
          mode,
      ...
      if (!response.ok) {
        ...
        console.error(
          `[SummarizeArticle:${provider}] API error:`,
          response.status,
          errorText,
        );
        throw new Error(
          response.status === 429 ? "Rate limited" : `${provider} API error`,
        );
      }
      ...
      if (
        ["brief", "analysis"].includes(mode) &&
        hasReasoningPreamble(rawContent)
      ) {
        console.warn(
          `[SummarizeArticle:${provider}] Reasoning preamble detected, rejecting`,
        );
        return null;
      }
    ...
    console.error(
      `[SummarizeArticle:${provider}] Error:`,
      error.name,
      error.message,
    );
    return {
