...
const fallbackDigestCache = new Map<
  string,
  { data: ListFeedDigestResponse; ts: number }
>();
...
async function fetchAndParseRss(
  ...
  try {
    const cached = await cachedFetchJson<ParsedItem[]>(
      cacheKey,
      600,
      async () => {
        ...
        if (!text) {
          ...
          if (relayBase) {
            ...
            const { controller, cleanup } =
              createTimeoutLinkedController(signal);
            try {
...
function extractTag(xml: string, tag: string): string {
  ...
  const plainRe =
    cached?.plain ?? new RegExp(`<${tag}[^>]*>([^<]*)<\\/${tag}>`, "i");
...
export async function listFeedDigest(
  ...
  try {
    const cached = await cachedFetchJson<ListFeedDigestResponse>(
      digestCacheKey,
      900,
      async () => {
...
async function buildDigest(
  ...
  try {
    ...
    if (variant === "full") {
      const filteredIntel = INTEL_SOURCES.filter(
        (f) => !f.lang || f.lang === lang,
      );
