import type {
  ...
  UcdpViolenceEvent,
  UcdpViolenceType,
} from "../../../../src/generated/server/globalwatch/conflict/v1/service_server";
...
async function fetchDirectFromUcdp(): Promise<UcdpViolenceEvent[]> {
  ...
  for (const v of candidates) {
    try {
      const data = (await fetchGedPage(v, 0, token)) as {
        Result?: unknown[];
        TotalPages?: number;
      };
  ...
  for (const raw of allEvents) {
    ...
    mapped.push({
      ...
      violenceType:
        VIOLENCE_TYPE_MAP[Number(e.type_of_violence)] ||
...
export async function listUcdpEvents(
  ...
  try {
    ...
    if (
      raw?.events?.length &&
      (!raw.fetchedAt || Date.now() - raw.fetchedAt < MAX_AGE_MS)
    ) {
  ...
  if (Date.now() - lastDirectFetchMs > DIRECT_FETCH_COOLDOWN_MS) {
    try {
      ...
      if (events.length > 0) {
        ...
        let filtered = events;
        if (req.country)
          filtered = filtered.filter((e) => e.country === req.country);
