...
} from "../../../../src/generated/server/globalwatch/cyber/v1/service_server";
...
function filterSeededThreats(
  ...
  if (req.minSeverity && req.minSeverity !== "CRITICALITY_LEVEL_UNSPECIFIED") {
    ...
    results = results.filter(
      (t) => (SEVERITY_RANK[t.severity || ""] || 0) >= minRank,
    );
  }
  return results;
}
...
export async function listCyberThreats(
  ...
  try {
    ...
    const cached = await cachedFetchJson<CachedThreats>(
      cacheKey,
      REDIS_CACHE_TTL,
      async () => {
        ...
        if (feodoResult.status === "rejected")
          console.warn(
            "[cyber] feodo fetch failed, using partial results:",
            feodoResult.reason,
          );
        if (urlhausResult.status === "rejected")
          console.warn(
            "[cyber] urlhaus fetch failed, using partial results:",
            urlhausResult.reason,
          );
        if (c2intelResult.status === "rejected")
          console.warn(
            "[cyber] c2intel fetch failed, using partial results:",
            c2intelResult.reason,
          );
        if (otxResult.status === "rejected")
          console.warn(
            "[cyber] otx fetch failed, using partial results:",
            otxResult.reason,
          );
        if (abuseipdbResult.status === "rejected")
          console.warn(
            "[cyber] abuseipdb fetch failed, using partial results:",
            abuseipdbResult.reason,
          );
        const feodo =
          feodoResult.status === "fulfilled" ? feodoResult.value : fallback;
        const urlhaus =
          urlhausResult.status === "fulfilled" ? urlhausResult.value : fallback;
        const c2intel =
          c2intelResult.status === "fulfilled" ? c2intelResult.value : fallback;
        const otx =
          otxResult.status === "fulfilled" ? otxResult.value : fallback;
        const abuseipdb =
        ...
        if (req.type && req.type !== "CYBER_THREAT_TYPE_UNSPECIFIED") {
          ...
          results = results.filter(
            (t) => THREAT_TYPE_MAP[t.type] === filterType,
          );
        }
        if (req.source && req.source !== "CYBER_THREAT_SOURCE_UNSPECIFIED") {
          const filterSource = req.source;
          results = results.filter(
            (t) => SOURCE_MAP[t.source] === filterSource,
          );
        }
        if (
          req.minSeverity &&
          req.minSeverity !== "CRITICALITY_LEVEL_UNSPECIFIED"
        ) {
          const minRank = SEVERITY_RANK[req.minSeverity] || 0;
          results = results.filter(
            (t) =>
              (SEVERITY_RANK[SEVERITY_MAP[t.severity] || ""] || 0) >= minRank,
          );
        }
        ...
        const threats = results
          .slice(0, MAX_CACHED_THREATS)
          .map(toProtoCyberThreat);
