...
function sanitizeRawThreat(
  threat: Partial<RawThreat> & { indicator?: string },
): RawThreat | null {
  ...
  return {
    id:
      cleanString(threat.id, 255) ||
      `${threat.source || "feodo"}:${indicatorType}:${indicator}`,
...
const geoCache = new Map<
  string,
  { lat: number; lon: number; country: string; ts: number }
>();
...
function setGeoCached(
  ...
  if (geoCache.size >= GEO_CACHE_MAX_SIZE) {
    const keysToDelete = Array.from(geoCache.keys()).slice(
      0,
      Math.floor(GEO_CACHE_MAX_SIZE / 4),
    );
...
export async function hydrateThreatCoordinates(
  threats: RawThreat[],
): Promise<RawThreat[]> {
...
function inferFeodoSeverity(record: any, malwareFamily: string): string {
  if (/emotet|qakbot|trickbot|dridex|ransom/i.test(malwareFamily))
    return "critical";
...
export async function fetchUrlhausSource(
  limit: number,
  cutoffMs: number,
): Promise<SourceResult> {
...
export async function fetchOtxSource(
  limit: number,
  days: number,
): Promise<SourceResult> {
...
export async function fetchAbuseIpDbSource(
  limit: number,
): Promise<SourceResult> {
