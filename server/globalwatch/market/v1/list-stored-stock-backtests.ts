...
import { getStoredStockBacktestSnapshots } from "./stock-store";

const DEFAULT_EVAL_WINDOW_DAYS = 10;

export const listStoredStockBacktests: MarketServiceHandler["listStoredStockBacktests"] =
  async (
    _ctx,
    ...
    const evalWindowDays = Math.max(
      3,
      Math.min(30, req.evalWindowDays || DEFAULT_EVAL_WINDOW_DAYS),
    );
    const items = await getStoredStockBacktestSnapshots(
      symbols,
      evalWindowDays,
    );
    return { items };
