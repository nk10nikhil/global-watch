...
const fallbackCryptoCache = new Map<
  string,
  { data: ListCryptoQuotesResponse; ts: number }
>();

const SYMBOL_TO_ID = new Map(
  Object.entries(CRYPTO_META).map(([id, m]) => [m.symbol, id]),
);

async function trySeededCrypto(
  ids: string[],
): Promise<ListCryptoQuotesResponse | null> {
  try {
    const [seedData, seedMeta] = await Promise.all([
      getCachedJson(REDIS_CACHE_KEY, true) as Promise<{
        quotes: CryptoQuote[];
      } | null>,
      getCachedJson("seed-meta:market:crypto", true) as Promise<{
        fetchedAt?: number;
      } | null>,
    ]);
    ...
    const filtered =
      allIds.size === 0
        ? seedData.quotes
        : seedData.quotes.filter((q) =>
            allIds.has(SYMBOL_TO_ID.get(q.symbol) ?? ""),
          );

    if (filtered.length === 0) return null;
    if (isFresh || !process.env.SEED_FALLBACK_CRYPTO)
