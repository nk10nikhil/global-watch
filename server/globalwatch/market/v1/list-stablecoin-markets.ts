  ...
  try {
    const [seedData, seedMeta] = await Promise.all([
      getCachedJson(
        REDIS_CACHE_KEY,
        true,
      ) as Promise<ListStablecoinMarketsResponse | null>,
      getCachedJson("seed-meta:market:stablecoins", true) as Promise<{
        fetchedAt?: number;
      } | null>,
    ]);
...
export async function listStablecoinMarkets(
  ...
  if (
    stablecoinCache &&
    now - stablecoinCacheTimestamp < STABLECOIN_CACHE_TTL
  ) {
