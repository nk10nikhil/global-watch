...
const quotesCache = new Map<
  string,
  { data: ListMarketQuotesResponse; timestamp: number }
>();
...
export async function listMarketQuotes(
  ...
  try {
    const bootstrap = (await getCachedJson(
      ...
      true,
    )) as ListMarketQuotesResponse | null;
    if (bootstrap?.quotes?.length) {
      const symbolSet = new Set(parsedSymbols);
      const filtered = bootstrap.quotes.filter((q: MarketQuote) =>
        symbolSet.has(q.symbol),
      );
      if (filtered.length > 0) {
        const resp: ListMarketQuotesResponse = {
          quotes: filtered,
