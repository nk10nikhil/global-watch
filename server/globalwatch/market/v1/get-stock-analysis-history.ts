...
import { getStoredStockAnalysisHistory } from "./stock-store";

const DEFAULT_LIMIT_PER_SYMBOL = 4;
const MAX_LIMIT_PER_SYMBOL = 32;

export const getStockAnalysisHistory: MarketServiceHandler["getStockAnalysisHistory"] =
  async (
    _ctx,
    ...
    const limitPerSymbol = Math.max(
      1,
      Math.min(
        MAX_LIMIT_PER_SYMBOL,
        req.limitPerSymbol || DEFAULT_LIMIT_PER_SYMBOL,
      ),
    );
    const history = await getStoredStockAnalysisHistory(
      symbols,
      !!req.includeNews,
      limitPerSymbol,
    );
