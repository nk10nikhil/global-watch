 ...
 *
 * Inspired by https://github.com/nk10nikhil/globalwatch/pull/641 (@aa5064).
 */
...
export async function listGulfQuotes(
  ...
  try {
    const [seedData, seedMeta] = await Promise.all([
      ...
      getCachedJson("seed-meta:market:gulf-quotes", true) as Promise<{
        fetchedAt?: number;
      } | null>,
    ]);
  ...
  try {
    const result = await cachedFetchJson<ListGulfQuotesResponse>(
      REDIS_KEY,
      REDIS_TTL,
      async () => {
