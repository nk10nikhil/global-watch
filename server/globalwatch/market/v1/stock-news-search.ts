...
import type { StockAnalysisHeadline } from "../../../../src/generated/server/globalwatch/market/v1/service_server";
import { CHROME_UA } from "../../../_shared/constants";
import { cachedFetchJson } from "../../../_shared/redis";
import { UPSTREAM_TIMEOUT_MS } from "./_shared";

export type StockNewsSearchProviderId =
  | "tavily"
  | "brave"
  | "serpapi"
  | "google-news-rss";
...
export function buildStockNewsSearchQuery(
  symbol: string,
  name: string,
): string {
...
async function searchWithBrave(
  ...
  const freshness =
    days <= 1 ? "pd" : days <= 7 ? "pw" : days <= 30 ? "pm" : "py";
  ...
  return dedupeHeadlines(
    rawResults
      .map((item) => ({
        ...
        source:
          String(item.source || "").trim() ||
          extractDomain(String(item.link || "")),
        link: String(item.link || "").trim(),
        publishedAt: relativeDateToTimestamp(item.date),
      }))
      .filter(
        (item) =>
          !item.publishedAt || Date.now() - item.publishedAt <= maxAgeMs,
      ),
    maxResults,
  );
}
...
async function searchViaProviders(
  ...
  for (const provider of providers) {
    ...
    for (const apiKey of candidates) {
      try {
        const headlines = await provider.search(
          query,
          maxResults,
          days,
          apiKey,
      ...
      items.map((item) => {
        const source =
          typeof item.source === "string"
            ? item.source
            : typeof (item.source as Record<string, unknown> | undefined)?.[
                  "#text"
                ] === "string"
