...
} from "../../../../src/generated/server/globalwatch/seismology/v1/service_server";
...
async function trySeededData(): Promise<EarthquakeCache | null> {
  try {
    const [seedData, seedMeta] = await Promise.all([
      ...
      getCachedJson("seed-meta:seismology:earthquakes", true) as Promise<{
        fetchedAt?: number;
      } | null>,
    ]);
...
export const listEarthquakes: SeismologyServiceHandler["listEarthquakes"] =
  async (
    _ctx: ServerContext,
    ...
    try {
      ...
      if (seeded) {
        ...
        return {
          earthquakes: earthquakes.slice(0, pageSize),
          pagination: undefined,
        };
      }

      const cached = await cachedFetchJson<EarthquakeCache>(
        CACHE_KEY,
        CACHE_TTL,
        async () => {
