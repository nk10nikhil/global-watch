...
} from "../../../../src/generated/server/globalwatch/economic/v1/service_server";
...
async function computeMacroSignals(): Promise<GetMacroSignalsResponse> {
  ...
  const jpyChart = await fetchJSON(
    `${yahooBase}/JPY=X?range=1y&interval=1d`,
  ).catch(() => null);
  const btcChart = await fetchJSON(
    `${yahooBase}/BTC-USD?range=1y&interval=1d`,
  ).catch(() => null);
  const qqqChart = await fetchJSON(
    `${yahooBase}/QQQ?range=1y&interval=1d`,
  ).catch(() => null);
  const xlpChart = await fetchJSON(
  ...
  const signalList = [
    ...
    },
    {
      name: "Fear & Greed",
      status: fgLabel,
      bullish: fgValue !== undefined && fgValue > 50,
    },
  ];
...
export async function getMacroSignals(
  ...
  if (
    macroSignalsCached &&
    now - macroSignalsCacheTimestamp < MACRO_CACHE_TTL * 1000
  ) {
  ...
  try {
    ...
    const result = await cachedFetchJson<GetMacroSignalsResponse>(
      ...
      async () => {
        ...
        return !computed.unavailable && computed.totalCount > 0
          ? computed
          : null;
      },
    );
