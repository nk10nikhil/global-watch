...
async function fetchCapacityForSource(
  ...
  for (const row of rows) {
    ...
    const mw =
      typeof row.capability === "number"
        ? row.capability
        : parseFloat(String(row.capability));
...
async function fetchCoalCapacity(
  ...
  const subResults = await Promise.all(
    COAL_SUBTYPES.map((code) =>
      fetchCapacityForSource(code, apiKey, startYear),
    ),
  );
      ...
      async () => {
        ...
        for (const source of requestedSources) {
          try {
            ...
            const dataPoints: EnergyCapacityYear[] = Array.from(
              yearTotals.entries(),
            )
              .sort(([a], [b]) => a - b)
