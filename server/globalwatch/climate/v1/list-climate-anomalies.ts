...
import type {
  ...
  AnomalyType,
  ClimateAnomaly,
} from "../../../../src/generated/server/globalwatch/climate/v1/service_server";
...
async function fetchZone(
  ...
  const tempDelta =
    Math.round((avg(recentTemps) - avg(baselineTemps)) * 10) / 10;
  const precipDelta =
...
async function trySeededData(): Promise<AnomalyCache | null> {
  try {
    const [seedData, seedMeta] = await Promise.all([
      ...
      getCachedJson("seed-meta:climate:anomalies", true) as Promise<{
        fetchedAt?: number;
      } | null>,
    ]);
...
export const listClimateAnomalies: ClimateServiceHandler["listClimateAnomalies"] =
  async (
    _ctx: ServerContext,
      ...
      result = await cachedFetchJson<ListClimateAnomaliesResponse>(
        ...
        async () => {
          ...
          return anomalies.length > 0
            ? { anomalies, pagination: undefined }
            : null;
        },
      );
    } catch {
