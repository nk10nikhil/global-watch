...
import type {
  ...
  TradeRestriction,
} from "../../../../src/generated/server/globalwatch/trade/v1/service_server";
...
function toRestriction(row: any): TradeRestriction | null {
  ...
  const reporterCode = String(
    row.ReportingEconomyCode ?? row.reportingEconomyCode ?? "",
  );
  const year = String(row.Year ?? row.year ?? row.Period ?? "");
  const indicator = String(
    row.Indicator ?? row.indicator ?? row.IndicatorCode ?? "",
  );

  return {
    id: `${reporterCode}-${year}-${row.IndicatorCode ?? ""}`,
    reportingCountry:
      WTO_MEMBER_CODES[reporterCode] ??
...
async function fetchRestrictions(
  ...
  for (const row of dataset) {
    ...
    if (
      !existing ||
      year > parseInt(existing.Year ?? existing.year ?? "0", 10)
    ) {
