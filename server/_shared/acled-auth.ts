 ...
 * Fixes: https://github.com/nk10nikhil/globalwatch/issues/1283
 */
...
async function exchangeCredentials(
  ...
  if (!data.access_token || !data.refresh_token) {
    throw new Error(
      "ACLED OAuth response missing access_token or refresh_token",
    );
  }
      ...
      try {
        ...
        if (memCached?.refreshToken) {
          try {
            ...
            console.warn(
              "[acled-auth] Refresh token expired, re-authenticating",
              refreshErr,
            );
          }
        }
