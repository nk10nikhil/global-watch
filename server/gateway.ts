...
const TIER_HEADERS: Record<CacheTier, string> = {
  ...
  static:
    "public, s-maxage=7200, stale-while-revalidate=600, stale-if-error=14400",
  daily:
    "public, s-maxage=86400, stale-while-revalidate=7200, stale-if-error=172800",
  "no-store": "no-store",
};
...
const TIER_CDN_CACHE: Record<CacheTier, string | null> = {
  ...
  medium:
    "public, s-maxage=1200, stale-while-revalidate=600, stale-if-error=1800",
  slow: "public, s-maxage=3600, stale-while-revalidate=900, stale-if-error=7200",
  static:
    "public, s-maxage=14400, stale-while-revalidate=3600, stale-if-error=28800",
  daily:
...
export function createDomainGateway(
  ...
  return async function handler(originalRequest: Request): Promise<Response> {
    ...
    const endpointRlResponse = await checkEndpointRateLimit(
      request,
      pathname,
      corsHeaders,
    );
