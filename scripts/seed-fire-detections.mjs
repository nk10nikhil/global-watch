...
async function fetchRegionSource(apiKey, regionName, bbox, source) {
  ...
  if (!res.ok)
    throw new Error(`FIRMS ${res.status} for ${regionName}/${source}`);
  ...
  for (const source of FIRMS_SOURCES) {
    for (const [regionName, bbox] of entries) {
      try {
        ...
        failed++;
        console.error(
          `  [FIRMS] ${source}/${regionName}: ${err.message || err}`,
        );
      }
      await sleep(6_000); // FIRMS free tier: 10 req/min — 6s between calls stays safely under limit
    }
    console.log(
      `  ${source}: ${fireDetections.length} total (${fulfilled} ok, ${failed} failed)`,
    );
  }
...
async function main() {
  ...
  await runSeed(
    ...
    "fires",
    CANONICAL_KEY,
    () => fetchAllRegions(apiKey),
    {
      validateFn: (data) =>
        Array.isArray(data?.fireDetections) && data.fireDetections.length > 0,
      ttlSeconds: 7200,
