 ...
 *   rclone copy public/data/country-boundary-overrides.geojson r2:GLOBALWATCH-maps/
 *
...
async function main() {
  ...
  const pak = data.features.find(
    (f) =>
      f.properties?.ISO_A2 === "PK" ||
      f.properties?.["ISO3166-1-Alpha-2"] === "PK",
  );
  if (!pak) {
