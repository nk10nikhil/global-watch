...
function classifyGdeltEventType(name) {
  ...
  if (lowerName.includes("demonstration"))
...
async function fetchGdeltEvents() {
  ...
  for (const [, loc] of locationMap) {
    ...
    events.push({
      ...
      actors: [],
      confidence:
        loc.count > 20 ? "CONFIDENCE_LEVEL_HIGH" : "CONFIDENCE_LEVEL_MEDIUM",
    });
  }

  console.log(
    `  GDELT: ${features.length} mentions → ${events.length} aggregated events`,
  );
  return events;
}
...
async function fetchUnrestEvents() {
  ...
  if (results[0].status === "rejected")
    console.log(
      `  ACLED failed: ${results[0].reason?.message || results[0].reason}`,
    );
  if (results[1].status === "rejected")
    console.log(
      `  GDELT failed: ${results[1].reason?.message || results[1].reason}`,
    );
