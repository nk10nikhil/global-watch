...
async function fetchGedPage(version, page, token) {
  ...
  if (!resp.ok)
    throw new Error(
      `UCDP GED API error (${version}, page ${page}): ${resp.status}`,
    );
  return resp.json();
}
...
async function discoverVersion(token) {
  ...
  for (const version of candidates) {
    try {
      ...
      console.log(
        `  Found v${version} with ${page0.Result.length} events on page 0`,
      );
...
async function main() {
  ...
  console.log(
    `  UCDP Token: ${ucdpToken ? maskToken(ucdpToken) : "(none — unauthenticated)"}`,
  );
  console.log();
  ...
  for (
    ...
    if (page === 0) {
      ...
      pagesToFetch.push(
        fetchGedPage(version, page, ucdpToken).catch(() => FAILED),
      );
    }
  }
  ...
  console.log(
    `  Raw events: ${allEvents.length} | Failed pages: ${failedPages}`,
  );
  ...
  const mapped = filtered.map((e) => ({
    ...
    violenceType:
      VIOLENCE_TYPE_MAP[e.type_of_violence] || "UCDP_VIOLENCE_TYPE_UNSPECIFIED",
  ...
  if (mapped.length > MAX_EVENTS)
    console.log(`  Capped: ${mapped.length} → ${MAX_EVENTS}`);
  ...
  if (!resp.ok) {
    ...
    console.error(
      `Redis SET failed: HTTP ${resp.status} — ${text.slice(0, 200)}`,
    );
    process.exit(1);
  }
