...
test("RSS proxy: negative caching prevents thundering herd on 429", async (t) => {
  ...
  const r1 = await fetch(
    `http://127.0.0.1:${proxyPort}/?url=${encodeURIComponent(feedUrl)}`,
  );
...
test("RSS proxy: FIFO eviction caps cache size", async (t) => {
  ...
  assert.ok(
    !proxy.cache.has("http://example.com/feed-0"),
    "Oldest entry should be evicted",
  );
  assert.ok(
    proxy.cache.has("http://example.com/feed-new"),
...
test("RSS proxy: conditional GET returns REVALIDATED on 304", async (t) => {
  ...
  upstream.setResponse(
    200,
    "<rss><channel><title>Conditional</title></channel></rss>",
  );
  ...
  assert.equal(
    upstream.getHitCount(),
    2,
    "Should not hit upstream — cache refreshed by 304",
  );
...
test("RSS proxy: conditional GET with If-Modified-Since", async (t) => {
  ...
  upstream.setResponse(
    200,
    "<rss><channel><title>LM Test</title></channel></rss>",
  );
...
test("RSS proxy: stale-on-error resolves in-flight (no hang)", async (t) => {
  ...
  const r2 = await Promise.race([
    ...
    new Promise((_, reject) =>
      setTimeout(
        () => reject(new Error("Request hung — in-flight not settled")),
        3000,
      ),
    ),
  ]);
  ...
  assert.ok(
    r2.status === 200 || r2.status === 502,
    `Expected stale/error, got ${r2.status}`,
  );
  ...
  assert.equal(
    proxy.inFlight.size,
    0,
    "In-flight map should be empty after settlement",
  );
