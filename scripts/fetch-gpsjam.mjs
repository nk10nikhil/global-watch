...
function processHexes(rawHexes) {
  ...
  for (const hex of rawHexes) {
    ...
    if (!Number.isFinite(hex.npAvg)) {
      console.error(
        `[gpsjam] WARN: skipping hex ${hex.h3Index} — npAvg not finite: ${hex.npAvg}`,
      );
      skipped++;
      continue;
    }
...
async function seedRedis(output) {
  ...
  if (!redisUrl || !redisToken) {
    console.error(
      "[gpsjam] No UPSTASH_REDIS_REST_URL/TOKEN — skipping Redis seed",
    );
    return;
  }

  console.error(
    `[gpsjam] Seeding Redis keys "${REDIS_KEY_V2}" and "${REDIS_KEY_V1}"...`,
  );
  ...
  if (!v2Resp.ok) {
    ...
    console.error(
      `[gpsjam] Redis SET v2 failed: HTTP ${v2Resp.status} — ${text.slice(0, 200)}`,
    );
    return;
  }
  ...
  if (!v1Resp.ok) {
    ...
    console.error(
      `[gpsjam] Redis SET v1 failed: HTTP ${v1Resp.status} — ${text.slice(0, 200)}`,
    );
  } else {
  ...
  const getResp = await fetch(
    `${redisUrl}/get/${encodeURIComponent(REDIS_KEY_V2)}`,
    {
  ...
  const meta = {
    fetchedAt: Date.now(),
    recordCount: output.hexes?.length || 0,
  };
