...
async function fetchSubmarineCables() {
  ...
  console.log(
    `  Fetching ${allIds.length} strategic cables from TeleGeography...`,
  );
  ...
  if (!cableGeoResp.ok)
    throw new Error(`cable-geo.json: HTTP ${cableGeoResp.status}`);
  ...
  if (!lpGeoResp.ok)
    throw new Error(`landing-point-geo.json: HTTP ${lpGeoResp.status}`);
  ...
  for (let i = 0; i < allIds.length; i += 5) {
    ...
    for (const result of results) {
          ...
          if (coords) {
            landingPoints.push({
              country: getCountryCode(
                lp.country || coords.name?.split(",").pop()?.trim() || "",
              ),
      ...
      const share =
        countries.length > 0
          ? Math.min(Math.round(100 / countries.length) / 100, 0.3)
          : 0;
  ...
  for (const cable of cables) {
    if (seenIds.has(cable.id)) {
      throw new Error(
        `ID collision: '${cable.id}' from '${seenIds.get(cable.id)}' and current`,
      );
    }
