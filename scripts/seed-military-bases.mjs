...
const R2_BUCKET_URL =
  "https://api.cloudflare.com/client/v4/accounts/{acct}/r2/buckets/GLOBALWATCH-data/objects/seed-data/military-bases-final.json";
...
function parseArgs() {
  ...
  if (!valid.includes(env)) {
    console.error(
      `Invalid --env "${env}". Must be one of: ${valid.join(", ")}`,
    );
    process.exit(1);
  }
...
async function seedGeo(url, token, geoKey, entries) {
  ...
  for (let i = 0; i < total; i += BATCH_SIZE) {
    ...
    if (seeded % PROGRESS_INTERVAL === 0 || seeded === total) {
      console.log(
        `  GEO: ${seeded.toLocaleString()} / ${total.toLocaleString()}`,
      );
    }
  }
...
async function seedMeta(url, token, metaKey, entries) {
  ...
  for (let i = 0; i < total; i += BATCH_SIZE) {
    ...
    if (seeded % PROGRESS_INTERVAL === 0 || seeded === total) {
      console.log(
        `  META: ${seeded.toLocaleString()} / ${total.toLocaleString()}`,
      );
    }
  }
...
async function validate(url, token, prefix, version, expectedCount) {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    try {
      ...
      throw new Error(
        `Sample ID "${sampleIds[i]}" has invalid JSON in META hash`,
      );
    }
  }

  console.log(
    `  Sampled ${parseOk}/${sampleIds.length} entries — all valid JSON`,
  );
...
async function cleanupOldVersion(url, token, prefix, newVersion) {
  ...
  if (!currentActive || String(currentActive) === String(newVersion))
    return null;
...
async function main() {
  ...
  if (!redisUrl) {
    console.error(
      "Missing UPSTASH_REDIS_REST_URL. Set it in .env.local or as an env var.",
    );
    process.exit(1);
  }
  if (!redisToken) {
    console.error(
      "Missing UPSTASH_REDIS_REST_TOKEN. Set it in .env.local or as an env var.",
    );
    process.exit(1);
  }
  ...
  let dataPath = existsSync(volumePath)
    ? volumePath
  ...
  if (!dataPath) {
    const cfToken =
      process.env.CLOUDFLARE_R2_TOKEN || process.env.CLOUDFLARE_API_TOKEN || "";
    if (cfToken) {
      ...
      try {
        ...
        if (resp.ok) {
          ...
          dataPath = localPath;
          console.log(
            `  Downloaded ${(body.length / 1024 / 1024).toFixed(1)}MB from R2`,
          );
        } else {
  ...
  if (!dataPath) {
    const activeKey = `${prefix}military:bases:active`;
    const check = await pipelineRequest(redisUrl, redisToken, [
      ["GET", activeKey],
    ]);
    const existing = check[0]?.result;
    if (existing) {
      console.log(
        `No data file found — Redis already has active version ${existing}, skipping.`,
      );
      process.exit(0);
    }
    console.error(
      `Data file not found locally or on R2, and no existing data in Redis.`,
    );
    process.exit(1);
  }
  ...
  const oldInfo = await cleanupOldVersion(
    redisUrl,
    redisToken,
    prefix,
    version,
  ...
  if (oldInfo) {
    ...
    console.log(
      `  Will clean up after grace period: ${oldInfo.oldGeoKey}, ${oldInfo.oldMetaKey}`,
    );
  }
  ...
  if (oldInfo) {
    ...
    console.log(
      `Cleaning up old keys: ${oldInfo.oldGeoKey}, ${oldInfo.oldMetaKey}`,
    );
