...
console.log(
  `[Relay] Heap limit: ${(_heapStats.heap_size_limit / 1024 / 1024).toFixed(0)}MB`,
);
...
if (!API_KEY) {
  console.error(
    "[Relay] Error: AISSTREAM_API_KEY environment variable not set",
  );
...
const UPSTREAM_QUEUE_LOW_WATER = Math.max(
  ...
  Math.min(
    ...
    Number(process.env.AIS_UPSTREAM_QUEUE_LOW_WATER || 1000),
  ),
);
...
const UPSTREAM_DRAIN_BATCH = Math.max(
  1,
  Number(process.env.AIS_UPSTREAM_DRAIN_BATCH || 250),
);
...
const MAX_VESSEL_HISTORY = safeInt(
...
const RELAY_AUTH_HEADER = (
  process.env.RELAY_AUTH_HEADER || "x-relay-key"
).toLowerCase();
const ALLOW_UNAUTHENTICATED_RELAY =
...
const RELAY_RATE_LIMIT_MAX = Number.isFinite(
...
const RELAY_OPENSKY_RATE_LIMIT_MAX = Number.isFinite(
  ...
  ? Number(process.env.RELAY_OPENSKY_RATE_LIMIT_MAX)
  : 600;
const RELAY_RSS_RATE_LIMIT_MAX = Number.isFinite(
  Number(process.env.RELAY_RSS_RATE_LIMIT_MAX),
)
  ? Number(process.env.RELAY_RSS_RATE_LIMIT_MAX)
  : 300;
const RELAY_LOG_THROTTLE_MS = Math.max(
  1000,
  Number(process.env.RELAY_LOG_THROTTLE_MS || 10000),
);
...
const OPENSKY_PROXY_AUTH =
  process.env.OPENSKY_PROXY_AUTH || process.env.OREF_PROXY_AUTH || "";
...
const OREF_LOCAL_FILE = (() => {
  ...
  try {
    ...
    if (!stat.isDirectory()) {
      console.warn(
        `[Relay] OREF_DATA_DIR is not a directory: ${OREF_DATA_DIR}`,
      );
      return "";
    }
  } catch {
...
const RELAY_OREF_RATE_LIMIT_MAX = Number.isFinite(
...
if (
  ...
  console.error(
    "[Relay] Set RELAY_SHARED_SECRET on Railway and Vercel to secure relay endpoints",
  );
  console.error(
...
const RELAY_ENV_PREFIX = process.env.RELAY_ENV
...
if (UPSTASH_REDIS_REST_URL && !UPSTASH_REDIS_REST_URL.startsWith("https://")) {
  console.warn(
    "[Relay] UPSTASH_REDIS_REST_URL must start with https:// — Redis disabled",
  );
}
if (UPSTASH_ENABLED) {
    ...
    zlib.gzip(
      ...
      (err, compressed) => {
        ...
        safeEnd(
          res,
          statusCode,
          { ...headers, "Content-Encoding": "gzip", Vary: vary },
          compressed,
        );
      },
    );
...
const TELEGRAM_MAX_FEED_ITEMS = Math.max(
  50,
  Number(process.env.TELEGRAM_MAX_FEED_ITEMS || 200),
);
const TELEGRAM_MAX_TEXT_CHARS = Math.max(
  200,
  Number(process.env.TELEGRAM_MAX_TEXT_CHARS || 800),
);
...
function loadTelegramChannels() {
  ...
  try {
    ...
    if (!telegramState.channels.length) {
      console.warn(
        `[Relay] Telegram channel set "${set}" is empty — no channels to poll`,
      );
    }
...
async function initTelegramClientIfNeeded() {
  ...
  try {
    ...
    if (
      ...
      /Cannot find package|Directory import/.test(em)
    ) {
...
async function pollTelegramOnce() {
  ...
  const channels = telegramState.channels.length
    ? telegramState.channels
  ...
  for (const channel of channels) {
    ...
    try {
      ...
      await new Promise((r) =>
        setTimeout(
          r,
          Math.max(300, Number(process.env.TELEGRAM_RATE_LIMIT_MS || 800)),
        ),
      );
    } catch (e) {
      const em = e?.message || String(e);
      ...
      if (/FLOOD_WAIT/.test(em)) {
        ...
        console.warn(
          `[Relay] Telegram FLOOD_WAIT ${wait}s — stopping poll cycle early`,
        );
        break;
      }
    }
    ...
    orefState.historyCount24h = orefState.history
      ...
      .reduce(
        (sum, h) =>
          sum +
          h.alerts.reduce(
            (s, a) => s + (Array.isArray(a.data) ? a.data.length : 1),
            0,
          ),
        0,
      );
...
function orefLoadLocalHistory() {
  ...
  try {
    ...
    if (err.code !== "ENOENT")
      console.warn("[Relay] OREF local file read error:", err.message);
    return null;
  }
}
...
async function orefBootstrapHistoryWithRetry() {
  ...
  try {
    ...
    if (cached && Array.isArray(cached.history) && cached.history.length > 0) {
      ...
      if (valid) {
        ...
        console.log(
          "[Relay] OREF Redis data all stale (>7d) — falling through to upstream",
        );
      }
    }
  } catch (err) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      console.log(
        `[Relay] OREF upstream bootstrap succeeded on attempt ${attempt}`,
      );
      return;
    } catch (err) {
  ...
  console.warn(
    "[Relay] OREF bootstrap exhausted all attempts — starting with empty history",
  );
}
...
async function startOrefPollLoop() {
  ...
  orefFetchAlerts().catch((e) =>
    console.warn("[Relay] OREF initial poll error:", e?.message || e),
  );
  setInterval(() => {
    orefFetchAlerts().catch((e) =>
      console.warn("[Relay] OREF poll error:", e?.message || e),
    );
  }, OREF_POLL_INTERVAL_MS).unref?.();
  console.log(
    `[Relay] OREF poll loop started (interval ${OREF_POLL_INTERVAL_MS}ms)`,
  );
}
...
// ─────────────────────────────────────────────────────────────
const UCDP_ACCESS_TOKEN = (
  process.env.UCDP_ACCESS_TOKEN ||
  process.env.UC_DP_KEY ||
  ""
).trim();
    ...
    const req = https.request(
      ...
      (resp) => {
        ...
        if (resp.statusCode < 200 || resp.statusCode >= 300) {
          resp.resume();
          return reject(
            new Error(`UCDP ${version} page ${page}: HTTP ${resp.statusCode}`),
          );
        }
        let data = "";
        resp.on("data", (chunk) => {
        ...
        resp.on("end", () => {
          try {
            ...
            if (typeof parsed === "string")
              return reject(
                new Error(`UCDP ${version} page ${page}: ${parsed}`),
              );
            resolve(parsed);
          } catch (e) {
            reject(e);
...
async function ucdpDiscoverVersion() {
  ...
  const candidates = [
    ...new Set([`${year}.1`, `${year - 1}.1`, "25.1", "24.1"]),
  ];
  ...
  const attempts = candidates.map(async (v) => {
    ...
    if (!Array.isArray(p0?.Result) || p0.Result.length === 0)
      throw new Error(`${v}: no results`);
  ...
  try {
    ...
    const reasons =
      aggErr.errors?.map((e) => e?.message).join("; ") || aggErr.message;
...
async function seedUcdpEvents() {
  try {
    ...
    for (
      let offset = 0;
      offset < UCDP_MAX_PAGES && newestPage - offset >= 0;
      offset++
    ) {
      ...
      fetches.push(
        pg === 0
          ...
          : ucdpFetchPage(version, pg).catch(() => FAILED),
      );
    }
    ...
    for (const raw of pageResults) {
      ...
      for (const e of events) {
        ...
        if (
          Number.isFinite(ms) &&
          (!Number.isFinite(latestMs) || ms > latestMs)
        )
          latestMs = ms;
      }
    }
    ...
    const mapped = filtered
      .map((e) => ({
        ...
        violenceType:
          UCDP_VIOLENCE_TYPE_MAP[e.type_of_violence] ||
...
async function startUcdpSeedLoop() {
  ...
  seedUcdpEvents().catch((e) =>
    console.warn("[UCDP] Initial seed error:", e?.message || e),
  );
  setInterval(() => {
    seedUcdpEvents().catch((e) =>
      console.warn("[UCDP] Seed error:", e?.message || e),
...
function satClassify(name) {
  ...
  if (/COSMO-SKYMED|TERRASAR|PAZ|SAR-LUPE|YAOGAN/i.test(n)) type = "sar";
    ...
    /WORLDVIEW|SKYSAT|PLEIADES|KOMPSAT|GAOFEN|JILIN|CARTOSAT|ZIYUAN/i.test(n)
  )
    type = "optical";
  ...
  else if (/^WORLDVIEW|^SAPPHIRE|^PRAETORIAN|^USA|^SKYSAT/i.test(n))
    country = "US";
  else if (
    /^SENTINEL|^COSMO-SKYMED|^TERRASAR|^SAR-LUPE|^PAZ|^PLEIADES/i.test(n)
  )
    country = "EU";
          ...
          const req = https.request(
            ...
            (resp) => {
              if (resp.statusCode < 200 || resp.statusCode >= 300) {
                resp.resume();
                return reject(
                  new Error(`CelesTrak ${group}: HTTP ${resp.statusCode}`),
                );
              }
              let data = "";
              let size = 0;
...
async function startSatelliteSeedLoop() {
  ...
  console.log(
    `[Satellites] Seed loop starting (interval ${SAT_SEED_INTERVAL_MS / 1000 / 60}min)`,
  );
...
function fetchYahooChartDirect(symbol) {
  return new Promise((resolve) => {
    ...
    req.on("timeout", () => {
      ...
      logThrottled(
        "warn",
        `market-yahoo-timeout:${symbol}`,
        `[Market] Yahoo ${symbol} timeout`,
      );
      resolve(null);
    });
  });
...
async function fetchCryptoCoinPaprika() {
  const data = await cyberHttpGetJson(
  ...
  const paprikaIds = new Set(
    CRYPTO_IDS.map((id) => CRYPTO_PAPRIKA_MAP[id]).filter(Boolean),
  );
  const reverseMap = Object.fromEntries(
    Object.entries(CRYPTO_PAPRIKA_MAP).map(([g, p]) => [p, g]),
  );
  return data
...
async function seedCryptoQuotes() {
  ...
  try {
    ...
    console.warn(
      `[Crypto] CoinGecko failed: ${err.message} — trying CoinPaprika`,
    );
    try {
      data = await fetchCryptoCoinPaprika();
    } catch (e2) {
      console.warn(
        `[Crypto] CoinPaprika also failed: ${e2.message} — skipping`,
      );
      return 0;
    }
  }
...
async function fetchStablecoinCoinPaprika() {
  ...
  const paprikaIds = new Set(
    ids.map((id) => STABLECOIN_PAPRIKA_MAP[id]).filter(Boolean),
  );
  const reverseMap = Object.fromEntries(
...
async function seedStablecoinMarkets() {
  ...
  try {
    ...
    console.warn(
      `[Stablecoin] CoinGecko failed: ${err.message} — trying CoinPaprika`,
    );
    try {
      data = await fetchStablecoinCoinPaprika();
    } catch (e2) {
      console.warn(
        `[Stablecoin] CoinPaprika also failed: ${e2.message} — skipping`,
      );
      return 0;
    }
  }
...
async function seedAllMarketData() {
  ...
  console.log(
    `[Market] Seed complete: ${q} quotes, ${c} commodities, ${s} sectors, ${g} gulf, ${e} etf, ${cr} crypto, ${sc} stablecoins (${((Date.now() - t0) / 1000).toFixed(1)}s)`,
  );
}
...
async function startMarketDataSeedLoop() {
  if (process.env.DISABLE_RELAY_MARKET_SEED) {
    console.log(
      "[Market] Relay market seeding disabled via DISABLE_RELAY_MARKET_SEED",
    );
    return;
  }
  if (!UPSTASH_ENABLED) {
  ...
  seedAllMarketData().catch((e) =>
    console.warn("[Market] Initial seed error:", e?.message || e),
  );
  setInterval(() => {
    seedAllMarketData().catch((e) =>
      console.warn("[Market] Seed error:", e?.message || e),
    );
...
function aviationAggregateFlights(iata, flights) {
  ...
  const total =
    resolved >= AVIATION_MIN_FLIGHTS_FOR_CLOSURE ? resolved : flights.length;
...
async function seedAviationDelays() {
  ...
  for (
    let i = 0;
    i < AVIATIONSTACK_AIRPORTS.length;
    i += AVIATION_BATCH_CONCURRENCY
  ) {
    ...
    const chunk = AVIATIONSTACK_AIRPORTS.slice(
      i,
      i + AVIATION_BATCH_CONCURRENCY,
    );
    const results = await Promise.allSettled(
      chunk.map((iata) =>
        fetchAviationStackSingle(AVIATIONSTACK_API_KEY, iata),
      ),
    );
  ...
  if (!healthy) {
    console.warn(
      `[Aviation] Systemic failure: ${failed}/${failed + succeeded} airports failed — preserving existing cache`,
    );
    return;
  }
...
async function startAviationSeedLoop() {
  ...
  setInterval(() => {
    seedAviationDelays().catch((e) =>
      console.warn("[Aviation] Seed error:", e?.message || e),
    );
  ...
  for (const n of notams) {
    ...
    const isClosureCode =
      ...
      (code45 === "LC" ||
        ...
        code45 === "AW");
    const isClosureText =
      /\b(AD CLSD|AIRPORT CLOSED|AIRSPACE CLOSED|AD NOT AVBL|CLSD TO ALL)\b/.test(
        text,
      );
...
function startNotamSeedLoop() {
  ...
  setInterval(() => {
    seedNotamClosures().catch((e) =>
      console.warn("[NOTAM-Seed] Seed error:", e?.message || e),
    );
...
function cyberToMs(v) {
  ...
  const d2 = new Date(
    raw.replace(" UTC", "Z").replace(" GMT", "Z").replace(" ", "T"),
  );
...
function cyberSanitize(t) {
  ...
  return {
    id:
      cyberClean(t.id, 255) ||
      `${t.source || "feodo"}:${t.indicatorType || "ip"}:${ind}`,
  ...
  for (const t of threats) {
    ...
    if ((t.lastSeen || t.firstSeen) >= (ex.lastSeen || ex.firstSeen))
      map.set(key, {
        ...ex,
        ...t,
        tags: cyberNormTags([...ex.tags, ...t.tags]),
      });
  }
...
function cyberToProto(t) {
  return {
    ...
    indicatorType:
      CYBER_INDICATOR_MAP[t.indicatorType] ||
      "CYBER_THREAT_INDICATOR_TYPE_UNSPECIFIED",
    location: cyberValidCoords(t.lat, t.lon)
...
function cyberHttpGetJson(url, reqHeaders, timeoutMs) {
  return new Promise((resolve) => {
    const req = https.get(
      url,
      {
        headers: { "User-Agent": CHROME_UA, ...reqHeaders },
...
async function cyberGeoLookup(ip) {
  ...
  const d1 = await cyberHttpGetJson(
...
async function cyberFetchFeodo(limit, cutoffMs) {
  try {
    ...
    for (const r of records) {
      ...
      const status = cyberClean(
        r?.status || r?.c2_status || "",
        30,
      ).toLowerCase();
      if (status && status !== "online" && status !== "offline") continue;
      const firstSeen = cyberToMs(
        r?.first_seen || r?.first_seen_utc || r?.dateadded,
      );
      ...
      if ((lastSeen || firstSeen) && (lastSeen || firstSeen) < cutoffMs)
        continue;
      const malwareFamily = cyberClean(
        r?.malware || r?.malware_family || r?.family,
        80,
      );
      const sev =
        status === "online"
...
async function cyberFetchUrlhaus(limit, cutoffMs) {
  ...
  try {
    ...
    for (const r of rows) {
      ...
      const recordIp = cyberClean(
        r?.host || r?.ip_address || r?.ip,
        80,
      ).toLowerCase();
      const ipCandidate = cyberIsIp(recordIp)
        ? recordIp
        : cyberIsIp(hostname)
          ? hostname
          : "";
      ...
      const firstSeen = cyberToMs(
        r?.dateadded || r?.firstseen || r?.first_seen,
      );
      const lastSeen = cyberToMs(
        r?.last_online || r?.last_seen || r?.dateadded,
      );
      if ((lastSeen || firstSeen) && (lastSeen || firstSeen) < cutoffMs)
        continue;
      const threat = cyberClean(
        r?.threat || r?.threat_type || "",
        40,
      ).toLowerCase();
  ...
  try {
    ...
    for (const line of text.split("\n")) {
      ...
      const t = cyberSanitize({
        ...
        lon: null,
        country: "",
        severity: /cobaltstrike|cobalt.strike|brute.?ratel/i.test(desc)
          ? "high"
          : "medium",
        malwareFamily,
...
async function cyberFetchOtx(limit, days) {
  ...
  try {
    ...
    for (const r of results) {
      ...
      const t = cyberSanitize({
        id: `otx:${ip}`,
        type: tags.some((tt) => /c2|botnet/.test(tt))
          ? "c2_server"
          : "malware_host",
        source: "otx",
        indicator: ip,
        ...
        lon: null,
        country: "",
        severity: tags.some((tt) => /ransomware|apt|c2|botnet/.test(tt))
          ? "high"
          : "medium",
...
async function seedCyberThreats() {
  ...
  if (
    ...
    console.warn(
      "[Cyber] All sources returned 0 threats — skipping Redis write",
    );
    return 0;
  }

  const combined = cyberDedupe([
    ...feodo,
    ...urlhaus,
    ...c2intel,
    ...otx,
  ...
  const geoCount = hydrated.filter((t) =>
    cyberValidCoords(t.lat, t.lon),
  ).length;
  ...
  hydrated.sort((a, b) => {
    ...
    return bySev !== 0
      ? bySev
      : (b.lastSeen || b.firstSeen) - (a.lastSeen || a.firstSeen);
  });
...
async function startCyberThreatsSeedLoop() {
  ...
  seedCyberThreats().catch((e) =>
    console.warn("[Cyber] Initial seed error:", e?.message || e),
  );
  setInterval(() => {
    seedCyberThreats().catch((e) =>
      console.warn("[Cyber] Seed error:", e?.message || e),
    );
        ...
        resp.on("end", () => {
          try {
            ...
            for (const f of features) {
              ...
              if (!name) continue;
              if (name.startsWith("ERROR:") || name.includes("unknown error"))
                continue;
...
async function seedPositiveEvents() {
  ...
  try {
    ...
    if (!anyQuerySucceeded) {
      console.warn(
        "[PositiveEvents] All queries failed — preserving last good data",
      );
      return;
    }
...
async function seedClassifyForVariant(variant, apiKey, apiUrl, model) {
  ...
  if (misses.length === 0)
    return { total: titleArr.length, classified: 0, skipped: 0 };
  ...
  for (let b = 0; b < misses.length; b += CLASSIFY_BATCH_SIZE) {
    ...
    for (const entry of llmResult) {
      ...
      const category = CLASSIFY_VALID_CATEGORIES.includes(entry?.c)
        ? entry.c
        : null;
...
async function seedClassify() {
  ...
  try {
    ...
    if (
      ...
      console.warn(
        "[Classify] LLM_API_URL must be HTTPS in production — skipping",
      );
      return;
    }
    ...
    for (let v = 0; v < CLASSIFY_VARIANTS.length; v++) {
      if (v > 0)
        await new Promise((r) => setTimeout(r, CLASSIFY_VARIANT_STAGGER_MS));
      try {
        const stats = await seedClassifyForVariant(
          CLASSIFY_VARIANTS[v],
          apiKey,
          apiUrl,
          model,
        ...
        console.warn(
          `[Classify] ${CLASSIFY_VARIANTS[v]} error:`,
          e?.message || e,
        );
      }
    }
...
async function startClassifySeedLoop() {
  ...
  const apiKey = process.env.LLM_API_KEY || process.env.GROQ_API_KEY;
  console.log(
    `[Classify] Seed loop starting (interval ${CLASSIFY_SEED_INTERVAL_MS / 1000 / 60}min, apiKey:${apiKey ? "yes" : "no"})`,
  );
  seedClassify().catch((e) =>
    console.warn("[Classify] Initial seed error:", e?.message || e),
  );
  setInterval(() => {
    seedClassify().catch((e) =>
      console.warn("[Classify] Seed error:", e?.message || e),
...
const THEATER_MIL_SHORT_PREFIXES = [
  "AE",
  "RF",
  "TF",
  "PAT",
...
function theaterIsMilCallsign(callsign) {
  ...
  for (const prefix of THEATER_MIL_SHORT_PREFIXES) {
    if (
      cs.startsWith(prefix) &&
      cs.length > prefix.length &&
      /\d/.test(cs.charAt(prefix.length))
    )
      return true;
  }
...
function theaterDetectAircraftType(callsign) {
  ...
  if (/^(RCH|REACH|MOOSE|EVAC|DUSTOFF|C17|C5|C130|C40)/.test(cs))
    return "transport";
  if (/^(HOMER|OLIVE|JAKE|PSEUDO|GORDO|RC|U2|SR)/.test(cs))
    return "reconnaissance";
  if (/^(RQ|MQ|REAPER|PREDATOR|GLOBAL)/.test(cs)) return "drone";
  if (/^(DEATH|BONE|DOOM|B52|B1|B2)/.test(cs)) return "bomber";
  if (
    /^(BOLT|VIPER|RAPTOR|BRONCO|EAGLE|HORNET|FALCON|STRIKE|TANGO|FURY)/.test(cs)
  )
    return "fighter";
  return "unknown";
}
...
async function fetchTheaterFlightsFromOpenSky() {
  ...
  for (const region of THEATER_QUERY_REGIONS) {
    ...
    if (!resp.ok)
      throw new Error(`OpenSky proxy ${resp.status} for ${region.name}`);
    const data = await resp.json();
...
async function seedTheaterPosture() {
  ...
  if (flights.length === 0) {
    console.warn(
      "[TheaterPosture] No military flights from OpenSky or Wingbits — skipping",
    );
    return;
  }
  ...
  await upstashSet(
    ...
    { fetchedAt: Date.now(), recordCount: flights.length },
    604800,
  );
  const elevated = theaters.filter((t) => t.postureLevel !== "normal").length;
  const elapsed = ((Date.now() - t0) / 1000).toFixed(1);
  console.log(
    `[TheaterPosture] Seeded ${flights.length} mil flights, ${theaters.length} theaters (${elevated} elevated), redis: ${ok1 && ok2 && ok3 ? "OK" : "PARTIAL"} [${elapsed}s]`,
  );
}
...
function startCiiWarmPingLoop() {
  ...
  seedCiiWarmPing().catch((e) =>
    console.warn("[CII] Initial warm-ping error:", e?.message || e),
  );
  setInterval(() => {
    seedCiiWarmPing().catch((e) =>
      console.warn("[CII] Warm-ping error:", e?.message || e),
    ...
    const alerts = features
      ...
      .map((f) => {
        ...
        try {
          const g = f.geometry;
          if (g?.type === "Polygon")
            coords = g.coordinates[0]?.map((c) => [c[0], c[1]]) || [];
...
async function startWeatherSeedLoop() {
  ...
  console.log(
    `[Weather] Seed loop starting (interval ${WEATHER_SEED_INTERVAL_MS / 1000 / 60}min)`,
  );
  seedWeatherAlerts().catch((e) =>
    console.warn("[Weather] Initial seed error:", e?.message || e),
  );
  setInterval(() => {
    seedWeatherAlerts().catch((e) =>
      console.warn("[Weather] Seed error:", e?.message || e),
...
async function startSpendingSeedLoop() {
  ...
  seedUsaSpending().catch((e) =>
    console.warn("[Spending] Initial seed error:", e?.message || e),
  );
  setInterval(() => {
    seedUsaSpending().catch((e) =>
      console.warn("[Spending] Seed error:", e?.message || e),
      ...
      url,
      {
        headers: {
          "User-Agent":
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
          Accept:
            "text/calendar, application/rss+xml, application/xml, text/xml, */*",
        },
        timeout: 15000,
      },
      (response) => {
...
async function seedTechEvents() {
  ...
  try {
    ...
    if (events.length === 0) {
      console.warn(
        "[TechEvents] No events from any source — preserving last good data",
      );
      return;
    }
    ...
    const ok1 = await upstashSet(
      TECH_EVENTS_REDIS_KEY,
      payload,
      TECH_EVENTS_TTL_SECONDS,
    );
    const ok2 = await upstashSet(
      TECH_EVENTS_BOOTSTRAP_KEY,
      payload,
      TECH_EVENTS_TTL_SECONDS,
    );
    const ok3 = await upstashSet(
...
async function startTechEventsSeedLoop() {
  ...
  seedTechEvents().catch((e) =>
    console.warn("[TechEvents] Initial seed error:", e?.message || e),
  );
  setInterval(() => {
    seedTechEvents().catch((e) =>
      console.warn("[TechEvents] Seed error:", e?.message || e),
...
function wbComputeRankings(indicatorData) {
  ...
  for (const cc of allCountries) {
    const components = {
      internet: wbNormalize(
        indicatorData.internet[cc]?.value,
        WB_NORMALIZE_MAX.internet,
      ),
      mobile: wbNormalize(
        indicatorData.mobile[cc]?.value,
        WB_NORMALIZE_MAX.mobile,
      ),
      broadband: wbNormalize(
        indicatorData.broadband[cc]?.value,
        WB_NORMALIZE_MAX.broadband,
      ),
      rdSpend: wbNormalize(
        indicatorData.rdSpend[cc]?.value,
        WB_NORMALIZE_MAX.rdSpend,
      ),
    };
    let totalWeight = 0,
      weightedSum = 0;
    ...
    const name =
      indicatorData.internet[cc]?.name || indicatorData.mobile[cc]?.name || cc;
    scores.push({
      country: cc,
      countryName: name,
...
async function seedWorldBank() {
  try {
    ...
    for (const { key, id, dateRange } of WB_INDICATORS) {
      ...
      console.log(
        `[WB]   ${id}: ${Object.keys(indicatorData[key]).length} countries`,
      );
    }
    ...
    console.log(
      `[WB] Progress: ${progressWithData.length}/${progressData.length} with data`,
    );
    ...
    console.log(
      `[WB] techReadiness: ${rankings.length} rankings (redis: ${ok ? "OK" : "FAIL"})`,
    );
    await upstashSet(
    ...
    if (renewableData.historicalData.length > 0) {
      ...
      await upstashSet(
        ...
        {
          fetchedAt: Date.now(),
          recordCount: renewableData.historicalData.length,
        },
        metaTtl,
      );
    }
...
async function startWorldBankSeedLoop() {
  ...
  console.log(
    `[WB] Seed loop starting (interval ${WB_SEED_INTERVAL_MS / 1000 / 60 / 60}h)`,
  );
  seedWorldBank().catch((e) =>
    console.warn("[WB] Initial seed error:", e?.message || e),
  );
  setInterval(() => {
    seedWorldBank().catch((e) =>
      console.warn("[WB] Seed error:", e?.message || e),
...
const METRICS_WINDOW_SECONDS = Math.max(
  10,
  Number(process.env.RELAY_METRICS_WINDOW_SECONDS || 60),
);
...
function sampleRelayQueueSize(queueSize) {
  ...
  if (queueSize > relayMetricsQueueMaxLifetime)
    relayMetricsQueueMaxLifetime = queueSize;
}
...
function getRelayRollingMetrics() {
  ...
  const dedupCount =
    rollup.openskyDedup + rollup.openskyDedupNeg + rollup.openskyDedupEmpty;
  const cacheServedCount =
    rollup.openskyCacheHit + rollup.openskyNegativeHit + dedupCount;
  ...
  return {
    ...
    opensky: {
      ...
      global429CooldownRemainingMs: Math.max(
        0,
        openskyGlobal429Until - Date.now(),
      ),
...
function cleanupAggregates() {
  ...
  if (vessels.size > MAX_VESSELS) {
    const sorted = [...vessels.entries()].sort(
...
function detectDisruptions() {
  ...
  for (const history of vesselHistory.values()) {
    if (history.length >= 2) {
      ...
      if (
        lastSeen - secondLast > GAP_THRESHOLD &&
...
function calculateDensityZones() {
  const zones = [];
  const allCells = Array.from(densityGrid.values()).filter(
    (c) => c.vessels.size >= 2,
  );
  ...
  for (const [key, cell] of densityGrid) {
    ...
    const deltaPct =
      cell.previousCount > 0
        ? Math.round(
            ((cell.vessels.size - cell.previousCount) / cell.previousCount) *
              100,
          )
        : 0;
...
function buildSnapshot() {
  ...
  const withCandPayload = {
    ...lastSnapshot,
    candidateReports: getCandidateReportsSnapshot(),
  };
    ...
    const req = https.get(
      ...
      (res) => {
        if (res.statusCode !== 200) {
          res.resume();
          return reject(
            new Error(`UCDP API ${res.statusCode} (v${version} p${page})`),
          );
        }
        let data = "";
...
async function ucdpFetchAllEvents() {
  ...
  for (
    let offset = 0;
    offset < UCDP_RELAY_MAX_PAGES && newestPage - offset >= 0;
    offset++
  ) {
    const page = newestPage - offset;
    const rawData =
      page === 0 ? page0 : await ucdpRelayFetchPage(version, page);
    ...
    console.log(
      `[UCDP] Fetched v${version} page ${page} (${events.length} events)`,
    );
  }
  ...
  const sanitized = allEvents
    .filter((e) => {
      ...
      return (
        Number.isFinite(ms) && ms >= latestDatasetMs - UCDP_TRAILING_WINDOW_MS
      );
    })
    .map((e) => ({
      id: String(e.id || ""),
      ...
      type_of_violence:
        UCDP_RELAY_VIOLENCE_TYPE_MAP[e.type_of_violence] || "state-based",
...
async function handleUcdpEventsRequest(req, res) {
  ...
  if (ucdpCache.data && !ucdpFetchInProgress) {
    ...
    ucdpFetchAllEvents()
      .then((result) => {
        ...
        console.log(
          `[UCDP] Background refresh: ${result.count} events (v${result.version})`,
        );
      })
      .catch((err) =>
        console.error("[UCDP] Background refresh error:", err.message),
      )
      .finally(() => {
  ...
  try {
    ...
    ucdpFetchInProgress = false;
    console.log(
      `[UCDP] Cold fetch complete: ${result.count} events (v${result.version})`,
    );
...
const OPENSKY_CACHE_TTL_MS =
  Number(process.env.OPENSKY_CACHE_TTL_MS) || 60 * 1000; // 60s default — env-configurable
...
const OPENSKY_BBOX_QUANT_STEP = Number.isFinite(
  Number(process.env.OPENSKY_BBOX_QUANT_STEP),
)
...
const OPENSKY_BBOX_DECIMALS =
  OPENSKY_BBOX_QUANT_STEP > 0
    ? Math.min(
        6,
        (String(OPENSKY_BBOX_QUANT_STEP).split(".")[1] || "").length || 0,
      )
    : 6;
const OPENSKY_DEDUP_EMPTY_RESPONSE_JSON = JSON.stringify({
  states: [],
  time: 0,
});
const OPENSKY_DEDUP_EMPTY_RESPONSE_GZIP = gzipSyncBuffer(
...
function rssRecordFailure(feedUrl) {
  ...
  const ttl = Math.min(
    RSS_NEGATIVE_CACHE_TTL_MS * Math.pow(2, prev),
...
function normalizeOpenSkyBbox(params) {
  ...
  const normalized = {};
  for (const key of keys)
    normalized[key] = formatCoordinate(quantizeCoordinate(values[key]));
  return {
...
const OPENSKY_429_COOLDOWN_MS =
  Number(process.env.OPENSKY_429_COOLDOWN_MS) || 90 * 1000; // 90s cooldown after any 429
const OPENSKY_REQUEST_SPACING_MS =
  Number(process.env.OPENSKY_REQUEST_SPACING_MS) || 2000; // 2s minimum between consecutive upstream requests
  ...
  try {
    for (let attempt = 0; attempt < OPENSKY_AUTH_MAX_RETRIES; attempt++) {
      ...
      if (result.token) {
        ...
        console.log(
          "[Relay] OpenSky token acquired, expires in",
          result.expiresIn,
          "seconds",
        );
        return openskyToken;
  ...
  if (OPENSKY_PROXY_ENABLED) {
    return _openskyProxyConnect(parsed.hostname, 443, 15000)
      ...
      .catch((err) => ({
        status: 0,
        data: null,
        error: new Error(`PROXY: ${err.message}`),
      }));
  }
...
function openskyQueuedFetch(url, token) {
  const job = openskyUpstreamQueue.then(async () => {
    ...
    const wait =
      OPENSKY_REQUEST_SPACING_MS - (Date.now() - openskyLastUpstreamTime);
...
async function handleOpenSkyRequest(req, res, PORT) {
  ...
  try {
    ...
    if (
      negCached &&
      Date.now() - negCached.timestamp < OPENSKY_NEGATIVE_CACHE_TTL_MS
    ) {
    ...
    if (existing) {
      ...
      if (
        dedupNeg &&
        Date.now() - dedupNeg.timestamp < OPENSKY_NEGATIVE_CACHE_TTL_MS
      ) {
    ...
    logThrottled(
      "log",
      `opensky-miss:${cacheKey}`,
      "[Relay] OpenSky request (MISS):",
      openskyUrl,
    );
    ...
    return sendCompressed(
      ...
      upstreamStatus,
      {
        "Content-Type": "application/json",
        "Cache-Control":
          upstreamStatus === 200 ? "public, max-age=30" : "no-cache",
        "CDN-Cache-Control":
          upstreamStatus === 200 ? "public, max-age=15" : "no-store",
      ...
      response.on("end", () => {
        try {
          ...
          for (const record of records || []) {
            ...
            if (
              !transformed.latestByCountry[cc] ||
              yr > transformed.latestByCountry[cc].year
            ) {
              transformed.latestByCountry[cc] = {
                code: cc,
                name: cn,
          ...
          transformed.timeSeries.sort(
            (a, b) =>
              b.year - a.year || a.countryCode.localeCompare(b.countryCode),
          );
    ...
    .then((slotResult) => {
      if (slotResult === "REJECTED") {
        polymarketCache.set(cacheKey, {
          data: "[]",
          timestamp:
            Date.now() - POLYMARKET_CACHE_TTL_MS + POLYMARKET_NEG_TTL_MS,
        });
        return null;
      }
...
function handlePolymarketRequest(req, res) {
  if (!POLYMARKET_ENABLED) {
    return sendCompressed(
      ...
      JSON.stringify({
        error: "polymarket disabled",
        reason: "POLYMARKET_ENABLED=false",
      }),
    );
  }
  const url = new URL(req.url, `http://localhost:${PORT}`);
...
setInterval(() => {
  ...
  for (const [key, entry] of openskyResponseCache) {
    if (now - entry.timestamp > OPENSKY_CACHE_TTL_MS * 2)
      openskyResponseCache.delete(key);
  }
  for (const [key, entry] of openskyNegativeCache) {
    if (now - entry.timestamp > OPENSKY_NEGATIVE_CACHE_TTL_MS * 2)
      openskyNegativeCache.delete(key);
  }
  for (const [key, entry] of rssResponseCache) {
    if (now - entry.timestamp > RSS_CACHE_TTL_MS * 2)
  ...
  for (const key of rssFailureCount.keys()) {
    if (!rssBackoffUntil.has(key) && !rssResponseCache.has(key))
      rssFailureCount.delete(key);
  }
  for (const [key, entry] of worldbankCache) {
    if (now - entry.timestamp > WORLDBANK_CACHE_TTL_MS * 2)
      worldbankCache.delete(key);
  }
  for (const [key, entry] of polymarketCache) {
    if (now - entry.timestamp > POLYMARKET_CACHE_TTL_MS * 2)
      polymarketCache.delete(key);
  }
  for (const [key, entry] of yahooChartCache) {
    if (now - entry.ts > YAHOO_CHART_CACHE_TTL_MS * 2)
      yahooChartCache.delete(key);
  }
  for (const [key, bucket] of requestRateBuckets) {
    if (now >= bucket.resetAt + RELAY_RATE_LIMIT_WINDOW_MS * 2)
      ...
      upstream.on("end", () => {
        if (upstream.statusCode !== 200) {
          ...
          return sendCompressed(
            ...
            JSON.stringify({
              error: `Yahoo upstream ${upstream.statusCode}`,
              symbol,
            }),
          );
        }
...
function ytFetchViaProxy(targetUrl, proxy) {
  return new Promise((resolve, reject) => {
    ...
    if (proxy.auth) {
      connectOpts.headers["Proxy-Authorization"] =
...
function handleYouTubeLiveRequest(req, res) {
  ...
  ytFetch(liveUrl)
    .then((r) => {
      ...
      const html = r.body;
      const channelExists =
        html.includes('"channelId"') || html.includes("og:url");
...
function getCorsOrigin(req) {
  ...
  if (ALLOW_VERCEL_PREVIEW_ORIGINS && origin.endsWith(".vercel.app"))
    return origin;
  return "";
}
...
const server = http.createServer(async (req, res) => {
  ...
  const isPublicRoute =
    pathname === "/health" || pathname === "/" || isRssRoute;
  if (!isPublicRoute) {
  ...
  if (pathname === "/health" || pathname === "/") {
    ...
    sendCompressed(
      ...
      JSON.stringify({
        ...
        oref: {
          ...
          lastPollAt: orefState.lastPollAt
    ...
    if (json) {
      ...
      const payload = {
        ...lastSnapshot,
        candidateReports: includeCandidates
    ...
    console.log(
      "[Relay] OpenSky auth + rate-limit state reset via /opensky-reset",
    );
    ...
    diag.steps.push({
      ...
      tokenExpiry: openskyTokenExpiry
        ? new Date(openskyTokenExpiry).toISOString()
        : null,
          ...
          (apiRes) => {
            ...
            apiRes.on("end", () =>
              resolve({
                ...
                statesCount: data.match(/"states":\s*\[/)
                  ? "present"
                  : "missing",
              }),
            );
    ...
    try {
      ...
      const items = Array.isArray(telegramState.items)
        ? telegramState.items
        : [];
      const filtered = items
        .filter((it) => {
          if (topic && String(it.topic || "").toLowerCase() !== topic)
            return false;
          if (channel && String(it.channel || "").toLowerCase() !== channel)
            return false;
          return true;
        })
        .slice(0, limit);
    ...
    try {
      ...
      if (!RSS_ALLOWED_DOMAINS.has(parsed.hostname)) {
        ...
        return res.end(
          JSON.stringify({ error: "Domain not allowed on Railway proxy" }),
        );
      }
      ...
      if (backoffExpiry && backoffNow < backoffExpiry) {
        ...
        if (
          ...
          return sendCompressed(
            ...
            {
              "Content-Type":
                rssCachedForBackoff.contentType || "application/xml",
              "Cache-Control": "no-store",
              "CDN-Cache-Control": "no-store",
        ...
        const remainSec = Math.max(
          1,
          Math.round((backoffExpiry - backoffNow) / 1000),
        );
        res.writeHead(503, {
          "Content-Type": "application/json",
      ...
      if (existing) {
        try {
          ...
          return safeEnd(
            ...
            { "Content-Type": "application/json" },
            JSON.stringify({
              error: "Upstream fetch completed but not cached",
            }),
          );
        } catch {
      ...
      const fetchPromise = new Promise((resolveInFlight, rejectInFlight) => {
        ...
        const fetchWithRedirects = (url, redirectCount = 0) => {
          ...
          if (rssCached?.etag)
            conditionalHeaders["If-None-Match"] = rssCached.etag;
...
function connectUpstream() {
  ...
  const drainUpstreamQueue = () => {
    ...
    if (queueSize >= UPSTREAM_QUEUE_HIGH_WATER && !upstreamPaused) {
      ...
      socket.pause();
      console.warn(
        `[Relay] Upstream paused (queue=${queueSize}, dropped=${droppedMessages})`,
      );
  ...
  socket.on("message", (data) => {
    ...
    if (
      !upstreamPaused &&
      getUpstreamQueueSize() >= UPSTREAM_QUEUE_HIGH_WATER
    ) {
      upstreamPaused = true;
      socket.pause();
      console.warn(
...
wss.on("connection", (ws, req) => {
  ...
  console.log(
    `[Relay] Client connected (${clients.size + 1}/${MAX_WS_CLIENTS})`,
  );
  clients.add(ws);
  connectUpstream();
