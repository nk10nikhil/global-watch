...
const CALLSIGN_PATTERNS = [
  ...
  {
    re: /^CHINA\s?(AIR\s?FORCE|MIL|NAVY)/i,
    operator: "plaaf",
    aircraftType: null,
  },
];
  ...
  return new Promise((resolve, reject) => {
    ...
    const connectReq = http.request({
      ...
      headers: {
        ...
        "Proxy-Authorization":
          "Basic " + Buffer.from(proxy.userPass).toString("base64"),
      },
      timeout,
    });
            ...
            (resp) => {
              ...
              resp.on("end", () => {
                ...
                if (resp.statusCode >= 400) {
                  return reject(
                    new Error(
                      `HTTP ${resp.statusCode}: ${data.substring(0, 200)}`,
                    ),
                  );
                }
                try {
    ...
    connectReq.on("error", (e) => {
      clearTimeout(timer);
      reject(new Error(redactProxy(e.message)));
    });
...
async function fetchOpenSkyAuthenticated(region) {
  ...
  if (!resp.ok) {
    ...
    throw new Error(
      `OpenSky auth HTTP ${resp.status}: ${body.substring(0, 200)}`,
    );
  }
...
async function fetchOpenSkyAnonymous(region) {
  ...
  if (!resp.ok) {
    const body = await resp.text().catch(() => "");
    throw new Error(
      `OpenSky anon HTTP ${resp.status}: ${body.substring(0, 200)}`,
    );
  }
  const data = await resp.json();
  return data.states || [];
...
async function fetchWingbits() {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (flightList.length > 0) {
      console.log(
        `  [Wingbits]     sample[0]: ${JSON.stringify(flightList[0]).substring(0, 200)}`,
      );
    }
  }
...
async function fetchAllStates() {
  ...
  for (const region of QUERY_REGIONS) {
    ...
    try {
      ...
      console.warn(
        `  [OpenSky Auth] ${region.name}: ${redactProxy(e.message)}`,
      );
    }
    ...
    if (!states || states.length === 0) {
      try {
        ...
        if (states && states.length > 0) {
          ...
          console.log(
            `  [OpenSky Anon] ${region.name}: ${states.length} states`,
          );
        }
      } catch (e) {
        console.warn(
          `  [OpenSky Anon] ${region.name}: ${redactProxy(e.message)}`,
        );
      }
    }
    ...
    if (states) {
      ...
      if (added > 0)
        console.log(
          `  [OpenSky] +${added} new from ${region.name} (total: ${allStates.length})`,
        );
    }
  }
...
function filterMilitaryFlights(allStates) {
  ...
  for (const state of allStates) {
    ...
    const csMatch = callsign
      ? identifyByCallsign(callsign, originCountry)
      : null;
    ...
    flights.push({
      ...
      verticalRate:
        vertRate != null ? Math.round(vertRate * 196.85) : undefined,
      onGround: state[8],
...
async function main() {
  ...
  console.log(
    `=== military:flights Seed (proxy: ${PROXY_ENABLED ? "enabled" : "direct"}) ===`,
  );
  ...
  try {
    ...
    await redisSet(
      ...
      {
        fetchedAt: Date.now(),
        recordCount: theaterFlights.length,
    ...
    console.log(
      `  Theater posture: ${theaters.length} theaters (${elevated} elevated), redis: ${tp1 && tp2 && tp3 ? "OK" : "PARTIAL"}`,
    );
