...
async function main() {
  ...
  if (tauriChanged) {
    mismatches.push(
      `src-tauri/tauri.conf.json (${tauriCurrentVersion} -> ${targetVersion})`,
    );
  }
  if (cargoUpdate.changed) {
    mismatches.push(
      `src-tauri/Cargo.toml (${cargoUpdate.currentVersion} -> ${targetVersion})`,
    );
  }
  ...
  if (!tauriChanged && !cargoUpdate.changed) {
    console.log(
      `[version:sync] No changes needed. All files already at ${targetVersion}.`,
    );
    return;
  }
...
main().catch((error) => {
  console.error(
    `[version:sync] Failed: ${error instanceof Error ? error.message : String(error)}`,
  );
  process.exit(1);
});
