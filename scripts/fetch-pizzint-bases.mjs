...
// ---------------------------------------------------------------------------
const SUPABASE_URL =
  process.env.SUPABASE_URL || "https://qevdnlpgjxpwusesmtpx.supabase.co";
const SUPABASE_ANON_KEY = process.env.SUPABASE_ANON_KEY;

if (!SUPABASE_ANON_KEY) {
  console.error("ERROR: SUPABASE_ANON_KEY is required.");
  console.error(
    "Set it in .env.local at the project root or export it as an env var.",
  );
  process.exit(1);
}
...
async function fetchPage(pageIndex) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      console.warn(
        `  Page ${pageIndex} attempt ${attempt}/${MAX_RETRIES} failed: ${err.message}`,
      );
...
async function main() {
  ...
  for (let page = 0; page < TOTAL_PAGES; page++) {
    ...
    try {
      ...
      if (!data || data.length === 0) {
        emptyPages++;
        console.log(
          `  Page ${page}: empty (${emptyPages} consecutive empty pages)`,
        );
        if (emptyPages >= 3) {
      ...
      console.error(
        `  FATAL: Page ${page} failed after ${MAX_RETRIES} retries: ${err.message}`,
      );
      saveCheckpoint({ pages: fetchedPages, rows: allRows });
  ...
  for (const row of allRows) {
    if (
      ...
      if (nullCoordCount <= 20) {
        console.warn(
          `  Skipping row with null coords: osm_id=${row.osm_id}, name=${row.name}`,
        );
      }
      continue;
    }
  ...
  if (nullCoordCount > 20) {
    console.warn(
      `  ... and ${nullCoordCount - 20} more rows with null lat/lon`,
    );
  }
  ...
  const sizeMB = (
    Buffer.byteLength(JSON.stringify(validRows)) /
    1024 /
    1024
  ).toFixed(1);
  console.log("");
  console.log(
    `Output: ${OUTPUT_PATH} (${validRows.length} entries, ${sizeMB} MB)`,
  );
  console.log("Done.");
}
