...
function processFeature(feature) {
  ...
  return {
    ...
    lon,
    kind:
      p.FEATUREDESCRIPTION && p.FEATUREDESCRIPTION !== "na"
...
async function main() {
  ...
  console.log(
    `  Total boundary features: ${boundaryGeoJson.features.length}\n`,
  );
  ...
  const rawSizeMB = (
    Buffer.byteLength(JSON.stringify(combinedRaw)) /
    1024 /
    1024
  ).toFixed(2);
  ...
  const pointNames = new Set(
    pointsGeoJson.features.map((f) =>
      (f.properties?.SITENAME || "").toLowerCase().trim(),
    ),
  );
  ...
  for (const inst of processed) {
    ...
    componentCounts[inst.component] =
      (componentCounts[inst.component] || 0) + 1;
  }

  console.log("\nBy branch:");
  for (const [k, v] of Object.entries(branchCounts).sort(
    (a, b) => b[1] - a[1],
  )) {
    console.log(`  ${k}: ${v}`);
  }
  ...
  console.log("\nBy component:");
  for (const [k, v] of Object.entries(componentCounts).sort(
    (a, b) => b[1] - a[1],
  )) {
    console.log(`  ${k}: ${v}`);
  }
  ...
  for (const s of samples) {
    console.log(
      `  ${s.name} | ${s.branch} | ${s.status} | ${s.state} | (${s.lat}, ${s.lon})`,
    );
  }
