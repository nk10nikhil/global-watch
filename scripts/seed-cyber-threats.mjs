...
function sanitize(t) {
  ...
  return {
    id:
      clean(t.id, 255) ||
      `${t.source || "feodo"}:${t.indicatorType || "ip"}:${indicator}`,
  ...
  try {
    ...
    if (resp.ok) {
      ...
      if (validCoords(lat, lon))
        return { lat, lon, country: normCountry(d.country) };
    }
  } catch {
    /* fall through */
  }
  ...
  return threats.map((t) => {
    ...
    if (lookup)
      return {
        ...
        lat: lookup.lat,
        lon: lookup.lon,
        country: t.country || lookup.country,
      };
...
async function fetchC2Intel() {
  try {
    ...
    for (const line of text.split("\n")) {
      ...
      if (dl.includes("cobaltstrike") || dl.includes("cobalt strike"))
        tags.push("cobaltstrike");
      if (dl.includes("metasploit")) tags.push("metasploit");
      if (dl.includes("sliver")) tags.push("sliver");
      if (dl.includes("brute ratel") || dl.includes("bruteratel"))
        tags.push("bruteratel");
      const sev = /cobaltstrike|cobalt.strike|brute.?ratel/i.test(desc)
        ? "high"
        : "medium";
      const t = sanitize({
        id: `c2intel:${ip}`,
...
async function fetchOtx(days) {
  ...
  try {
    ...
    for (const r of results) {
      ...
      if (!isIp(ip)) continue;
      const tags = normTags(r?.tags || []);
      const sev = tags.some((t) => /ransomware|apt|c2|botnet/.test(t))
        ? "high"
        : "medium";
      const type = tags.some((t) => /c2|botnet/.test(t))
        ? "c2_server"
        : "malware_host";
  ...
  for (const t of threats) {
    ...
    if (cSeen >= eSeen) {
      map.set(key, {
        ...existing,
        ...t,
        tags: normTags([...existing.tags, ...t.tags]),
      });
    }
  }
...
function toProto(raw) {
  return {
    ...
    indicatorType:
      INDICATOR_TYPE_MAP[raw.indicatorType] ||
      "CYBER_THREAT_INDICATOR_TYPE_UNSPECIFIED",
    location: validCoords(raw.lat, raw.lon)
