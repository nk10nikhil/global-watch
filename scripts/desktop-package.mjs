...
const syncVersionsResult = spawnSync(
  process.execPath,
...
const bundles =
  os === "macos" ? "app,dmg" : os === "linux" ? "appimage" : "nsis,msi";
const env = {
  ...process.env,
...
if (sign) {
  ...
  if (os === "windows") {
    const hasThumbprint = Boolean(
      env.TAURI_BUNDLE_WINDOWS_CERTIFICATE_THUMBPRINT,
    );
    const hasPfx = Boolean(
      env.TAURI_BUNDLE_WINDOWS_CERTIFICATE &&
      env.TAURI_BUNDLE_WINDOWS_CERTIFICATE_PASSWORD,
    );
...
if (!skipNodeRuntime) {
  ...
  const downloadResult = spawnSync(
    "bash",
    ["scripts/download-node.sh", "--target", nodeTarget],
    {
      env: {
        ...env,
