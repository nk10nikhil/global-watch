 ...
 *
 * Requires: Referer header from trusted origin OR X-GlobalWatch-Key header.
 * Uses myglobalwatch.vercel.app by default.
 */
...
const ORIGIN = "https://myglobalwatch.vercel.app";
...
const API_KEY = process.env.GLOBALWATCH_KEY || "";
...
async function fetchEndpoint(endpoint) {
  ...
  const resp = await fetch(url, {
    headers: {
      ...
      ...(API_KEY ? { "X-GlobalWatch-Key": API_KEY } : {}),
    },
...
async function runTest(test) {
  ...
  try {
    ...
    const belowMin =
      test.minRecords != null && count != null && count < test.minRecords;
    ...
    return {
      ...
      warn: belowMin,
      reason: belowMin
        ? `Only ${count} records (expected >= ${test.minRecords})`
        : null,
      count,
      elapsed,
    };
...
async function main() {
  ...
  if (health.pass && health.seeds) {
    ...
    for (const [domain, info] of Object.entries(health.seeds)) {
      const dIcon =
        info.status === "ok" ? PASS : info.status === "stale" ? WARN : FAIL;
      const age = info.ageMinutes != null ? ` (${info.ageMinutes}m ago)` : "";
      const count =
        info.recordCount != null ? `, ${info.recordCount} records` : "";
  ...
  if (health.seeds) {
    ...
    for (const [domain, testName] of Object.entries(seedDomainToTest)) {
      ...
      if (seedInfo.status === "ok" && rpcResult.pass) {
        ...
        console.log(
          `  ${FAIL} ${domain}: seed fresh but RPC failed (${rpcResult.reason})`,
        );
      } else if (seedInfo.status !== "ok" && rpcResult.pass) {
        console.log(
        ...
        console.log(
          `  ${FAIL} ${domain}: seed ${seedInfo.status} AND RPC failed`,
        );
      }
    }
  }
