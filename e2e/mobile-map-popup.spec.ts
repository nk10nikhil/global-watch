...
for (const deviceConfig of MOBILE_DEVICE_MATRIX) {
  test.describe(`Mobile SVG popup QA (${deviceConfig.label})`, () => {
    const { defaultBrowserType: _defaultBrowserType, ...contextOptions } =
      deviceConfig.use;
    test.use(contextOptions);

    test("keeps popup in viewport and supports dismissal patterns", async ({
      page,
    }) => {
      ...
      expect(
        (popupRect?.right ?? 0) - (popupRect?.viewportWidth ?? 0),
      ).toBeLessThanOrEqual(1);
      expect(
        (popupRect?.bottom ?? 0) - (popupRect?.viewportHeight ?? 0),
      ).toBeLessThanOrEqual(1);
...
test.describe("Mobile SVG popup integration path", () => {
  ...
  test("opens popup through MapComponent hotspot marker tap", async ({
    page,
  }) => {
    const pageErrors: string[] = [];
    page.on("pageerror", (error) => pageErrors.push(error.message));
