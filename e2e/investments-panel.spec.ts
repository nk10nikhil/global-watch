...
test.describe("GCC investments coverage", () => {
  test("focusInvestmentOnMap enables layer and recenters map", async ({
    page,
  }) => {
    await page.goto("/tests/runtime-harness.html");

    const result = await page.evaluate(async () => {
      const { focusInvestmentOnMap } =
        await import("/src/services/investments-focus.ts");
      ...
      focusInvestmentOnMap(
        ...
        mapLayers as unknown as { gulfInvestments: boolean } & Record<
          string,
          boolean
        >,
        24.4667,
  ...
  test("InvestmentsPanel supports search/filter/sort and row click callbacks", async ({
    page,
  }) => {
    await page.goto("/tests/runtime-harness.html");
    ...
    const result = await page.evaluate(async () => {
      ...
      const { InvestmentsPanel } =
        await import("/src/components/InvestmentsPanel.ts");
      ...
      const searchToken =
        firstInvestment?.assetName.split(/\s+/)[0]?.toLowerCase() ?? "";
      ...
      await pollUntil(
        () => root.querySelectorAll(".fdi-row").length !== totalRows,
      );
      ...
      const expectedSaRows = GULF_INVESTMENTS.filter(
        (inv) => inv.investingCountry === "SA",
      ).length;

      // --- Sort by investment desc ---
      // Re-query sort header after country filter re-render
      let investmentSort = root.querySelector<HTMLElement>(
        '.fdi-sort[data-sort="investmentUSD"]',
      );
      const rowsBefore1 =
        root.querySelector<HTMLElement>(".fdi-row")?.dataset.id;
      ...
      investmentSort = root.querySelector<HTMLElement>(
        '.fdi-sort[data-sort="investmentUSD"]',
      );
      const rowsBefore2 =
        root.querySelector<HTMLElement>(".fdi-row")?.dataset.id;
      ...
      const expectedTopSaId =
        GULF_INVESTMENTS.filter((inv) => inv.investingCountry === "SA")
          .slice()
          .sort((a, b) => (b.investmentUSD ?? -1) - (a.investmentUSD ?? -1))[0]
          ?.id ?? null;
