...
test.describe("keyword spike modal/badge flow", () => {
  test("injects synthetic headlines and renders keyword_spike end-to-end", async ({
    page,
  }) => {
    ...
    const setup = await page.evaluate(async () => {
      ...
      const { IntelligenceGapBadge } =
        await import("/src/components/IntelligenceGapBadge.ts");
      ...
      if (spikes.length === 0) {
        ...
        return {
          ok: false,
          reason: "No keyword spikes emitted from synthetic data",
        };
      }
      ...
      return {
        ...
        badgeCount:
          (document.querySelector(".findings-count") as HTMLElement | null)
            ?.textContent ?? "0",
      };
    });
  ...
  test("does not emit spikes from source-attribution suffixes", async ({
    page,
  }) => {
    await page.goto("/tests/runtime-harness.html");
    ...
    const result = await page.evaluate(async () => {
      ...
      try {
        ...
        const headlines = [
          {
            source: "Reuters",
            title: "Qzxalpha ventures stabilize - WireDesk",
          },
        ...
        return {
          ...
          hasWireDeskSpike: spikes.some((signal) =>
            /wiredesk/i.test(signal.title),
          ),
        };
      } finally {
        trending.updateTrendingConfig(previousConfig);
  ...
  test("suppresses month-name token spikes", async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      try {
        ...
        return {
          emittedTitles: spikes.map((signal) => signal.title),
          hasJanuarySpike: spikes.some((signal) =>
            /january/i.test(signal.title),
          ),
        };
      } finally {
        trending.updateTrendingConfig(previousConfig);
