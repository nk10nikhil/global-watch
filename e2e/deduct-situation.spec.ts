...
test.describe("Deduct Situation Panel Options", () => {
  test("It successfully requests deduction from the intelligence API", async ({
    page,
  }) => {
    ...
    await expect(
      panel.locator('text="Analyzing timeline and impact..."'),
    ).toBeVisible();

    // Verify the resolved output is rendered
    if (!process.env.TEST_REAL_LLM) {
      await expect(panel.locator('text="Mocked AI Analysis"')).toBeVisible({
        timeout: 10000,
      });
      ...
      await expect(panel.locator(".deduction-result")).not.toBeEmpty({
        timeout: 30000,
      });
    }
  });
