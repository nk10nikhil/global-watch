...
test.describe("desktop runtime routing guardrails", () => {
  ...
  test("runtime fetch patch falls back to cloud for local failures", async ({
    page,
  }) => {
    await page.goto("/tests/runtime-harness.html");
    ...
    const result = await page.evaluate(async () => {
      ...
      try {
        ...
        const fredResponse = await window.fetch(
          "/api/fred-data?series_id=CPIAUCSL",
        );
        const fredBody = (await fredResponse.json()) as {
    ...
    expect(
      result.calls.some((url) => url.includes("127.0.0.1:46123/api/fred-data")),
    ).toBe(true);
    expect(
    ...
    expect(
      result.calls.some((url) =>
        url.includes("127.0.0.1:46123/api/stablecoin-markets"),
      ),
    ).toBe(true);
    expect(
      result.calls.some((url) =>
  ...
  test("runtime fetch patch never sends local-only endpoints to cloud", async ({
    page,
  }) => {
    await page.goto("/tests/runtime-harness.html");
    ...
    expect(
      result.calls.some((url) =>
        url.includes("127.0.0.1:46123/api/local-env-update"),
      ),
    ).toBe(true);
    expect(
      result.calls.some((url) =>
        url.includes("127.0.0.1:46123/api/local-validate-secret"),
      ),
    ).toBe(true);
    expect(
      result.calls.some((url) =>
  ...
  test("chunk preload reload guard is one-shot until app boot clears it", async ({
    page,
  }) => {
    await page.goto("/tests/runtime-harness.html");
  ...
  test("update badge picks architecture-correct desktop download url", async ({
    page,
  }) => {
    await page.goto("/tests/runtime-harness.html");
    ...
    const result = await page.evaluate(async () => {
      ...
      try {
        ...
        const macArm = await updaterProto.resolveUpdateDownloadUrl.call(
          fakeApp,
          releaseUrl,
        );
        ...
        const windowsX64 = await updaterProto.resolveUpdateDownloadUrl.call(
          fakeApp,
          releaseUrl,
        );
        ...
        const linuxFallback = await updaterProto.resolveUpdateDownloadUrl.call(
          fakeApp,
          releaseUrl,
        );
  ...
  test("MapContainer falls back to SVG when WebGL2 is unavailable", async ({
    page,
  }) => {
    await page.goto("/tests/runtime-harness.html");
    ...
    const result = await page.evaluate(async () => {
      ...
      try {
        ...
        return {
          ...
          deckWrapperCount: mapHost.querySelectorAll(".deckgl-map-wrapper")
            .length,
  ...
  test("loadMarkets keeps Yahoo-backed data when Finnhub is skipped", async ({
    page,
  }) => {
    await page.goto("/tests/runtime-harness.html");
    ...
    const result = await page.evaluate(async () => {
      ...
      const fakeApp = {
        ctx: {
          ...
          panels: {
            markets: {
              renderMarkets: (data: Array<unknown>) =>
                marketRenders.push(data.length),
              showConfigError: (message: string) =>
                marketConfigErrors.push(message),
            },
            heatmap: {
              renderHeatmap: (data: Array<unknown>) =>
                heatmapRenders.push(data.length),
              showConfigError: (message: string) =>
                heatmapConfigErrors.push(message),
            },
            commodities: {
              renderCommodities: (data: Array<unknown>) =>
                commoditiesRenders.push(data.length),
              showConfigError: (message: string) =>
                commoditiesConfigErrors.push(message),
              showRetrying: () => {},
            },
            crypto: {
              renderCrypto: (data: Array<unknown>) =>
                cryptoRenders.push(data.length),
              showRetrying: () => {},
            },
          },
          statusPanel: {
      ...
      try {
        await (
          DataLoaderManager.prototype as unknown as {
    ...
    expect(
      result.apiStatuses.some(
        (entry) => entry.name === "Finnhub" && entry.status === "error",
      ),
    ).toBe(true);
    expect(
      result.apiStatuses.some(
        (entry) => entry.name === "CoinGecko" && entry.status === "ok",
      ),
    ).toBe(true);
  });

  test("fetchHapiSummary maps proto countryCode to iso2 field", async ({
    page,
  }) => {
    await page.goto("/tests/runtime-harness.html");
    ...
    const result = await page.evaluate(async () => {
      ...
      try {
        ...
        return {
          ...
          usIso2: us?.iso2 ?? null,
          hasIso3Field:
            !!us && Object.prototype.hasOwnProperty.call(us, "iso3"),
        };
      } finally {
        window.fetch = originalFetch;
      }
