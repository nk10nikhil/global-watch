...
description: "All JSON API endpoints in Global Watch must use sebuf. This guide walks through adding a new RPC to an existing service and adding an entirely new service."
---

All JSON API endpoints in Global Watch **must** use sebuf. Do not create standalone `api/*.js` files — the legacy pattern is deprecated and being removed.

This guide walks through adding a new RPC to an existing service and adding an entirely new service.

> **Important:** After modifying any `.proto` file, you **must** run `make generate` before building or pushing. The generated TypeScript files in `src/generated/` are checked into the repo and must stay in sync with the proto definitions. CI does not run generation yet — this is your responsibility until we add it to the pipeline (see [#200](https://github.com/nk10nikhil/globalwatch/issues/200)).
...
Create `proto/globalwatch/seismology/v1/get_earthquake_details.proto`:

```protobuf
syntax = "proto3";
package GLOBALWATCH.seismology.v1;

import "buf/validate/validate.proto";
import "globalwatch/seismology/v1/earthquake.proto";
...
Edit `proto/globalwatch/seismology/v1/service.proto`:

```protobuf
import "globalwatch/seismology/v1/get_earthquake_details.proto";
...
Create `server/globalwatch/seismology/v1/get-earthquake-details.ts`:
...
export const getEarthquakeDetails: SeismologyServiceHandler["getEarthquakeDetails"] =
  async (
    _ctx: ServerContext,
...
Edit `server/globalwatch/seismology/v1/handler.ts`:
...
```
proto/globalwatch/sanctions/v1/
```

### 2. Define entity messages

Create `proto/globalwatch/sanctions/v1/sanctions_entry.proto`:

```protobuf
syntax = "proto3";
package GLOBALWATCH.sanctions.v1;
...
Create `proto/globalwatch/sanctions/v1/list_sanctions.proto`:
...
import "buf/validate/validate.proto";
import "globalwatch/core/v1/pagination.proto";
import "globalwatch/sanctions/v1/sanctions_entry.proto";
...
message ListSanctionsRequest {
  ...
  GLOBALWATCH.core.v1.PaginationRequest pagination = 3;
}
...
message ListSanctionsResponse {
  ...
  // Pagination metadata.
  GLOBALWATCH.core.v1.PaginationResponse pagination = 2;
}
```

### 4. Define the service

Create `proto/globalwatch/sanctions/v1/service.proto`:
...
import "sebuf/http/annotations.proto";
import "globalwatch/sanctions/v1/list_sanctions.proto";
...
```
server/globalwatch/sanctions/v1/
├── handler.ts               # thin re-export
└── list-sanctions.ts        # RPC implementation
```

`server/globalwatch/sanctions/v1/list-sanctions.ts`:
...
} from "../../../../src/generated/server/globalwatch/sanctions/v1/service_server";
...
`server/globalwatch/sanctions/v1/handler.ts`:

```typescript
import type { SanctionsServiceHandler } from "../../../../src/generated/server/globalwatch/sanctions/v1/service_server";
...
```typescript
import { createSanctionsServiceRoutes } from "../src/generated/server/globalwatch/sanctions/v1/service_server";
import { sanctionsHandler } from "./server/globalwatch/sanctions/v1/handler";
...
} from "@/generated/client/globalwatch/sanctions/v1/service_client";
import { createCircuitBreaker } from "@/utils";

export type { SanctionsEntry };

const client = new SanctionsServiceClient("", {
  fetch: fetch.bind(globalThis),
});
const breaker = createCircuitBreaker<ListSanctionsResponse>({
  name: "Sanctions",
});
...
| Type                 | Import                                  | Use for                                                        |
| -------------------- | --------------------------------------- | -------------------------------------------------------------- |
| `GeoCoordinates`     | `globalwatch/core/v1/geo.proto`        | Any lat/lon location (has built-in -90/90 and -180/180 bounds) |
| `BoundingBox`        | `globalwatch/core/v1/geo.proto`        | Spatial filtering                                              |
| `TimeRange`          | `globalwatch/core/v1/time.proto`       | Time-based filtering (has `INT64_ENCODING_NUMBER`)             |
| `PaginationRequest`  | `globalwatch/core/v1/pagination.proto` | Request pagination (has page_size 1-100 constraint)            |
| `PaginationResponse` | `globalwatch/core/v1/pagination.proto` | Response pagination metadata                                   |
