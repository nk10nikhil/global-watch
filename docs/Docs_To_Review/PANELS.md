...
> **Global Watch** — Config-driven panel architecture powering three site variants.
>
...
4. [Full Variant Panels](#4-full-variant-panels-GLOBALWATCHio)
5. [Tech Variant Panels](#5-tech-variant-panels-techGLOBALWATCHio)
6. [Finance Variant Panels](#6-finance-variant-panels-financeGLOBALWATCHio)
...
Global Watch uses a **config-driven panel system** where every dashboard tile — from live news feeds to AI insights to market data — is declared as a `PanelConfig` entry inside a variant-specific configuration object. The system is designed around three principles:
...
| --------- | ----------------------- | ----------------------------------------- | ----------- |
| `full`    | GLOBALWATCH.io         | Geopolitical intelligence, OSINT, defense | 37          |
| `tech`    | tech.GLOBALWATCH.io    | Technology, AI/ML, startups, VC           | 34          |
| `finance` | finance.GLOBALWATCH.io | Markets, trading, macro, commodities      | 29          |
...
- **`name`** — Variant-specific. The same panel ID (e.g. `map`) can have different display names across variants: _"Global Map"_ (full), _"Global Tech Map"_ (tech), _"Global Markets Map"_ (finance).
...
export const STORAGE_KEYS = {
  panels: "GLOBALWATCH-panels", // Panel visibility toggles
  monitors: "GLOBALWATCH-monitors", // Monitor keyword configs
  mapLayers: "GLOBALWATCH-layers", // Map layer toggles
  disabledFeeds: "GLOBALWATCH-disabled-feeds", // Per-source feed disabling
} as const;
```
...
| ------------------------------- | ---------------------------------------------- | ---------- |
| `GLOBALWATCH-panel-spans`      | Panel height/span sizes (1–4)                  | `Panel.ts` |
| `panel-order`                   | Drag-and-drop panel ordering                   | `App.ts`   |
| `globalwatch-variant`          | Last-active variant (triggers reset on change) | `App.ts`   |
| `GLOBALWATCH-panel-order-v1.9` | Migration flag for v1.9 panel layout           | `App.ts`   |
...
Span values are persisted per-panel in the `GLOBALWATCH-panel-spans` localStorage key as a JSON object `{ [panelId]: spanNumber }`.
...
## 4. Full Variant Panels (GLOBALWATCH.io)
...
## 5. Tech Variant Panels (tech.GLOBALWATCH.io)
...
## 6. Finance Variant Panels (finance.GLOBALWATCH.io)
...
| ----------------- | ----------------------------------- | ---------------------------------- | ------------------------ | :-------------: |
| Panel visibility  | `GLOBALWATCH-panels`               | `Record<string, PanelConfig>` JSON | `DEFAULT_PANELS`         |       ✅        |
| Panel ordering    | `panel-order`                       | `string[]` JSON                    | Config declaration order |       ✅        |
| Panel sizes/spans | `GLOBALWATCH-panel-spans`          | `Record<string, number>` JSON      | All span-1               |       ✅        |
| Map layer toggles | `GLOBALWATCH-layers`               | `MapLayers` JSON                   | `DEFAULT_MAP_LAYERS`     |       ✅        |
| Monitor keywords  | `GLOBALWATCH-monitors`             | `Monitor[]` JSON                   | `[]`                     |       ✅        |
| Disabled sources  | `GLOBALWATCH-disabled-feeds`       | `string[]` JSON                    | `[]`                     |       ✅        |
| Active variant    | `globalwatch-variant`              | Plain string                       | `SITE_VARIANT`           |       ✅        |
| Banner dismissal  | `banner-dismissed` (sessionStorage) | Timestamp string                   | —                        |  Session only   |

### 9.2 Variant Change Reset

When the stored variant (`globalwatch-variant`) differs from the current `SITE_VARIANT`, the App constructor performs a full reset:
...
This ensures users switching between variant domains (e.g. from GLOBALWATCH.io to tech.GLOBALWATCH.io) get a clean default experience for the new variant.
...
for (const key of Object.keys(FEEDS)) {
  ...
  const panelKey =
    this.panels[key] && !this.newsPanels[key] ? `${key}-news` : key;
...
Step-by-step guide to adding a new panel to Global Watch.
...
tasks.push({
  name: "myPanel",
  task: runGuarded("myPanel", () =>
    (this.panels["my-panel"] as MyPanel).refresh(),
  ),
});
```
...
sequenceDiagram
    ...
    App->>localStorage: saveToStorage('GLOBALWATCH-panels', panelSettings)
    ...
    App->>localStorage: loadFromStorage('GLOBALWATCH-panels')
