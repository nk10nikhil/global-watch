# Documentation Roadmap — Global Watch
...
- [ ] Nginx configuration (reference `deploy/nginx-GLOBALWATCH.conf`)
- [ ] SystemD service setup (reference `deploy/GLOBALWATCH-api.service`)
...
- [ ] Add entries for UI customizations on `feat/ui-customizations-GLOBALWATCH` branch
