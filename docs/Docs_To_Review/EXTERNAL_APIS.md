...
> Comprehensive reference for every external API consumed by Global Watch.
...
Global Watch integrates **38 distinct external API sources** (plus ~150 RSS feed
...
| **Quirks**            | Requires both key _and_ email as separate params. Data lags 1–2 weeks behind real-time events. Pagination via `page` param. |
...
How Global Watch behaves when each external API is unavailable:
...
_This document is auto-referenced from [todo_docs.md](todo_docs.md) §3.2.
See also: [ARCHITECTURE.md](ARCHITECTURE.md), [DATA_MODEL.md](DATA_MODEL.md)_
