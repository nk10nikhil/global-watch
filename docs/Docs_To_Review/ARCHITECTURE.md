...
| **IndexedDB**             | `GLOBALWATCH_db`                           | Client-side storage for playback snapshots and temporal baseline data.                                                                                                |
...
Global Watch ships as three product variants from a single codebase. Each variant surfaces a different subset of panels, map layers, and data sources.

| Variant   | Domain                   | Focus                                             |
| --------- | ------------------------ | ------------------------------------------------- |
| `full`    | myglobalwatch.vercel.app         | Geopolitics, military, OSINT, conflicts, markets  |
| `tech`    | globalwatchtech.vercel.app    | AI/ML, startups, cybersecurity, developer tools   |
| `finance` | globalwatchfinance.vercel.app | Markets, trading, central banks, macro indicators |
...
```
localStorage('globalwatch-variant')  →  import.meta.env.VITE_VARIANT  →  default 'full'
```
...
graph TD
    ...
    subgraph LayerConfig["Layer Configuration"]
        ...
        UserPref["localStorage overrides<br/>(GLOBALWATCH-layers)"]
...
2. **User localStorage** — Stored under the key `GLOBALWATCH-layers`. Users toggle layers in the map controls UI, and their preferences persist across sessions.
...
Global Watch employs a five-tier caching strategy to minimise API costs, reduce latency, and enable offline operation.
...
graph TD
    ...
    subgraph Tier5["Tier 5: Persistent Cache"]
        ...
        LSFallback["localStorage fallback<br/>prefix: GLOBALWATCH-persistent-cache:"]
        PC --> TauriInvoke
        PC --> LSFallback
    end
...
The `GLOBALWATCH_db` IndexedDB database contains two object stores:
...
**2. Feature Toggles** — 14 `RuntimeFeatureId` values stored in localStorage under the key `GLOBALWATCH-runtime-feature-toggles`:
...
Global Watch runs machine-learning inference directly in the browser using ONNX Runtime Web via Transformers.js, with API-based fallbacks for constrained devices.
...
Global Watch uses a circuit-breaker pattern to manage transient failures across its many data sources, preventing cascade failures and providing graceful degradation.
