...
🌐 **[Live Demo: myglobalwatch.vercel.app](https://myglobalwatch.vercel.app)** | 💻 **[Tech Variant: globalwatchtech.vercel.app](https://globalwatchtech.vercel.app)**
...
![Global Watch Dashboard](images/new-world-monitor.png)

## Platform Variants

Global Watch runs two specialized variants from a single codebase, each optimized for different monitoring needs:

| Variant              | URL                                                    | Focus                                                                                      |
| -------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------------------ |
| **🌍 Global Watch** | [myglobalwatch.vercel.app](https://myglobalwatch.vercel.app)           | Geopolitical intelligence, military tracking, conflict monitoring, infrastructure security |
| **💻 Tech Monitor**  | [globalwatchtech.vercel.app](https://globalwatchtech.vercel.app) | Technology sector intelligence, AI/startup ecosystems, cloud infrastructure, tech events   |
...
The tech variant ([globalwatchtech.vercel.app](https://globalwatchtech.vercel.app)) provides specialized layers for technology sector monitoring.
...
_\*AIS data via [AISStream.io](https://aisstream.io) uses terrestrial receivers with stronger coverage in European/Atlantic waters. Middle East, Asia, and open ocean coverage is limited. Satellite AIS providers (Spire, Kpler) offer global coverage but require commercial licenses._
...
The floor applies _after_ the standard calculation—if the computed score exceeds the floor, the computed score is used. This prevents false "all clear" signals while preserving sensitivity to actual escalations.
...
The `<mark>` highlighting in search escapes text _before_ wrapping matches, preventing injection via crafted search queries.
...
The Service Status panel tracks the operational health of external services that Global Watch users may depend on.
...
// App.ts
const header = `Global Watch v${__APP_VERSION__}`;
```
...
> Matthias Schäfer, Martin Strohmeier, Vincent Lenders, Ivan Martinovic and Matthias Wilhelm. "Bringing Up OpenSky: A Large-scale ADS-B Sensor Network for Research". In _Proceedings of the 13th IEEE/ACM International Symposium on Information Processing in Sensor Networks (IPSN)_, pages 83-94, April 2014.
...
- ✅ **Variant Switcher UI** - Compact orbital navigation between Global Watch and Tech Monitor
...
_Built for situational awareness and open-source intelligence gathering._
