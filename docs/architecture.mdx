---
title: "Architecture"
description: "System design, caching strategies, bootstrap hydration, edge functions, and implementation patterns used in Global Watch."
---
...
Global Watch is written in vanilla TypeScript — no frontend framework (React, Vue, Svelte, Angular) is used. This is a deliberate architectural decision, not an oversight.
...
All five dashboard variants (Global Watch, Tech Monitor, Finance Monitor, Commodity Monitor, Happy Monitor) serve from a **single Vercel deployment**. The variant is determined at runtime by hostname detection:

| Hostname                     | Variant     |
| ---------------------------- | ----------- |
| `globalwatchtech.vercel.app`      | `tech`      |
| `globalwatchfinance.vercel.app`   | `finance`   |
| `globalwatchcommodity.vercel.app` | `commodity` |
| `globalwatchhappy.vercel.app`     | `happy`     |
| `myglobalwatch.vercel.app` (default) | `full`      |

On the desktop app, the variant is stored in `localStorage['globalwatch-variant']` and can be switched without rebuilding. The variant selector in the header bar navigates between deployed domains on the web or toggles the localStorage value on desktop.
...
Transactions are sampled at 10% to balance observability with cost. Release tracking (`GLOBALWATCH@{version}`) enables regression detection across deployments.
...
The Service Status panel tracks the operational health of external services that GlobalWatch users may depend on.
