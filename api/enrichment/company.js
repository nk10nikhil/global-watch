...
export default async function handler(req) {
  ...
  if (!domain && !name) {
    return new Response(
      JSON.stringify({ error: "Provide ?domain= or ?name= parameter" }),
      {
        status: 400,
  ...
  const companyName =
    name || (domain ? inferFromDomain(domain).inferredName : "Unknown");
