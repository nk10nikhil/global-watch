...
async function verifyTurnstile(token, ip) {
  ...
  if (!secret) {
    ...
    if (!isLocal) {
      console.error(
        "[contact] TURNSTILE_SECRET_KEY not set in production, rejecting",
      );
      return false;
    }
    return true;
...
async function sendNotificationEmail(
  ...
  const notifyEmail =
    process.env.CONTACT_NOTIFY_EMAIL || "sales@globalwatch.vercel.app";
      ...
      body: JSON.stringify({
        ...
        html: `
          <div style="font-family: -apple-system, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p style="color: #999; font-size: 12px; margin-top: 24px;">Sent from globalwatch.vercel.app enterprise contact form</p>
          </div>`,
      }),
    });
    if (!res.ok) {
...
export default async function handler(req) {
  ...
  const safeMsg =
    typeof message === "string" ? message.slice(0, MAX_MESSAGE) : undefined;
  const safeSource =
    typeof source === "string" ? source.slice(0, 100) : "enterprise-contact";
