...
describe("CircuitBreaker keyed cache — market quote isolation", () => {
  it("caches different symbol sets independently within one breaker", async () => {
    ...
    try {
      ...
      await breaker.execute(async () => techData, fallback, {
        cacheKey: "AAPL,MSFT,NVDA",
      });
      await breaker.execute(async () => metalsData, fallback, {
        cacheKey: "GLD,SLV",
      });
      ...
      const cachedMetals = await breaker.execute(
        async () => fallback,
        fallback,
        { cacheKey: "GLD,SLV" },
      );
  ...
  it("global cooldown: failing key suppresses all keys, but cache remains isolated", async () => {
    ...
    try {
      ...
      await breaker.execute(async () => watchlistData, fallback, {
        cacheKey: "AAPL,MSFT",
      });
      ...
      assert.ok(
        ...
        "breaker must observe cooldown after repeated failures",
      );
  ...
  it("evicts least-recently-used entries when maxCacheEntries is reached", async () => {
    ...
    try {
      ...
      await breaker.execute(async () => quoteResponse("A", 100), fallback, {
        cacheKey: "A",
      });
      await breaker.execute(async () => quoteResponse("B", 110), fallback, {
        cacheKey: "B",
      });
      ...
      assert.equal(
        (
          await breaker.execute(async () => fallback, fallback, {
            cacheKey: "B",
          })
        ).quotes[0]?.symbol,
        "B",
      ...
      await breaker.execute(async () => quoteResponse("C", 120), fallback, {
        cacheKey: "C",
      });
      ...
      assert.equal(
        keys.includes("A"),
        false,
        "LRU entry A should be evicted when cap is reached",
      );
  ...
  it("fresh hits update LRU order even before the cache first reaches capacity", async () => {
    ...
    try {
      ...
      await breaker.execute(async () => quoteResponse("D", 130), fallback, {
        cacheKey: "D",
      });
      ...
      assert.equal(
        keys.includes("A"),
        true,
        "fresh hit should protect A from later LRU eviction",
      );
      assert.equal(
        keys.includes("B"),
        false,
        "B should become the LRU entry and be evicted",
      );
  ...
  it("does not touch stale/getCachedOrDefault reads for LRU ordering", async () => {
    ...
    try {
      ...
      assert.equal(
        breaker.getCachedOrDefault(fallback, "A").quotes[0]?.symbol,
        "A",
      );
      ...
      assert.equal(
        keys.includes("A"),
        false,
        "stale read should not protect A from LRU eviction",
      );
      assert.equal(
        keys.includes("B"),
        true,
        "B should be evicted only if A was promoted",
      );
  ...
  it("stale SWR hits still count as used for LRU before refresh completes", async () => {
    ...
    try {
      ...
      assert.equal(
        keysBeforeRefresh.includes("C"),
        true,
        "new key C should be retained",
      );
  ...
  it("clearCache(key) only removes that key, leaving others intact", async () => {
    ...
    try {
      ...
      await breaker.execute(async () => quoteResponse("AAPL", 150), fallback, {
        cacheKey: "AAPL",
      });
      await breaker.execute(async () => quoteResponse("MSFT", 400), fallback, {
        cacheKey: "MSFT",
      });
      ...
      assert.equal(
        ...
        null,
        "cleared key must return null",
      );
      assert.notEqual(
        breaker.getCached("MSFT"),
        null,
        "other key must survive clearCache(key)",
      );
    } finally {
      clearAllCircuitBreakers();
    }
  ...
  it("clearCache() with no argument removes all keyed entries", async () => {
    ...
    try {
      ...
      assert.equal(
        breaker.getCached("AAPL"),
        null,
        "AAPL must be gone after clearCache()",
      );
      assert.equal(
        breaker.getCached("MSFT"),
        null,
        "MSFT must be gone after clearCache()",
      );
    } finally {
      clearAllCircuitBreakers();
    }
  ...
  it("works with no cacheKey (backward compat — uses default key)", async () => {
    ...
    try {
      ...
      assert.notEqual(
        cached,
        null,
        "data cached with default key must be retrievable",
      );
      assert.equal(cached.quotes[0]?.symbol, "SPY");

      // Keyed call must not interfere
      await breaker.execute(async () => quoteResponse("QQQ", 480), fallback, {
        cacheKey: "QQQ",
      });
      const stillSpy = breaker.getCached();
      assert.equal(
        stillSpy.quotes[0]?.symbol,
        "SPY",
        "keyed entry must not overwrite default key",
      );
    } finally {
      clearAllCircuitBreakers();
    }
  ...
  it("SWR background refresh is per-key (does not block other keys)", async () => {
    ...
    try {
      ...
      assert.equal(
        techResult.quotes[0]?.price,
        150,
        "SWR must return stale tech data",
      );
      assert.equal(
        metalsResult.quotes[0]?.price,
        300,
        "SWR must return stale metals data",
      );
      ...
      assert.ok(
        metalsRefreshCalled,
        "metals key must trigger its own SWR refresh",
      );
      ...
      assert.equal(
        freshTech.quotes[0]?.price,
        155,
        "tech key must have refreshed data",
      );
      assert.equal(
        freshMetals.quotes[0]?.price,
        305,
        "metals key must have refreshed data",
      );
    } finally {
      clearAllCircuitBreakers();
    }
  ...
  it("success on another key resets global failure count before cooldown trips", async () => {
    ...
    try {
      ...
      await breaker.execute(async () => quoteResponse("B", 100), fallback, {
        cacheKey: "B",
      });
      ...
      assert.ok(
        !breaker.isOnCooldown(),
        "success on key B must reset global failure count",
      );

      await breaker.execute(alwaysFail, fallback, { cacheKey: "A" });
      assert.ok(
        breaker.isOnCooldown(),
        "two new consecutive failures should trip cooldown",
      );
    } finally {
      clearAllCircuitBreakers();
    }
  ...
  it("cooldown helpers reflect breaker-wide state without a cache key", async () => {
    ...
    try {
      ...
      assert.ok(
        !breaker.isOnCooldown(),
        "fresh breaker must not be on cooldown",
      );
      ...
      assert.ok(
        breaker.isOnCooldown(),
        "isOnCooldown() must be true when breaker is on cooldown",
      );
      assert.ok(
  ...
  it("empty responses are not cached when shouldCache rejects them (P1)", async () => {
    ...
    try {
      ...
      assert.deepEqual(
        result.quotes,
        [],
        "the empty result must still be returned to the caller",
      );
      assert.equal(
      ...
      assert.ok(
        secondFetchCalled,
        "second call must invoke fn again since nothing was cached",
      );
    } finally {
      clearAllCircuitBreakers();
    }
  ...
  it("non-cacheable successes still reset failures (P2)", async () => {
    ...
    try {
      ...
      await breaker.execute(alwaysFail, fallback, {
        cacheKey: "GC=F,CL=F",
        shouldCache,
      });
      assert.ok(
        !breaker.isOnCooldown(),
        "first failure alone must not trip cooldown",
      );
      ...
      assert.ok(
        !breaker.isOnCooldown(),
        "successful empty fetch must clear failure state",
      );
      ...
      assert.ok(
        !breaker.isOnCooldown(),
        "failure count must restart after non-cacheable success",
      );
