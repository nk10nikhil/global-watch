...
describe("Edge Function shared helpers resolve", () => {
  it("_rss-allowed-domains.js re-exports shared domain list", async () => {
    ...
    assert.ok(
      domains.length > 200,
      `Expected 200+ domains, got ${domains.length}`,
    );
    assert.ok(
      domains.includes("feeds.bbci.co.uk"),
      "Expected BBC feed domain in list",
    );
  });
});
    ...
    it(`${file} is in the legacy endpoint allowlist`, () => {
      assert.ok(
        ...
        `${file} is a new api/*.js endpoint not in the allowlist. ` +
          ...
          "(proto definition → buf generate → handler in server/globalwatch/{domain}/v1/ → wired in handler.ts). " +
