...
describe("shouldInclude", () => {
  ...
  it("excludes meme candidates", () => {
    assert.ok(
      !shouldInclude(market("Will LeBron become president?", 1, 500000)),
    );
  });
...
describe("scoreMarket", () => {
  it("50% price gets maximum uncertainty (0.6)", () => {
    ...
    assert.ok(
      score >= 0.59,
      `50% market should have uncertainty ~0.6, got ${score}`,
    );
  });
  ...
  it("1% price gets near-zero uncertainty", () => {
    ...
    assert.ok(
      midScore > lowScore,
      `50% score (${midScore}) should beat 1% score (${lowScore})`,
    );
  });
  ...
  it("higher volume increases score", () => {
    ...
    assert.ok(
      highVol > lowVol,
      `$1M vol (${highVol}) should beat $1K vol (${lowVol})`,
    );
  });
  ...
  it("symmetric: 40% and 60% get same uncertainty", () => {
    ...
    assert.ok(
      Math.abs(s40 - s60) < 0.001,
      `40% (${s40}) and 60% (${s60}) should have same score`,
    );
  });
});
...
describe("filterAndScore", () => {
  ...
  it("filters expired markets", () => {
    const candidates = [
      ...
      market("ECB rate decision", 45, 50000, {
        endDate: "2099-01-01T00:00:00Z",
      }),
    ];
  ...
  it("applies tag filter", () => {
    const candidates = [
      market("AI regulation", 50, 50000, {
        tags: ["tech"],
        endDate: "2099-01-01",
      }),
      market("Fed rate cut", 50, 50000, {
        tags: ["economy"],
        endDate: "2099-01-01",
      }),
    ];
  ...
  it("relaxes price bounds when < 15 markets pass strict filter", () => {
    ...
    assert.equal(
      result.length,
      2,
      "relaxed mode should include 7% and 93% markets",
    );
  });
...
describe("regression: meme market surfacing", () => {
  it("LeBron presidential market at 1% is excluded", () => {
    ...
    assert.ok(
      !shouldInclude(m),
      "LeBron 1% market should be excluded (meme + near-certain)",
    );
    assert.ok(
      isMemeCandidate(m.title, m.yesPrice),
      "should be flagged as meme",
    );
  });
  ...
  it("LeBron market scores lower than genuine uncertain market", () => {
    ...
    assert.ok(
      real > meme,
      `Real market (${real}) should score higher than meme (${meme})`,
    );
  });
