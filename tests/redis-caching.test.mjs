...
const REDIS_MODULE_URL = pathToFileURL(
  resolve(root, "server/_shared/redis.ts"),
).href;
...
async function importRedisFresh() {
  return import(
    `${REDIS_MODULE_URL}?t=${Date.now()}-${Math.random().toString(16).slice(2)}`
  );
}
  ...
  it("coalesces concurrent misses into one upstream fetcher execution", async () => {
    ...
    try {
      ...
      assert.equal(
        fetcherCalls,
        1,
        "concurrent callers should share a single miss fetch",
      );
      ...
      assert.equal(
        getCalls,
        3,
        "each caller should still attempt one cache read",
      );
      assert.equal(
        setCalls,
        1,
        "only one cache write should happen after coalesced fetch",
      );
    } finally {
  ...
  it("parses pipeline results and skips malformed entries", async () => {
    ...
    try {
      ...
      assert.equal(
        pipelineCalls,
        1,
        "batch lookup should use one pipeline round-trip",
      );
      assert.deepEqual(map.get("k1"), { details: { id: "a1" } });
      assert.equal(
        map.has("k2"),
        false,
        "malformed JSON entry should be skipped",
      );
...
describe("cachedFetchJsonWithMeta source labeling", { concurrency: 1 }, () => {
  it("reports source=cache on Redis hit", async () => {
    ...
    globalThis.fetch = async (url) => {
      ...
      if (raw.includes("/get/")) {
        return jsonResponse({
          result: JSON.stringify({ value: "cached-data" }),
        });
      }
      throw new Error(`Unexpected fetch URL: ${raw}`);
    };
  ...
  it("reports source=fresh for ALL coalesced concurrent callers", async () => {
    ...
    try {
      ...
      assert.equal(
        b.source,
        "fresh",
        "follower 1 should report fresh (not cache)",
      );
      assert.equal(
        c.source,
        "fresh",
        "follower 2 should report fresh (not cache)",
      );
  ...
  it("TOCTOU: reports cache when Redis is populated between concurrent reads", async () => {
    ...
    globalThis.fetch = async (url) => {
      ...
      if (raw.includes("/get/")) {
        ...
        return jsonResponse({
          result: JSON.stringify({ value: "from-other-instance" }),
        });
      }
      if (raw.includes("/set/")) return jsonResponse({ result: "OK" });
      throw new Error(`Unexpected fetch URL: ${raw}`);
    ...
    try {
      ...
      const first = await redis.cachedFetchJsonWithMeta(
        "meta:test:toctou",
        60,
        async () => {
      ...
      const second = await redis2.cachedFetchJsonWithMeta(
        "meta:test:toctou",
        60,
        async () => {
      ...
      assert.equal(
        second.source,
        "cache",
        "should report cache when Redis has data",
      );
...
describe("negative-result caching", { concurrency: 1 }, () => {
  it("caches sentinel on null fetcher result and suppresses subsequent upstream calls", async () => {
    ...
    try {
      ...
      const first = await redis.cachedFetchJson(
        "neg:test:suppress",
        300,
        fetcher,
      );
      ...
      const redis2 = await importRedisFresh();
      const second = await redis2.cachedFetchJson(
        "neg:test:suppress",
        300,
        fetcher,
      );
      assert.equal(
        second,
        null,
        "second call should return null from sentinel",
      );
      assert.equal(
        fetcherCalls,
        1,
        "fetcher should NOT run again — sentinel suppresses",
      );
    } finally {
      globalThis.fetch = originalFetch;
      restoreEnv();
  ...
  it("cachedFetchJsonWithMeta returns data:null source:cache on sentinel hit", async () => {
    ...
    try {
      ...
      assert.equal(
        second.source,
        "cache",
        "sentinel hit should report source=cache",
      );
    } finally {
      globalThis.fetch = originalFetch;
      restoreEnv();
  ...
  it("does not cache sentinel when fetcher throws", async () => {
    ...
    try {
      ...
      await assert.rejects(() =>
        redis.cachedFetchJson("neg:test:throw", 300, throwingFetcher),
      );
      assert.equal(fetcherCalls, 1);
      assert.equal(
        setCalls,
        0,
        "no sentinel should be cached when fetcher throws",
      );

      const redis2 = await importRedisFresh();
      await assert.rejects(() =>
        redis2.cachedFetchJson("neg:test:throw", 300, throwingFetcher),
      );
      assert.equal(
        fetcherCalls,
        2,
        "fetcher should run again after a thrown error (no sentinel)",
      );
    } finally {
      globalThis.fetch = originalFetch;
      restoreEnv();
...
describe("theater posture caching behavior", { concurrency: 1 }, () => {
  ...
  it("coalesces concurrent calls into a single upstream fetch", async () => {
    ...
    try {
      ...
      assert.equal(
        openskyFetchCount,
        2,
        "coalesced into one fetcher invocation × 2 theater query regions",
      );
  ...
  it("falls back to stale/backup when both upstreams are down", async () => {
    ...
    try {
      ...
      assert.deepEqual(
        result,
        staleData,
        "should return stale cache when upstreams fail",
      );
    } finally {
      cleanup();
      globalThis.fetch = originalFetch;
  ...
  it("returns empty theaters when all tiers exhausted", async () => {
    ...
    try {
      ...
      assert.deepEqual(
        result,
        { theaters: [] },
        "should return empty when all tiers exhausted",
      );
    } finally {
      cleanup();
      globalThis.fetch = originalFetch;
  ...
  it("awaits stale/backup writes before returning", async () => {
    ...
    try {
      ...
      assert.ok(
        staleWritten,
        "stale tier should be written before response returns",
      );
      assert.ok(
        backupWritten,
        "backup tier should be written before response returns",
      );
    } finally {
      cleanup();
      globalThis.fetch = originalFetch;
  ...
  it("uses distinct cache keys for distinct context snapshots", async () => {
    ...
    globalThis.fetch = async (url, init = {}) => {
      ...
      if (raw.includes("/set/")) {
        ...
        const encodedValue =
          raw.slice(raw.indexOf("/set/") + 5).split("/")[1] || "";
      ...
      if (raw.includes("api.groq.com/openai/v1/chat/completions")) {
        ...
        return jsonResponse({
          choices: [{ message: { content: `brief-${groqCalls}` } }],
        });
      }
      throw new Error(`Unexpected fetch URL: ${raw}`);
    };
    ...
    try {
      ...
      assert.equal(
        groqCalls,
        2,
        "different contexts should not share one cache entry",
      );
      assert.equal(setKeys.length, 2, "one cache write per unique context");
      assert.notEqual(
        setKeys[0],
        setKeys[1],
        "context hash should differentiate cache keys",
      );
      assert.ok(
        setKeys[0]?.startsWith("ci-sebuf:v2:IL:"),
        "cache key should use v2 country-intel namespace",
      );
      assert.ok(
        setKeys[1]?.startsWith("ci-sebuf:v2:IL:"),
        "cache key should use v2 country-intel namespace",
      );
  ...
  it("uses base cache key and prompt when context is missing or blank", async () => {
    ...
    globalThis.fetch = async (url, init = {}) => {
      ...
      if (raw.includes("api.groq.com/openai/v1/chat/completions")) {
        ...
        userPrompts.push(body.messages?.[1]?.content || "");
        return jsonResponse({
          choices: [{ message: { content: "base-brief" } }],
        });
      }
      throw new Error(`Unexpected fetch URL: ${raw}`);
    };
    ...
    try {
      ...
      const first = await module.getCountryIntelBrief(
        makeCtx(
          "https://example.com/api/intelligence/v1/get-country-intel-brief?country_code=US",
        ),
        req,
      );
      ...
      assert.ok(
        setKeys[0]?.endsWith(":base"),
        "missing context should use :base cache suffix",
      );
      assert.ok(
  ...
  async function importListMilitaryFlights() {
    return importPatchedTsModule(
      "server/globalwatch/military/v1/list-military-flights.ts",
      {
        ...
        "../../../_shared/redis": resolve(root, "server/_shared/redis.ts"),
        "../../../_shared/response-headers": resolve(
          root,
  ...
  it("filters cached quantized-cell results back to the requested bbox", async () => {
    ...
    try {
      ...
      assert.equal(
        redisGetCalls,
        1,
        "handler should read quantized cache first",
      );
