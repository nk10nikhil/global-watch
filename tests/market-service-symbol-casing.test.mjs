...
const MARKET_SERVICE_URL = pathToFileURL(
  resolve(root, "src/services/market/index.ts"),
).href;
...
describe("market service symbol casing", () => {
  it("preserves distinct-case symbols in the batched request and response mapping", async () => {
    const { clearAllCircuitBreakers } = await import(
      freshImportUrl(CIRCUIT_BREAKER_URL)
    );
    ...
    globalThis.fetch = async (input) => {
      ...
      return new Response(
        JSON.stringify(
          marketResponse([quote("btc-usd", 101), quote("BTC-USD", 202)]),
        ),
        {
          status: 200,
    ...
    try {
      const { fetchMultipleStocks } = await import(
        freshImportUrl(MARKET_SERVICE_URL)
      );
  ...
  it("keeps per-request cache keys isolated when symbols differ only by case", async () => {
    ...
    globalThis.fetch = async (input) => {
      ...
      return new Response(
        JSON.stringify(marketResponse([quote(symbol, price)])),
        {
          status: 200,
          headers: { "Content-Type": "application/json" },
        },
    ...
    try {
      ...
      assert.equal(
        fetchCount,
        2,
        "case-distinct symbol sets must not share one cache entry",
      );
