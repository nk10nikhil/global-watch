...
describe("premium stock gateway enforcement", () => {
  it("requires a World Monitor key for premium stock RPCs even from trusted browser origins", async () => {
    ...
    const premiumBlocked = await handler(
      new Request(
        "https://worldmonitor.app/api/market/v1/analyze-stock?symbol=AAPL",
        {
    ...
    const premiumAllowed = await handler(
      new Request(
        "https://worldmonitor.app/api/market/v1/analyze-stock?symbol=AAPL",
        {
          headers: {
    ...
    const publicAllowed = await handler(
      new Request(
        "https://worldmonitor.app/api/market/v1/list-market-quotes?symbols=AAPL",
        {
          headers: { Origin: "https://worldmonitor.app" },
        },
      ),
