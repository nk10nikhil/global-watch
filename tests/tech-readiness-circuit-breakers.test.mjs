...
describe("economic/index.ts — per-indicator World Bank circuit breakers", () => {
  ...
  it("mirrors fredBatchBreaker pattern (consistency check)", () => {
    ...
    assert.match(
      src,
      /fredBatchBreaker\s*=/,
      "fredBatchBreaker must exist as reference",
    );
    assert.match(
      src,
      /getWbBreaker\s*\(/,
    ...
    assert.match(
      src,
      /fredBatchBreaker\s*=\s*createCircuitBreaker/,
      "fredBatchBreaker uses createCircuitBreaker",
    );
    assert.match(
      src,
      /wbBreakers\s*=\s*new\s+Map/,
      "wbBreakers uses Map for per-indicator breakers",
    );
  });
});
...
describe("CircuitBreaker isolation — independent per-indicator instances", () => {
  ...
  it("two breakers with different names are independent (failure in one does not trip the other)", async () => {
    ...
    assert.equal(
      breakerA.isOnCooldown(),
      true,
      "breakerA should be on cooldown after 2 failures",
    );
  ...
  it("empty server response does not poison the cache for other indicators", async () => {
    ...
    assert.deepEqual(
      cachedA?.data,
      [],
      "breakerA caches empty array from server hiccup",
    );
...
describe("getTechReadinessRankings — bootstrap-only data flow", () => {
  ...
  it("reads from bootstrap hydration or endpoint, never calls WB API directly", () => {
    ...
    assert.match(
      fnBody,
      /getHydratedData\s*\(\s*'techReadiness'\s*\)/,
      "Must try bootstrap hydration cache first",
    );
    assert.match(
      fnBody,
      /\/api\/bootstrap\?keys=techReadiness/,
  ...
  it("indicator codes exist in TECH_INDICATORS for seed script parity", () => {
    assert.match(
      src,
      /'IT\.NET\.USER\.ZS'/,
      "Internet Users indicator must be present",
    );
    assert.match(
      src,
      /'IT\.CEL\.SETS\.P2'/,
      "Mobile Subscriptions indicator must be present",
    );
    assert.match(
      src,
      /'IT\.NET\.BBND\.P2'/,
      "Fixed Broadband indicator must be present",
    );
    assert.match(
      src,
      /'GB\.XPD\.RSDV\.GD\.ZS'/,
      "R&D Expenditure indicator must be present",
    );
  });
});
