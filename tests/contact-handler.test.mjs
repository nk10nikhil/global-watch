...
function makeRequest(body, opts = {}) {
  return new Request("https://myglobalwatch.vercel.app/api/contact", {
  ...
  describe("validation", () => {
    it("rejects GET requests", async () => {
      const res = await handler(
        new Request("https://myglobalwatch.vercel.app/api/contact", {
          method: "GET",
          headers: { origin: "https://myglobalwatch.vercel.app" },
        }),
      );
    ...
    it("rejects free email domains with 422", async () => {
      ...
      const res = await handler(
        makeRequest(validBody({ email: "test@gmail.com" })),
      );
    ...
    it("rejects disallowed origins", async () => {
      const req = new Request("https://myglobalwatch.vercel.app/api/contact", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    it("allows in development when TURNSTILE_SECRET_KEY is unset", async () => {
      ...
      globalThis.fetch = async (url, opts) => {
        if (url.includes("fake-convex")) {
          convexCalled = true;
          return new Response(
            JSON.stringify({ status: "success", value: { status: "sent" } }),
          );
        }
