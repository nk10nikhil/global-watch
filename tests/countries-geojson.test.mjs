...
describe("countries.geojson data integrity", () => {
  it("all feature names are unique", () => {
    ...
    assert.deepStrictEqual(
      dupes,
      [],
      `Duplicate names found: ${dupes.join(", ")}`,
    );
  });
  ...
  it("-99 count stays bounded (max 25)", () => {
    const minus99 = features.filter(
      (f) => f.properties["ISO3166-1-Alpha-2"] === "-99",
    );
    assert.ok(
      minus99.length <= 25,
    ...
    assert.ok(
      minus99.length > 0,
      "Expected some -99 features for unrecognized territories",
    );
  });
});
