...
const hlsMapMatch = liveNewsSrc.match(
  /const DIRECT_HLS_MAP[^{]*\{([\s\S]*?)\};/,
);
const hlsMapEntries = hlsMapMatch
  ? [...hlsMapMatch[1].matchAll(/'([^']+)':\s*'([^']+)'/g)].map((m) => ({
      id: m[1],
      url: m[2],
    }))
  : [];
...
describe("DIRECT_HLS_MAP integrity", () => {
  it("has at least 6 entries", () => {
    assert.ok(
      hlsMapEntries.length >= 6,
      `Expected ≥6 entries, got ${hlsMapEntries.length}`,
    );
  });
  ...
  it("every key maps to an existing channel definition", () => {
    for (const { id } of hlsMapEntries) {
      assert.ok(
        allChannelIds.has(id),
        `DIRECT_HLS_MAP key '${id}' has no matching channel`,
      );
    }
  });
  ...
  it("all HLS URLs use HTTPS", () => {
    for (const { id, url } of hlsMapEntries) {
      assert.ok(
        ...
        `HLS URL for '${id}' is not HTTPS: ${url}`,
      );
    }
  });
  ...
  it("all HLS URLs contain .m3u8", () => {
    for (const { id, url } of hlsMapEntries) {
      assert.ok(
        url.includes(".m3u8"),
        `HLS URL for '${id}' does not contain .m3u8: ${url}`,
      );
    }
  });
  ...
  it("no duplicate channel IDs in the map", () => {
    ...
    assert.equal(
      ids.length,
      new Set(ids).size,
      "Duplicate IDs in DIRECT_HLS_MAP",
    );
  });
});
...
describe("channel data integrity", () => {
  ...
  it("no channel ID appears in multiple arrays with conflicting definitions", () => {
    ...
    for (const [id, count] of Object.entries(counts)) {
      if (count > 1) {
        const defs = [
          ...liveNewsSrc.matchAll(new RegExp(`id:\\s*'${id}'[^}]*}`, "g")),
        ].map((m) => m[0]);
...
describe("getDirectHlsUrl cooldown", () => {
  ...
  it("cooldown is at least 1 minute", () => {
    const match = liveNewsSrc.match(
      /HLS_COOLDOWN_MS\s*=\s*(\d+)\s*\*\s*(\d+)\s*\*\s*(\d+)/,
    );
...
describe("player decision tree", () => {
  it("switchChannel checks direct HLS before videoId validation", () => {
    ...
    const videoIdPos = switchMethod.indexOf(
      "!/^[\\w-]{10,12}$/.test(channel.videoId)",
    );
...
describe("optional channels fallback coverage", () => {
  ...
  for (const id of highPriorityOptional) {
    it(`${id} has a fallback path`, () => {
      ...
      const hasFallback = /fallbackVideoId:\s*'[A-Za-z0-9_-]{11}'/.test(
        match[0],
      );
  ...
  it("channels with useFallbackOnly also have fallbackVideoId or hlsUrl", () => {
    const useFallbackMatches = [
      ...liveNewsSrc.matchAll(
        /id:\s*'([^']+)'[^}]*useFallbackOnly:\s*true[^}]*}/g,
      ),
    ];
