...
describe("escalation adapter — country normalization structure", () => {
  ...
  it("imports nameToCountryCode and getCountryNameByCode from country-geometry", () => {
    ...
    assert.match(
      ...
      "must import getCountryNameByCode",
    );
...
describe("escalation adapter — behavioral country normalization", () => {
  before(async () => {
    ...
    const { preloadCountryGeometry } =
      await import("@/services/country-geometry");
    await preloadCountryGeometry();
  });

  it('collectSignals normalizes "Iran" protest to ISO2 "IR"', async () => {
    const { escalationAdapter } =
      await import("@/services/correlation-engine/adapters/escalation");
    const now = new Date();
    const ctx = {
      intelligenceCache: {
        protests: {
    ...
    assert.ok(
      ...
      "should produce at least one conflict signal",
    );
    for (const s of conflictSignals) {
      assert.equal(
        s.country,
        "IR",
        `conflict signal country should be "IR", got "${s.country}"`,
      );
    }
  });
  ...
  it('generateTitle shows full name "Iran" not code "IR"', async () => {
    ...
    assert.ok(
      title.includes("Iran"),
      `title should contain "Iran", got "${title}"`,
    );
    assert.ok(
      title.includes("conflict"),
      `title should contain "conflict", got "${title}"`,
    );
    assert.ok(
  ...
  it('two-letter alias "UK" normalizes to canonical "GB" via nameToCountryCode', async () => {
    ...
    for (const s of ukSignals) {
      assert.equal(
        s.country,
        "GB",
        `"UK" alias should normalize to "GB", got "${s.country}"`,
      );
    }
  });
});
