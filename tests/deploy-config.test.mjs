...
describe("deploy/cache configuration guardrails", () => {
  it("disables caching for HTML entry routes on Vercel", () => {
    ...
    assert.equal(
      getCacheHeaderValue("/index.html"),
      "no-cache, no-store, must-revalidate",
    );
  });
...
describe("security header guardrails", () => {
  ...
  it("Permissions-Policy disables all expected browser APIs", () => {
    ...
    for (const directive of expectedDisabled) {
      assert.ok(
        ...
        `Permissions-Policy missing: ${directive}`,
      );
    }
  });
  ...
  it("CSP connect-src does not allow unencrypted WebSocket (ws:)", () => {
    ...
    assert.ok(
      ...
      "CSP connect-src should keep wss: for secure WebSocket",
    );
  });
  ...
  it("CSP script-src uses hashes instead of unsafe-inline", () => {
    ...
    assert.ok(
      ...
      "CSP script-src must not contain unsafe-inline — use sha256 hashes",
    );
    assert.match(
      scriptSrc,
      /sha256-/,
      "CSP script-src should contain at least one sha256 hash",
    );
  });

  it("security.txt exists in public/.well-known/", () => {
    const secTxt = readFileSync(
    ...
    assert.match(
      secTxt,
      /^Contact:/m,
      "security.txt must have a Contact field",
    );
    assert.match(
      secTxt,
      /^Expires:/m,
      "security.txt must have an Expires field",
    );
  });
});
