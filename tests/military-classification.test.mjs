...
const sharedSrc = readFileSync(
  join(root, "server/globalwatch/military/v1/_shared.ts"),
  "utf-8",
);
...
function extractArray(src, name) {
  ...
  const re = new RegExp(
    `(?:export )?const ${name}\\s*=\\s*(?:new Set\\()?\\[([\\s\\S]*?)\\]`,
  );
...
const SHORT_MILITARY_PREFIXES = extractArray(
  sharedSrc,
  "SHORT_MILITARY_PREFIXES",
);
...
describe("Military hex range classifier (client-side)", () => {
  ...
  describe("validates range boundaries are tight", () => {
    it("US military starts at ADF7C8, not A00000", () => {
      assert.ok(
        !isKnownMilitaryHex("ADF7C7"),
        "ADF7C7 (last N-number) should be civilian",
      );
