...
const SRC = readFileSync(
  join(__dirname, "..", "src", "services", "oref-locations.ts"),
  "utf8",
);
...
describe("oref-locations.ts", () => {
  ...
  it("has a substantial number of location entries", () => {
    ...
    assert.ok(
      matches.length > 1000,
      `expected >1000 entries, got ${matches.length}`,
    );
  });
  ...
  it("contains known cities", () => {
    ...
    assert.ok(
      SRC.includes("'אשקלון'") || SRC.includes("Ashkelon"),
      "should include Ashkelon",
    );
    assert.ok(
      SRC.includes("'באר שבע'") ||
        SRC.includes("Be'er Sheva") ||
        SRC.includes("Beer Sheva"),
  ...
  it("handles empty input", () => {
    assert.ok(
      SRC.includes("if (!hebrew) return hebrew"),
      "should guard empty input",
    );
  });
