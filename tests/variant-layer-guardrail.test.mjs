...
const layerDefsSource = readFileSync(
  new URL("map-layer-definitions.ts", SRC),
  "utf8",
);
...
function extractRecordBlock(source, name) {
  const re = new RegExp(
    `(?:const|export const)\\s+${name}[^=]*=\\s*\\{([\\s\\S]*?)\\n\\};`,
  );
...
function extractLayerRenderers(source) {
  ...
  while ((m = defRe.exec(body)) !== null) {
    const key = m[1];
    const fullCall = m[0];
    const renderersMatch = fullCall.match(
      /\[([^\]]*)\]\s*(?:,\s*(?:'[^']*'|undefined))?\s*\)/,
    );
    if (renderersMatch) {
      const renderers =
        renderersMatch[1].match(/'(\w+)'/g)?.map((s) => s.replace(/'/g, "")) ??
        [];
...
const svgOnlyLayers =
  extractRecordBlock(layerDefsSource, "SVG_ONLY_LAYERS") ?? {};
...
const VARIANT_DEFAULTS = {
  ...
  finance: {
    desktop: "FINANCE_MAP_LAYERS",
    mobile: "FINANCE_MOBILE_MAP_LAYERS",
  },
  happy: { desktop: "HAPPY_MAP_LAYERS", mobile: "HAPPY_MOBILE_MAP_LAYERS" },
  commodity: {
    desktop: "COMMODITY_MAP_LAYERS",
    mobile: "COMMODITY_MOBILE_MAP_LAYERS",
  },
};

describe("variant layer guardrail", () => {
  for (const [variant, { desktop, mobile }] of Object.entries(
    VARIANT_DEFAULTS,
  )) {
