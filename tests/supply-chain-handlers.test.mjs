...
describe("Spike detection", () => {
  it("detects spike when value > mean + 2*stdDev", () => {
    assert.equal(
      detectSpike([100, 102, 98, 101, 99, 100, 103, 97, 100, 500]),
      true,
    );
  });
...
describe("Chokepoint assignment", () => {
  it("matches explicit chokepoint names", () => {
    assert.equal(
      resolveChokepointId({
        text: "Convoy delays reported in the Suez Canal transit corridor",
      }),
      "suez",
    );
    assert.equal(
      resolveChokepointId({
        text: "New advisory issued for Strait of Hormuz tanker traffic",
      }),
      "hormuz",
    );
  });
  ...
  it("does not classify a single broad regional token", () => {
    assert.equal(
      resolveChokepointId({
        text: "General security alert for Red Sea traffic",
      }),
      null,
    );
  });
...
describe("Threat config freshness", () => {
  ...
  it("becomes stale after the max-age window", () => {
    ...
    assert.equal(
      isThreatConfigFresh(reviewedAtMs + oneHundredTwentyOneDaysMs),
      false,
    );
  });
});
