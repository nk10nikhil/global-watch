...
import { computeCIIScores } from "../server/globalwatch/intelligence/v1/get-risk-scores.ts";
...
describe("CII scoring", () => {
  ...
  it("UCDP war floor: composite >= 70", () => {
    ...
    assert.ok(
      ua.combinedScore >= 70,
      `UA score ${ua.combinedScore} should be >= 70 with UCDP war`,
    );
  });
  ...
  it("outage TOTAL severity gives higher unrest component than PARTIAL", () => {
    ...
    auxTotal.outages = [
      { countryCode: "DE", severity: "OUTAGE_SEVERITY_TOTAL" },
    ];
    const auxPartial = emptyAux();
    auxPartial.outages = [
      { countryCode: "DE", severity: "OUTAGE_SEVERITY_PARTIAL" },
    ];
  ...
  it("GPS high level gives higher weight than medium", () => {
    ...
    auxHigh.gpsHexes = Array.from({ length: 5 }, () => ({
      lat: 33.0,
      lon: 35.0,
      level: "high",
    }));
    const auxMed = emptyAux();
    auxMed.gpsHexes = Array.from({ length: 5 }, () => ({
      lat: 33.0,
      lon: 35.0,
      level: "medium",
    }));
  ...
  it("log2 scaling dampens high-volume low-multiplier countries vs linear", () => {
    const manyProtests = Array.from({ length: 100 }, () =>
      acledEvent("United States", "Protests"),
    );
    const fewProtests = Array.from({ length: 10 }, () =>
      acledEvent("United States", "Protests"),
    );
    const many = scoreFor(computeCIIScores(manyProtests, emptyAux()), "US")!;
    const few = scoreFor(computeCIIScores(fewProtests, emptyAux()), "US")!;
    const ratio =
      many.components!.ciiContribution /
      Math.max(1, few.components!.ciiContribution);
    assert.ok(
      ratio < 5,
  ...
  it("scores capped at 100", () => {
    const acled = Array.from({ length: 200 }, () =>
      acledEvent("Syria", "Battles", 50),
    );
    const aux = emptyAux();
    ...
    for (const s of scores) {
      assert.ok(
        s.combinedScore <= 100,
        `${s.region} score ${s.combinedScore} should be <= 100`,
      );
    }
  });
  ...
  it("UAE geo events attributed to AE not SA despite bbox overlap", () => {
    ...
    assert.ok(
      ae.components!.militaryActivity > 0,
      "AE should get the Dubai GPS hex",
    );
    assert.equal(
      sa.components!.militaryActivity,
      0,
      "SA should not get the Dubai GPS hex",
    );
  });
