...
async function runTests() {
  if (!OREF_PROXY_AUTH) {
    console.log(
      "SKIP: OREF_PROXY_AUTH not set — set it to run proxy connectivity tests",
    );
    console.log(
  ...
  try {
    ...
    if (cleaned === "" || cleaned === "[]" || cleaned === "null") {
      console.log(
        "  PASS: OREF alerts fetch → no active alerts (empty response)",
      );
    } else {
      ...
      assert.ok(
        alerts[0].id || alerts[0].cat,
        "alert should have id or cat field",
      );
      console.log(
        `  PASS: OREF alerts fetch → ${alerts.length} active alert(s)`,
      );
    }
    passed++;
  } catch (err) {
