...
describe("Fix 1: message.reasoning fallback", () => {
  const src = readSrc("server/globalwatch/news/v1/summarize-article.ts");
...
describe("Fix 2: thinking tag stripping formats", () => {
  ...
  it("strips <|thinking|> tags", () => {
    assert.ok(
      src.includes("\\|thinking\\|"),
      "Should handle <|thinking|> tags",
    );
  });
  ...
  it("handles unterminated <think> blocks", () => {
    ...
    const unterminatedSection = lines.findIndex((l) =>
      l.includes("Strip unterminated"),
    );
    assert.ok(
      unterminatedSection > -1,
      "Should have unterminated block stripping section",
    );
    const sectionSlice = lines
      .slice(unterminatedSection, unterminatedSection + 8)
      .join("\n");
    assert.ok(
      sectionSlice.includes("<think>"),
      "Should strip unterminated <think>",
    );
  });
  ...
  it("handles unterminated <|thinking|> blocks", () => {
    ...
    const sectionSlice = lines
      ...
      .join("\n");
    assert.ok(
      sectionSlice.includes("\\|thinking\\|"),
      "Should strip unterminated <|thinking|>",
    );
  });
  ...
  it("handles unterminated <reasoning> blocks", () => {
    ...
    const sectionSlice = lines
      ...
      .join("\n");
    assert.ok(
      sectionSlice.includes("<reasoning>"),
      "Should strip unterminated <reasoning>",
    );
  });
  ...
  it("handles unterminated <reflection> blocks", () => {
    ...
    const sectionSlice = lines
      ...
      .join("\n");
    assert.ok(
      sectionSlice.includes("<reflection>"),
      "Should strip unterminated <reflection>",
    );
  });

  it("strips <|begin_of_thought|> tags (terminated)", () => {
    assert.ok(
      src.includes("begin_of_thought"),
      "Should handle <|begin_of_thought|> tags",
    );
  });
  ...
  it("strips <|begin_of_thought|> tags (unterminated)", () => {
    ...
    const sectionSlice = lines
      .slice(unterminatedSection, unterminatedSection + 10)
      .join("\n");
    assert.ok(
...
describe("Fix 3: hasReasoningPreamble", () => {
  ...
  function extractRegex(source, name) {
    const match = source.match(
      new RegExp(`export const ${name}\\s*=\\s*(/[^;]+/[gimsuy]*)`),
    );
  ...
  it('detects "So we need to..."', () => {
    assert.ok(
      hasReasoningPreamble("So we need to lead with WHAT happened and WHERE."),
    );
  });
  ...
  it('detects "Summarize the top story..."', () => {
    assert.ok(
      hasReasoningPreamble("Summarize the top story: the headline says..."),
    );
  });

  it('detects "The top story is likely..."', () => {
    assert.ok(
      hasReasoningPreamble(
        "The top story is likely the first headline about Russia.",
      ),
    );
  });
  ...
  it("passes clean geopolitical summary", () => {
    assert.ok(
      !hasReasoningPreamble(
        "Iran's nuclear program faces increased international scrutiny.",
      ),
    );
  });
  ...
  it("passes clean event summary", () => {
    assert.ok(
      !hasReasoningPreamble(
        "The US Treasury announced new sanctions against Russian entities.",
      ),
    );
  });
  ...
  it("passes clean protest summary", () => {
    assert.ok(
      !hasReasoningPreamble(
        "Protests erupted in multiple cities across France.",
      ),
    );
  });
  ...
  it("passes summary starting with country name", () => {
    assert.ok(
      !hasReasoningPreamble("Russia has escalated its nuclear rhetoric."),
    );
  });
  ...
  it('detects "First, I need to..."', () => {
    assert.ok(
      hasReasoningPreamble(
        "First, I need to identify the most important headline.",
      ),
    );
  });
  ...
  it('detects "To summarize the headlines..."', () => {
    assert.ok(
      hasReasoningPreamble(
        "To summarize the headlines, we look at the key events.",
      ),
    );
  });
  ...
  it('detects "Step 1: analyze..."', () => {
    assert.ok(
      hasReasoningPreamble("Step 1: analyze the most important headline."),
    );
  });
  ...
  it('passes "First responders arrived..."', () => {
    assert.ok(
      !hasReasoningPreamble(
        "First responders arrived at the scene of the earthquake.",
      ),
    );
  });
  ...
  it('passes "To summarize, the summit concluded..."', () => {
    assert.ok(
      !hasReasoningPreamble(
        "To summarize, the summit concluded with a joint statement.",
      ),
    );
  });
  ...
  it('passes "My task force deployed..."', () => {
    assert.ok(
      !hasReasoningPreamble("My task force deployed to the border region."),
    );
  });
