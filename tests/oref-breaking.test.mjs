...
describe("breaking-news-alerts oref_siren integration", () => {
  it("includes oref_siren in origin union type", () => {
    assert.ok(
      SRC.includes("'oref_siren'"),
      "origin type should include oref_siren",
    );
  });
  ...
  it("imports OrefAlert type", () => {
    assert.ok(
      SRC.includes("import type { OrefAlert }") ||
        SRC.includes("import { OrefAlert }"),
  ...
  it("uses stable dedupe key from alert identifiers", () => {
    ...
    assert.ok(
      SRC.includes(".sort()"),
      "key parts should be sorted for stability",
    );
  });

  it("sets threatLevel to critical", () => {
    assert.ok(
      SRC.includes("threatLevel: 'critical'"),
      "oref alerts should be critical",
    );
  });
  ...
  it("bypasses global cooldown (no isGlobalCooldown check)", () => {
    ...
    assert.ok(
      !fnBody.includes("isGlobalCooldown"),
      "should not check global cooldown",
    );
  });
...
describe("data-loader oref breaking news wiring", () => {
  const DL = readFileSync(
    join(__dirname, "..", "src", "app", "data-loader.ts"),
    "utf8",
  );
  ...
  it("calls dispatchOrefBreakingAlert on initial fetch", () => {
    const orefSection = DL.slice(
      DL.indexOf("// OREF sirens"),
      DL.indexOf("// GPS/GNSS"),
    );
  ...
  it("calls dispatchOrefBreakingAlert in onOrefAlertsUpdate callback", () => {
    ...
    const callbackSection = orefSection.slice(
      orefSection.indexOf("onOrefAlertsUpdate"),
    );
    assert.ok(
