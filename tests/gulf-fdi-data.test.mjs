...
describe("gulf-fdi dataset integrity", () => {
  ...
  it("has unique IDs", () => {
    ...
    assert.equal(
      uniqueCount,
      ids.length,
      "Expected all gulf-fdi IDs to be unique",
    );
  });
  ...
  it("keeps latitude/longitude in valid ranges", () => {
    for (const investment of GULF_INVESTMENTS) {
      assert.ok(
        Number.isFinite(investment.lat) &&
          investment.lat >= -90 &&
          investment.lat <= 90,
        `Invalid lat for ${investment.id}: ${investment.lat}`,
      );
      assert.ok(
        Number.isFinite(investment.lon) &&
          investment.lon >= -180 &&
          investment.lon <= 180,
  ...
  it("keeps optional numeric fields in sane bounds", () => {
    for (const investment of GULF_INVESTMENTS) {
      if (investment.investmentUSD != null) {
        assert.ok(
          Number.isFinite(investment.investmentUSD) &&
            investment.investmentUSD > 0,
  ...
  it("validates year and URL fields when present", () => {
    for (const investment of GULF_INVESTMENTS) {
      ...
      if (
        investment.yearAnnounced != null &&
        investment.yearOperational != null
      ) {
        assert.ok(
