...
describe("Bootstrap cache key registry", () => {
  ...
  const cacheKeysBlock =
    cacheKeysSrc.match(/BOOTSTRAP_CACHE_KEYS[^{]*\{([^}]+)\}/)?.[1] ?? "";
  ...
  it("exports BOOTSTRAP_CACHE_KEYS with at least 10 entries", () => {
    ...
    assert.ok(
      ...
      `Expected ≥10 keys, found ${matches?.length ?? 0}`,
    );
  });
  ...
  it("api/bootstrap.js inlined keys match server/_shared/cache-keys.ts", () => {
    ...
    assert.ok(
      Object.keys(canonical).length >= 10,
      "Canonical registry too small",
    );
    ...
    for (const [name, key] of Object.entries(inlined)) {
      assert.equal(
        canonical[name],
        key,
        `Extra inlined key '${name}' not in canonical registry`,
      );
    }
  });
  ...
  it("every cache key maps to a handler file with a matching cache key string", () => {
    ...
    const handlerDirs = join(root, "server", "GLOBALWATCH");
    const handlerFiles = [];
    function walk(dir) {
    ...
    walk(handlerDirs);
    const allHandlerCode = handlerFiles
...
describe("Bootstrap endpoint (api/bootstrap.js)", () => {
  ...
  it("defines BOOTSTRAP_CACHE_KEYS inline", () => {
    assert.ok(
      src.includes("BOOTSTRAP_CACHE_KEYS"),
      "Missing BOOTSTRAP_CACHE_KEYS definition",
    );
  });

  it("defines getCachedJsonBatch inline (self-contained, no server imports)", () => {
    assert.ok(
      src.includes("getCachedJsonBatch"),
      "Missing getCachedJsonBatch function",
    );
    assert.ok(
  ...
  it("sets Cache-Control header with s-maxage for both tiers", () => {
    assert.ok(
      src.includes("s-maxage=3600"),
      "Missing s-maxage=3600 for slow tier",
    );
    assert.ok(
      src.includes("s-maxage=600"),
      "Missing s-maxage=600 for fast tier",
    );
    assert.ok(
      src.includes("stale-while-revalidate"),
...
describe("Frontend hydration (src/services/bootstrap.ts)", () => {
  ...
  it("exports getHydratedData function", () => {
    assert.ok(
      src.includes("export function getHydratedData"),
      "Missing getHydratedData export",
    );
  });
  ...
  it("has a fast timeout cap to avoid regressing startup", () => {
    const timeoutMatch = src.match(
      /(?:AbortSignal\.timeout|setTimeout)\D+(\d+)\)/,
    );
    assert.ok(timeoutMatch, "Missing timeout");
    const ms = parseInt(timeoutMatch[1], 10);
    assert.ok(
      ms <= 2000,
      `Timeout ${ms}ms too high — should be ≤2000ms to avoid regressing startup`,
    );
  });

  it("fetches tiered bootstrap URLs", () => {
    assert.ok(
      src.includes("/api/bootstrap?tier="),
      "Missing tiered bootstrap fetch URLs",
    );
  });
  ...
  it("fetches both tiers in parallel", () => {
    assert.ok(
      ...
      "Missing Promise.all for parallel tier fetches",
    );
  ...
  it("every bootstrap key has a getHydratedData consumer in src/", () => {
    ...
    function walk(dir) {
      for (const entry of readdirSync(dir)) {
        const full = join(dir, entry);
        if (statSync(full).isDirectory()) walk(full);
        else if (entry.endsWith(".ts") && !full.includes("/generated/"))
          srcFiles.push(full);
      }
    }
    walk(join(root, "src"));
...
describe("Bootstrap tier definitions", () => {
  ...
  const cacheKeysSrc = readFileSync(
    join(root, "server", "_shared", "cache-keys.ts"),
    "utf-8",
  );
  ...
  it("SLOW_KEYS + FAST_KEYS cover all BOOTSTRAP_CACHE_KEYS with no overlap", () => {
    ...
    assert.equal(
      intersection.length,
      0,
      `Overlap between tiers: ${intersection.join(", ")}`,
    );
  });
  ...
  it("tier sets in bootstrap.js match BOOTSTRAP_TIERS in cache-keys.ts", () => {
    ...
    for (const k of slow) {
      assert.equal(
        tiers[k],
        "slow",
        `SLOW_KEYS has '${k}' but BOOTSTRAP_TIERS says '${tiers[k]}'`,
      );
    }
    for (const k of fast) {
      assert.equal(
        tiers[k],
        "fast",
        `FAST_KEYS has '${k}' but BOOTSTRAP_TIERS says '${tiers[k]}'`,
      );
    }
...
describe("Adaptive backoff adopters", () => {
  it("ServiceStatusPanel.fetchStatus returns Promise<boolean>", () => {
    const src = readFileSync(
      join(root, "src/components/ServiceStatusPanel.ts"),
      "utf-8",
    );
    assert.ok(
    ...
    assert.ok(
      src.includes("lastServicesJson"),
      "Missing lastServicesJson for change detection",
    );
  });

  it("MacroSignalsPanel.fetchData returns Promise<boolean>", () => {
    const src = readFileSync(
      join(root, "src/components/MacroSignalsPanel.ts"),
      "utf-8",
    );
    assert.ok(
    ...
    assert.ok(
      src.includes("lastTimestamp"),
      "Missing lastTimestamp for change detection",
    );
  });

  it("StrategicRiskPanel.refresh returns Promise<boolean>", () => {
    const src = readFileSync(
      join(root, "src/components/StrategicRiskPanel.ts"),
      "utf-8",
    );
    assert.ok(
