...
describe("flushStaleRefreshes behavior", () => {
  ...
  it("does nothing when hiddenSince is 0", () => {
    ...
    assert.equal(
      called,
      false,
      "No services should flush when hiddenSince is 0",
    );
  });
  ...
  it("skips services hidden for less than their interval", () => {
    ...
    assert.equal(
      ctx.hiddenSince,
      0,
      "hiddenSince must still be reset even if no services flushed",
    );
  });
  ...
  it("staggers re-triggered services deterministically by 150ms", () => {
    ...
    assert.deepEqual(
      timestamps,
      [0, 150, 300],
      "Services should fire in 150ms steps",
    );
  });
  ...
  it("cleans up stale flush timeout IDs after triggering", () => {
    ...
    assert.ok(
      ctx.flushTimeoutIds.size > 0,
      "Should have pending flush timeout IDs",
    );
