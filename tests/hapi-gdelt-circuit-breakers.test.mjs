...
describe("conflict/index.ts — per-country HAPI circuit breakers", () => {
  ...
  const breakerSection = src.slice(
    src.indexOf("hapiBreakers"),
    src.indexOf("hapiBreakers") + 400,
  );
  const fnStart = src.indexOf("export async function fetchHapiSummary");
  assert.ok(
    fnStart !== -1,
    "fetchHapiSummary not found in conflict/index.ts — was it renamed?",
  );
...
describe("gdelt-intel.ts — dedicated circuit breakers per GDELT query type", () => {
  ...
  assert.ok(
    regStart !== -1,
    "fetchGdeltArticles not found in gdelt-intel.ts — was it renamed?",
  );
...
describe("CircuitBreaker isolation — HAPI per-country independence", () => {
  ...
  it("HAPI: failure in one country does not trip another", async () => {
    ...
    try {
      const breakerUS = createCircuitBreaker({
        name: "HDX HAPI:US",
        cacheTtlMs: 30 * 60 * 1000,
      });
      const breakerRU = createCircuitBreaker({
        name: "HDX HAPI:RU",
        cacheTtlMs: 30 * 60 * 1000,
      });
...
describe("CircuitBreaker isolation — GDELT split breaker independence", () => {
  ...
  it("GDELT: positive breaker failure does not trip regular breaker", async () => {
    ...
    try {
      ...
      const positive = createCircuitBreaker({
        name: "GDELT Positive",
        cacheTtlMs: 10 * 60 * 1000,
      });
      ...
      const realArticles = {
        articles: [
          { url: "https://news.example/military", title: "Conflict update" },
        ],
        totalArticles: 1,
      };
  ...
  it("GDELT: regular and positive breakers cache different data independently", async () => {
    ...
    try {
      ...
      const militaryData = {
        articles: [
          {
            url: "https://news.example/military",
      ...
      const peaceData = {
        articles: [
          { url: "https://good.example/peace", title: "Peace agreement" },
        ],
        totalArticles: 1,
      };
      ...
      const cachedPositive = await positive.execute(
        async () => fallback,
        fallback,
      );
