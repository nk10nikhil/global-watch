  ...
  it("returns 3-element coalesce for CJK languages", () => {
    for (const lang of ["zh", "ja", "ko"]) {
      ...
      assert.equal(
        expr.length,
        4,
        `lang=${lang} should have coalesce + 3 gets`,
      );
...
describe("isLocalizableTextField", () => {
  ...
  describe("falsy / non-object values", () => {
    ...
    it("rejects undefined", () =>
      assert.equal(isLocalizableTextField(undefined), false));
    it("rejects empty string", () =>
      assert.equal(isLocalizableTextField(""), false));
    it("rejects false", () =>
      assert.equal(isLocalizableTextField(false), false));
  ...
  describe("expression arrays", () => {
    ...
    it("rejects expression without name references", () => {
      ...
      assert.equal(
        isLocalizableTextField(["coalesce", ["get", "class"]]),
        false,
      );
    });
  });
  ...
  it("localizes all expected layers and skips housenumber in full mock", () => {
    ...
    for (const layer of CARTO_LAYERS) {
      if (layer.shouldLocalize) {
        assert.ok(
          localizedIds.has(layer.id),
          `expected "${layer.id}" to be localized`,
        );
      } else {
        assert.ok(
          !localizedIds.has(layer.id),
          `expected "${layer.id}" to NOT be localized`,
        );
      }
    }
  });
