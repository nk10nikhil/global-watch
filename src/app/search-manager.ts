...
export class SearchManager implements AppModule {
  ...
  private setupSearchModal(): void {
    ...
    this.ctx.searchModal.registerSource(
      "country",
      this.buildCountrySearchItems(),
    );

    this.ctx.searchModal.setActivePanels(Object.keys(this.ctx.panels));
    this.ctx.searchModal.setOnSelect((result) =>
  ...
  private handleCommand(cmd: Command): void {
    ...
    switch (category) {
      ...
      case "layers": {
        const allowed = getAllowedLayerKeys(
          (SITE_VARIANT || "full") as MapVariant,
        );
        if (action === "all") {
          for (const key of Object.keys(this.ctx.mapLayers)) {
            this.ctx.mapLayers[key as keyof MapLayers] = allowed.has(
              key as keyof MapLayers,
            );
          }
        } else if (action === "none") {
          for (const key of Object.keys(this.ctx.mapLayers))
      ...
      case "layer": {
        ...
        const variantAllowed = getAllowedLayerKeys(
          (SITE_VARIANT || "full") as MapVariant,
        );
  ...
  private highlightNewsItem(itemId: string): void {
    setTimeout(() => {
      const item = document.querySelector(
        `[data-news-id="${CSS.escape(itemId)}"]`,
      );
      if (item) {
  ...
  private buildCountrySearchItems(): {
    ...
    const panelScores =
      (this.ctx.panels["cii"] as CIIPanel | undefined)?.getScores() ?? [];
