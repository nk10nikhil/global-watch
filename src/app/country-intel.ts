...
import { IntelligenceServiceClient } from "@/generated/client/globalwatch/intelligence/v1/service_client";
...
export class CountryIntelManager implements AppModule {
  ...
  async openCountryBriefByCode(
    ...
    const canonicalName =
      TIER1_COUNTRIES[code] || CountryIntelManager.resolveCountryName(code);
    ...
    this.ctx.countryBriefPage.updateSignalDetails?.(
      this.buildSignalDetails(code),
    );
    this.ctx.countryBriefPage.updateMilitaryActivity?.(
      this.buildMilitarySummary(code, country),
    );
    ...
    const searchTerms = CountryIntelManager.getCountrySearchTerms(
      country,
      code,
    );
    ...
    const filteredNews = matchingNews
      .filter((n) => {
        ...
        const otherPos = CountryIntelManager.firstMentionPosition(
          t,
          otherCountryTerms,
        );
        return (
      ...
      .sort((a, b) => {
        const severityDelta =
          this.newsSeverityRank(b) - this.newsSeverityRank(a);
      ...
      if (briefText) {
        ...
        if (fallbackBrief) {
          ...
          if (signals.protests > 0)
            lines.push(
              t("countryBrief.fallback.protestsDetected", {
                count: String(signals.protests),
              }),
            );
          ...
          if (signals.militaryVessels > 0)
            lines.push(
              t("countryBrief.fallback.vesselsTracked", {
                count: String(signals.militaryVessels),
              }),
            );
          if (signals.activeStrikes > 0)
            lines.push(
              t("countryBrief.fallback.activeStrikes", {
                count: String(signals.activeStrikes),
              }),
            );
          ...
          if (signals.outages > 0)
            lines.push(
              t("countryBrief.fallback.internetOutages", {
                count: String(signals.outages),
              }),
            );
          if (signals.criticalNews > 0)
            lines.push(
              `🚨 Critical headlines in scope: ${signals.criticalNews}`,
            );
          if (signals.cyberThreats > 0)
            lines.push(`🛡️ Cyber threat indicators: ${signals.cyberThreats}`);
          if (signals.aisDisruptions > 0)
            lines.push(
              `🚢 Maritime AIS disruptions: ${signals.aisDisruptions}`,
            );
          if (signals.satelliteFires > 0)
            lines.push(
              `🔥 Satellite fire detections: ${signals.satelliteFires}`,
            );
          if (signals.temporalAnomalies > 0)
            lines.push(
              `⏱️ Temporal anomaly alerts: ${signals.temporalAnomalies}`,
            );
          if (signals.earthquakes > 0)
            lines.push(
              t("countryBrief.fallback.recentEarthquakes", {
                count: String(signals.earthquakes),
              }),
            );
          ...
          if (context.stockIndex)
            lines.push(
              t("countryBrief.fallback.stockIndex", {
                value: context.stockIndex,
              }),
            );
          if (lines.length > 0) {
            this.ctx.countryBriefPage?.updateBrief({
  ...
  private async fetchCountryIntelBrief(
    ...
    const resp = await fetch(
      toApiUrl(
        `/api/intelligence/v1/get-country-intel-brief?${params.toString()}`,
      ),
      {
        method: "GET",
  ...
  private buildBriefContextSnapshot(
    ...
    if (signals.travelAdvisoryMaxLevel) {
      lines.push(
        `Travel advisory max level: ${signals.travelAdvisoryMaxLevel}`,
      );
    }

    const stockIndex =
      typeof context.stockIndex === "string" ? context.stockIndex : "";
    if (stockIndex) lines.push(`Stock index: ${stockIndex}`);

    const convergenceScore =
      typeof context.convergenceScore === "number"
        ? context.convergenceScore
        : null;
    const signalTypes = Array.isArray(context.signalTypes)
      ? (context.signalTypes as string[])
      : [];
    ...
    if (regionalConvergence.length > 0) {
      lines.push(
        `Regional context: ${regionalConvergence.slice(0, 3).join(" | ")}`,
      );
    }

    const headlines = Array.isArray(context.headlines)
      ? (context.headlines as string[])
      : [];
  ...
  private mountCountryTimeline(code: string, country: string): void {
    ...
    const hasGeoShape =
      hasCountryGeometry(code) || !!CountryIntelManager.COUNTRY_BOUNDS[code];
  ...
  getCountrySignals(code: string, country: string): CountryBriefSignals {
    ...
    if (countryCluster) {
      for (const s of countryCluster.signals) {
        ...
        else if (s.type === "temporal_anomaly")
          signalTypeCounts.temporalAnomalies++;
      }
    }
    const globalTemporalAnomalies = globalCluster
      ? globalCluster.signals.filter((s) => s.type === "temporal_anomaly")
          .length
      : 0;
    ...
    const criticalNews = this.ctx.latestClusters.filter((cluster) => {
      ...
      const ourPos = CountryIntelManager.firstMentionPosition(
        title,
        searchTerms,
      );
      const otherPos = CountryIntelManager.firstMentionPosition(
      ...
      if (ourPos === Infinity || (otherPos !== Infinity && otherPos < ourPos))
        return false;
      return (
        cluster.isAlert ||
    ...
    if (this.ctx.intelligenceCache.military) {
      militaryFlights = this.ctx.intelligenceCache.military.flights.filter(
        (f) =>
          hasGeoShape
            ? this.isInCountry(f.lat, f.lon, code)
            : f.operatorCountry?.toUpperCase() === code,
      ).length;
      militaryVessels = this.ctx.intelligenceCache.military.vessels.filter(
        (v) =>
          hasGeoShape
            ? this.isInCountry(v.lat, v.lon, code)
            : v.operatorCountry?.toUpperCase() === code,
    ...
    if (this.ctx.intelligenceCache.advisories) {
      ...
      for (const a of countryAdvisories) {
        if (
          a.level &&
          (advisoryLevelRank[a.level] || 0) >
            (advisoryLevelRank[travelAdvisoryMaxLevel || ""] || 0)
        ) {
    ...
    return {
      ...
      gpsJammingHexes:
        (ciiData?.gpsJammingHighCount ?? 0) +
        (ciiData?.gpsJammingMediumCount ?? 0),
      isTier1,
    };
  }
  ...
  private buildSignalDetails(code: string): CountryDeepDiveSignalDetails {
    ...
    for (const signal of rankedSignals) {
      const severity = this.normalizeSignalSeverity(
        signal.type,
        signal.severity,
      );
  ...
  private buildMilitarySummary(
    ...
    const centroid = getCountryCentroid(
      code,
      CountryIntelManager.COUNTRY_BOUNDS,
    );
    const nearbyBases = centroid
      ? getNearbyInfrastructure(centroid.lat, centroid.lon, ["base"])
          ...
          .map((base) => ({
            ...
            country: MILITARY_BASES.find((entry) => entry.id === base.id)
              ?.country,
          }))
      : [];
  ...
  private buildEconomicIndicators(
    ...
    if (
      countryData?.displacementOutflow &&
      countryData.displacementOutflow > 0
    ) {
      const displaced =
  ...
  private mapSignalType(
    type: string,
  ): CountryDeepDiveSignalDetails["recentHigh"][number]["type"] {
    if (type === "military_flight" || type === "military_vessel")
      return "MILITARY";
  ...
  static getOtherCountryTerms(code: string): string[] {
    ...
    Object.entries(CountryIntelManager.COUNTRY_ALIASES).forEach(
      ([countryCode, aliases]) => {
