...
export class App {
  ...
  private async primeVisiblePanelData(forceAll = false): Promise<void> {
    ...
    const primeTask = (key: string, task: () => Promise<unknown>): void => {
      if (this.visiblePanelPrimed.has(key) || this.state.inFlight.has(key))
        return;
      const wrapped = (async () => {
    ...
    if (shouldPrime("service-status")) {
      const panel = this.state.panels["service-status"] as
        | ServiceStatusPanel
        | undefined;
      if (panel) primeTask("service-status", () => panel.fetchStatus());
    }
    if (shouldPrime("macro-signals")) {
      const panel = this.state.panels["macro-signals"] as
        | MacroSignalsPanel
        | undefined;
    ...
    if (shouldPrime("stablecoins")) {
      const panel = this.state.panels["stablecoins"] as
        | StablecoinPanel
        | undefined;
    ...
    if (shouldPrime("gulf-economies")) {
      const panel = this.state.panels["gulf-economies"] as
        | GulfEconomiesPanel
        | undefined;
  ...
  constructor(containerId: string) {
    ...
    if (!localStorage.getItem(LAYOUT_RESET_MIGRATION_KEY)) {
      ...
      if (hadSavedOrder || hadSavedSpans) {
        ...
        localStorage.removeItem(PANEL_SPANS_KEY);
        console.log(
          "[App] Applied layout reset migration (v2.5): cleared panel order/spans",
        );
      }
    ...
    let initialUrlState: ParsedMapUrlState | null = parseMapUrlState(
    ...
    if (currentVariant === "full") {
      ...
      if (userLang !== "en" && !localStorage.getItem(localeKey)) {
        ...
        if (boosted.size > 0) {
          const current = loadFromStorage<string[]>(
    ...
    this.dataLoader.updateSearchIndex = () =>
  ...
  public async init(): Promise<void> {
    ...
    const initState = parseMapUrlState(
      window.location.search,
    ...
    if (this.state.correlationEngine) {
      void this.state.correlationEngine.run(this.state).then(() => {
        for (const domain of [
          ...
          const panel = this.state.panels[`${domain}-correlation`] as
            | CorrelationPanel
            | undefined;
    ...
    cleanOldSnapshots().catch((e) =>
      console.warn("[Storage] Snapshot cleanup failed:", e),
    );
  ...
  private handleDeepLinks(): void {
    ...
    const storyCode =
      this.pendingDeepLinkStoryCode ?? url.searchParams.get("c");
    ...
    if (url.pathname === "/story" || storyCode) {
      ...
      if (countryCode) {
        ...
        const countryName =
          getCountryNameByCode(countryCode.toUpperCase()) || countryCode;
        setTimeout(() => {
  ...
  private setupRefreshIntervals(): void {
    ...
    if (SITE_VARIANT !== "happy") {
      this.refreshScheduler.registerAll([
        ...
        },
        {
          ...
          intervalMs: 10 * 60 * 1000,
          condition: () =>
            CYBER_LAYER_ENABLED && this.state.mapLayers.cyberThreats,
        },
      ]);
    }
    ...
    this.refreshScheduler.scheduleRefresh(
      ...
      () =>
        (
          this.state.panels["service-status"] as ServiceStatusPanel
        ).fetchStatus(),
      3 * 60_000,
    ...
    this.refreshScheduler.scheduleRefresh(
      ...
      () =>
        (
          this.state.panels["strategic-posture"] as StrategicPosturePanel
        ).refresh(),
      15 * 60_000,
