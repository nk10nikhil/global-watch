...
if (import.meta.env.DEV) {
  for (const h of MILITARY_HOTSPOTS) {
    ...
    if (!covered)
      console.error(
        `[Military] HOTSPOT ${h.name} bbox not covered by any QUERY_REGION`,
      );
  }
}

export const USNI_REGION_COORDINATES: Record<
  string,
...
export function getUSNIRegionCoords(
  regionText: string,
): { lat: number; lon: number } | undefined {
  const normalized = normalizeUSNIRegion(regionText);
  if (USNI_REGION_COORDINATES[normalized])
    return USNI_REGION_COORDINATES[normalized];
...
export function getUSNIRegionApproxCoords(regionText: string): {
  lat: number;
  lon: number;
} {
