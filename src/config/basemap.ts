...
export function buildPMTilesStyle(
  ...
  return {
    ...
    layers: layers("basemap", namedFlavor(flavor), {
      lang: "en",
...
export const FALLBACK_LIGHT_STYLE =
...
export const MAP_PROVIDER_OPTIONS: { value: MapProvider; label: string }[] =
  (() => {
    ...
    if (hasTilesUrl) {
      opts.push({
        value: "auto",
        label: "Auto (PMTiles → OpenFreeMap fallback)",
      });
...
export const MAP_THEME_OPTIONS: Record<
  MapProvider,
...
export function getStyleForProvider(
  ...
  switch (provider) {
    ...
    default: {
      ...
      return (
        pmtiles ?? (lightFallback ? FALLBACK_LIGHT_STYLE : FALLBACK_DARK_STYLE)
      );
    }
  }
}
