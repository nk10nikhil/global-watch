...
function parseYouTubeInput(
  raw: string,
): { handle: string } | { videoId: string } | null {
  let url: URL;
  try {
    url = new URL(raw);
  } catch {
...
export async function initLiveChannelsWindow(
  ...
  function setupListDnD(listEl: HTMLElement): void {
    ...
    listEl.addEventListener("mousedown", (e) => {
      ...
      if (!row || row.classList.contains("live-news-manage-row-editing"))
        return;
      dragging = row;
      dragStarted = false;
      startY = e.clientY;
    ...
    for (const ch of channels) {
      ...
      if (isCustom) {
        row.addEventListener("click", (e) => {
          if (
            suppressRowClick ||
            row.classList.contains("live-news-manage-row-dragging")
          )
            return;
          if (
  ...
  function applyEditFormToChannels(
    ...
    if (isCustom) {
      const handleRaw = (
        formRow.querySelector(
          ".live-news-manage-edit-handle",
      ...
      if (handleRaw) {
        ...
        const existing = channels.find(
          (c) => c.id === newId && c.id !== currentCh.id,
        );
  ...
  function showEditForm(
    ...
    if (isCustom) {
      ...
      handleInput.placeholder =
        t("components.liveNews.youtubeHandle") ?? "YouTube handle";
    ...
    nameInput.placeholder =
      t("components.liveNews.displayName") ?? "Display name";
    row.appendChild(nameInput);

    const removeBtn = document.createElement("button");
    removeBtn.type = "button";
    removeBtn.className =
      "live-news-manage-remove live-news-manage-remove-in-form";
  ...
  function renderAvailableChannels(listEl: HTMLElement): void {
    ...
    for (const region of filteredRegions) {
      ...
      const addedCount = matchingChannels.filter((ch) =>
        currentIds.has(ch.id),
      ).length;

      const btn = document.createElement("button");
      btn.type = "button";
      btn.className =
        "panel-tab" + (region.key === activeRegionTab ? " active" : "");
    ...
    for (const region of filteredRegions) {
      ...
      panel.className =
        ...
        (region.key === activeRegionTab ? " active" : "");
      ...
      if (matchCount === 0 && term) {
        ...
        empty.textContent = (
          t("components.liveNews.noResults") ??
          'No channels found matching "{{term}}"'
        ).replace("{{term}}", term);
  ...
  document
    .getElementById("liveChannelsHandle")
    ?.addEventListener("input", (e) => {
      (e.target as HTMLInputElement).classList.remove("invalid");
    });
  document
    .getElementById("liveChannelsHlsUrl")
    ?.addEventListener("input", (e) => {
      (e.target as HTMLInputElement).classList.remove("invalid");
    });

  document
    .getElementById("liveChannelsRestoreBtn")
  ...
  addBtn?.addEventListener("click", async () => {
    ...
    const hlsInput = document.getElementById(
      "liveChannelsHlsUrl",
    ) as HTMLInputElement | null;
      ...
      if (!isHlsUrl(hlsUrl)) {
        if (hlsInput) {
          ...
          hlsInput.setAttribute(
            "title",
            t("components.liveNews.invalidHlsUrl") ??
              "Enter a valid HLS stream URL (.m3u8)",
          );
        }
        return;
      }
    ...
    if (parsed && "videoId" in parsed) {
      ...
      if (addBtn) {
        ...
        addBtn.textContent =
          t("components.liveNews.addChannel") ?? "Add channel";
      }
    ...
    try {
      const res = await fetch(
        toApiUrl(`/api/youtube/live?channel=${encodeURIComponent(handle)}`),
      );
      if (res.ok) {
        const data = await res.json();
      ...
      console.warn(
        "[LiveChannels] YouTube validation failed, allowing add:",
        e,
      );
    } finally {
      if (addBtn) {
