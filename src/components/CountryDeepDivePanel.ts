...
export class CountryDeepDivePanel implements CountryBriefPanel {
  ...
  constructor(map: MapContainer | null = null) {
    ...
    const closeButton =
      this.panel.querySelector<HTMLButtonElement>("#deep-dive-close");
    ...
    this.panel.addEventListener("click", (e) => {
      if (
        ...
        !(e.target as HTMLElement).closest(".panel-content")
      ) {
        this.minimize();
      }
    });
  ...
  public updateNews(headlines: NewsItem[]): void {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      top.append(
        this.badge(
          `Tier ${tier}`,
          `cdp-tier-badge tier-${Math.max(1, Math.min(4, tier))}`,
        ),
      );
      ...
      top.append(
        this.badge(
          ...
          `cdp-severity-badge sev-${severity}`,
        ),
      );
      ...
      if (risk.stateAffiliated) {
        top.append(
          this.badge(
            `State-affiliated: ${risk.stateAffiliated}`,
            "cdp-state-badge",
          ),
        );
      }
  ...
  public updateMilitaryActivity(summary: CountryDeepDiveMilitarySummary): void {
    ...
    stats.append(
      this.metric(
        t("countryBrief.ownFlights"),
        String(summary.ownFlights),
        "cdp-chip-neutral",
      ),
      this.metric(
      ...
      this.metric(
        ...
        summary.foreignPresence
          ? t("countryBrief.detected")
          : t("countryBrief.notDetected"),
        summary.foreignPresence ? "cdp-chip-danger" : "cdp-chip-success",
      ),
    );
    this.militaryBody.append(stats);

    const basesTitle = this.el(
      "div",
      "cdp-subtitle",
      t("countryBrief.nearestBases"),
    );
  ...
  public updateInfrastructure(countryCode: string): void {
    ...
    if (!centroid) {
      this.infrastructureBody.append(
        this.makeEmpty(t("countryBrief.noGeometry")),
      );
      return;
    }

    const assets = getNearbyInfrastructure(
      centroid.lat,
      centroid.lon,
      INFRA_TYPES,
    );
    if (assets.length === 0) {
      this.infrastructureBody.append(
        this.makeEmpty(t("countryBrief.noInfrastructure")),
      );
      return;
    }
    ...
    if (nearbyPorts.length > 0) {
      const portsTitle = this.el(
        "div",
        "cdp-subtitle",
        `\u2693 ${t("countryBrief.nearbyPorts")}`,
      );
  ...
  public updateEconomicIndicators(
  ...
  public updateCountryFacts(data: CountryFactsData): void {
    ...
    grid.append(
      this.factItem(
        t("countryBrief.facts.languages"),
        data.languages.join(", "),
      ),
    );
    grid.append(
      this.factItem(
        t("countryBrief.facts.currencies"),
  ...
  public updateScore(
    ...
    if (score) {
      ...
      const value = this.el(
        "div",
        `cdp-score-value cii-${band}`,
        `${score.score}/100`,
      );
      const trend = this.el(
        "div",
        "cdp-trend",
        `${this.trendArrow(score.trend)} ${score.trend}`,
      );
  ...
  public updateStock(data: StockIndexData): void {
    ...
    const base = this.economicIndicators.filter(
      (item) => item.label !== "Stock Index",
    );
    base.unshift({
      label: "Stock Index",
  ...
  public updateBrief(data: CountryIntelData): void {
    ...
    if (data.error || data.skipped || !data.brief) {
      this.briefBody.append(
        this.makeEmpty(
          data.error || data.reason || t("countryBrief.assessmentUnavailable"),
        ),
      );
      return;
    }
    ...
    fullText.innerHTML = this.formatBrief(
      data.brief,
      this.currentHeadlineCount,
    );
  ...
  private renderSkeleton(
    ...
    const flag = this.el(
      "span",
      "cdp-flag",
      CountryDeepDivePanel.toFlagEmoji(code),
    );
    ...
    const label = this.el(
      "span",
      "cdp-score-label",
      t("countryBrief.instabilityIndex"),
    );
    ...
    const [signalsCard, signalBody] = this.sectionCard(
      t("countryBrief.activeSignals"),
    );
    const [timelineCard, timelineBody] = this.sectionCard(
      t("countryBrief.timeline"),
    );
    const [newsCard, newsBody] = this.sectionCard(t("countryBrief.topNews"));
    const [militaryCard, militaryBody] = this.sectionCard(
      t("countryBrief.militaryActivity"),
    );
    const [infraCard, infraBody] = this.sectionCard(
      t("countryBrief.infrastructure"),
    );
    const [economicCard, economicBody] = this.sectionCard(
      t("countryBrief.economicIndicators"),
    );
    const [marketsCard, marketsBody] = this.sectionCard(
      t("countryBrief.predictionMarkets"),
    );
    const [briefCard, briefBody] = this.sectionCard(
      t("countryBrief.intelBrief"),
    );

    const [factsCard, factsBody] = this.sectionCard(
      t("countryBrief.countryFacts"),
    );
    ...
    militaryBody.append(
      this.makeLoading("Loading flights, vessels, and nearby bases…"),
    );
    infraBody.append(
      this.makeLoading("Computing nearby critical infrastructure…"),
    );
  ...
  private renderInitialSignals(signals: CountryBriefSignals): void {
    ...
    if (signals.displacementOutflow > 0) {
      ...
      chips.append(
        this.makeSignalChip(
          `🌊 ${fmt} ${t("countryBrief.chips.displaced")}`,
          "displacement",
        ),
      );
    }
    ...
    const seeded: CountryDeepDiveSignalDetails = {
      ...
      high:
        signals.militaryFlights + signals.militaryVessels + signals.protests,
    ...
    this.signalRecentBody.append(
      this.makeLoading("Loading top high-severity signals…"),
    );
  }
  ...
  private renderComponentBars(
    components: CountryScore["components"],
  ): HTMLElement {
    const wrap = this.el("div", "cdp-components");
    const items = [
      {
        label: t("countryBrief.components.unrest"),
        value: components.unrest,
        icon: "📢",
      },
      {
        label: t("countryBrief.components.conflict"),
        value: components.conflict,
        icon: "⚔",
      },
      {
        label: t("countryBrief.components.security"),
  ...
  private renderSignalBreakdown(details: CountryDeepDiveSignalDetails): void {
    ...
    this.signalBreakdownBody.append(
      this.metric(
        t("countryBrief.levels.critical"),
        String(details.critical),
        "cdp-chip-danger",
      ),
      this.metric(
        t("countryBrief.levels.high"),
        String(details.high),
        "cdp-chip-warn",
      ),
      this.metric(
        t("countryBrief.levels.moderate"),
        String(details.medium),
        "cdp-chip-neutral",
      ),
      this.metric(
        t("countryBrief.levels.low"),
        String(details.low),
        "cdp-chip-success",
      ),
    );
    ...
    for (const item of items.slice(0, 3)) {
      ...
      line.append(
        ...
        this.badge(
          ...
          `cdp-severity-badge sev-${item.severity}`,
        ),
      );
  ...
  private renderEconomicIndicators(): void {
    ...
    for (const indicator of this.economicIndicators.slice(0, 3)) {
      ...
      const isMarketRow =
        indicator.label === "Stock Index" ||
        indicator.label === "Weekly Momentum";
  ...
  private open(): void {
    ...
    this.onStateChangeCallback?.({
      visible: true,
      maximized: this.isMaximizedState,
    });
  }
  ...
  private getFocusableElements(): HTMLElement[] {
    ...
    return Array.from(
      this.panel.querySelectorAll<HTMLElement>(selectors),
    ).filter(
      (el) =>
