...
export class EconomicPanel extends Panel {
  ...
  private renderCentralBanks(): string {
    ...
    const sortedRates = [...this.bisData.policyRates].sort(
      (a, b) => b.rate - a.rate,
    );
    const policyHtml = `
        ...
        <div class="economic-indicators">
          ${sortedRates
            .map((r) => {
              ...
              const color =
                diff < 0 ? greenColor : diff > 0 ? redColor : neutralColor;
              const label =
                diff < 0
          ...
          <div class="economic-indicators">
            ${sortedCredit
              .map((r) => {
                ...
                const color =
                  diff > 0 ? redColor : diff < 0 ? greenColor : neutralColor;
                const arrow = diff > 0 ? "▲" : diff < 0 ? "▼" : "–";
                const changeStr =
                  diff !== 0
                    ? `${diff > 0 ? "+" : ""}${Math.round(diff * 10) / 10}pp`
                    : "–";
                return `
                <div class="economic-indicator">
                  <div class="indicator-header">
