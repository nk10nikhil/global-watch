...
export class TradePolicyPanel extends Panel {
  ...
  private render(): void {
    ...
    const hasTariffs =
      this.tariffsData && this.tariffsData.datapoints?.length > 0;
    const hasFlows = this.flowsData && this.flowsData.flows?.length > 0;
    const hasBarriers =
      this.barriersData && this.barriersData.barriers?.length > 0;
    ...
    return `<div class="trade-flows-list">
      ${this.flowsData.flows
        .map((f) => {
          ...
          const exportClass =
            f.yoyExportChange >= 0 ? "change-positive" : "change-negative";
          const importClass =
            f.yoyImportChange >= 0 ? "change-positive" : "change-negative";
