...
function resolveCommandLabel(cmd: Command): string {
  ...
  switch (prefix) {
    ...
    case "panel": {
      const panelName = t("panels." + kebabToCamel(action), {
...
const RECENT_SEARCHES_KEY = "GLOBALWATCH_recent_searches";
const MAX_RECENT = 8;
...
export class SearchModal {
  ...
  private createModal(): void {
    ...
    if (this.isMobile) {
      ...
      const sheet = this.overlay.querySelector(
        ".search-sheet",
      ) as HTMLElement | null;
  ...
  private matchCommands(query: string): CommandResult[] {
    ...
    for (const cmd of getAllCommands()) {
      ...
      for (const term of allTerms) {
        if (
          term.includes(query) ||
          (term.length >= 3 && query.includes(term))
        ) {
  ...
  private handleSearch(): void {
    ...
    trackSearchUsed(
      query.length,
      this.results.length + this.commandResults.length,
    );
    this.selectedIndex = 0;
    this.renderResults();
  ...
  private renderEmpty(): void {
    ...
    const tips: { icon: string; key: string; exampleKey: string }[] = [
      {
        icon: "\u{1F30D}",
        key: "commands.tips.map",
        exampleKey: "commands.tips.mapExample",
      },
      {
        icon: "\u{1F4CB}",
        key: "commands.tips.panel",
        exampleKey: "commands.tips.panelExample",
      },
      {
        icon: "\u{1F4C4}",
        key: "commands.tips.brief",
        exampleKey: "commands.tips.briefExample",
      },
      {
        icon: "\u{1F6E1}\uFE0F",
      ...
      {
        icon: "\u23F1\uFE0F",
        key: "commands.tips.time",
        exampleKey: "commands.tips.timeExample",
      },
      {
        icon: "\u2699\uFE0F",
  ...
  private renderAllCommandsList(): void {
    ...
    const backLink = this.resultsList.querySelector(
    ...
    this.resultsList
      .querySelectorAll(".search-command-category .command-item")
      .forEach((el) => {
        el.addEventListener("click", () => {
  ...
  private renderChips(query?: string): void {
    ...
    const actionCmds = commands.filter(
      (c) => c.category === "actions" || c.category === "view",
    );
