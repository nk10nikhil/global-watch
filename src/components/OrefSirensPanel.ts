...
export class OrefSirensPanel extends Panel {
  ...
  public setData(data: OrefAlertsResponse): void {
    if (!data.configured) {
      this.setContent(
        `<div class="panel-empty">${t("components.orefSirens.notConfigured")}</div>`,
      );
      this.setCount(0);
      return;
    }
    ...
    this.setCount(
      this.alerts.length || this.historyCount24h || this.totalHistoryCount,
    );
  ...
  private renderHistoryWaves(): string {
    ...
    const now = Date.now();
    const withTs = this.historyWaves.map((w) => ({
      wave: w,
    ...
    const rows = sorted
      .map(({ wave, ts }) => {
        ...
        const rowClass = isRecent
          ? "oref-wave-row oref-wave-recent"
          : "oref-wave-row";
        const badge = isRecent
          ? '<span class="oref-recent-badge">RECENT</span>'
          : "";
        const types = wave.alerts.map((a) => escapeHtml(a.title || a.cat));
        const uniqueTypes = [...new Set(types)];
        const totalAreas = wave.alerts.reduce(
          (sum, a) => sum + (a.data?.length || 0),
          0,
        );
        const summary =
          uniqueTypes.join(", ") +
          (totalAreas > 0 ? ` — ${totalAreas} areas` : "");
