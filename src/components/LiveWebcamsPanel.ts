...
export class LiveWebcamsPanel extends Panel {
  ...
  private static readonly ALL_GRID_IDS = [
    "jerusalem",
    "tehran",
    "kyiv",
    "washington",
  ...
  private createIframe(feed: WebcamFeed): HTMLIFrameElement {
    ...
    iframe.allow =
      "autoplay; encrypted-media; picture-in-picture; storage-access";
  ...
  private trackIframe(
    ...
    iframe.addEventListener("load", () => this.markIframeReady(iframe), {
      once: true,
    });
    tracker.timeout = setTimeout(
      () => this.markIframeBlocked(iframe),
      this.EMBED_READY_TIMEOUT_MS,
    );
  }
