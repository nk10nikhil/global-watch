...
export function openStoryModal(data: StoryData): void {
  ...
  requestAnimationFrame(async () => {
    ...
    try {
      ...
      if (content)
        content.innerHTML = `<div class="story-error">${t("modals.story.error")}</div>`;
    }
  });
}
...
function downloadStory(): void {
  ...
  a.download = `GLOBALWATCH-${currentData?.countryCode.toLowerCase() || "story"}-${Date.now()}.png`;
  a.click();
...
async function shareWhatsApp(data: StoryData): Promise<void> {
  ...
  const file = new File(
    [currentBlob],
    `${data.countryCode.toLowerCase()}-GLOBALWATCH.png`,
    { type: "image/png" },
  );
  ...
  try {
    ...
    flashButton(
      ".story-whatsapp",
      t("modals.story.copied"),
      t("modals.story.whatsapp"),
    );
  } catch {
    downloadStory();
    flashButton(
      ".story-whatsapp",
      t("modals.story.saved"),
      t("modals.story.whatsapp"),
    );
  }
...
async function shareTwitter(data: StoryData): Promise<void> {
  ...
  flashButton(
    ".story-twitter",
    t("modals.story.opening"),
    t("modals.story.twitter"),
  );
}
