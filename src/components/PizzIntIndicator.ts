...
export class PizzIntIndicator {
  ...
  constructor() {
    const panel = h(
      ...
      h(
        ...
        h(
          "span",
          { className: "pizzint-title" },
          t("components.pizzint.title"),
        ),
        h(
          "button",
          {
      ...
      h(
        ...
        h(
          "div",
          { className: "pizzint-tensions-title" },
          t("components.pizzint.tensionsTitle"),
        ),
  ...
  private renderTensions(): void {
    ...
    replaceChildren(
      ...
      ...this.tensions.map((tp) => {
        const trendIcon =
          tp.trend === "rising" ? "↑" : tp.trend === "falling" ? "↓" : "→";
        const changeText =
          tp.changePercent > 0
            ? `+${tp.changePercent}%`
            : `${tp.changePercent}%`;
        return h(
          ...
          h(
            ...
            h(
              "span",
              { className: "pizzint-tension-value" },
              tp.score.toFixed(1),
            ),
            h(
              "span",
  ...
  private formatTimeAgo(date: Date): string {
    ...
    if (diff < 3600000)
      return t("components.pizzint.minutesAgo", {
        m: String(Math.floor(diff / 60000)),
      });
    return t("components.pizzint.hoursAgo", {
      h: String(Math.floor(diff / 3600000)),
    });
  }
