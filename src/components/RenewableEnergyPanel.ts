...
export class RenewableEnergyPanel extends Panel {
  ...
  private renderSparkline(
    ...
    const xExtent = d3.extent(historicalData, (d) => d.year) as [
      number,
      number,
    ];
    const yExtent = d3.extent(historicalData, (d) => d.value) as [
      number,
      number,
    ];
  ...
  private renderRegions(
    ...
    for (let i = 0; i < regions.length; i++) {
      ...
      const opacity =
        regions.length > 1 ? 1.0 - (i / (regions.length - 1)) * 0.5 : 1.0;
  ...
  private renderCapacityChart(
    ...
    const solarMap = new Map(
      solarSeries?.data.map((d) => [d.year, d.capacityMw]) ?? [],
    );
    const windMap = new Map(
      windSeries?.data.map((d) => [d.year, d.capacityMw]) ?? [],
    );
    const coalMap = new Map(
      coalSeries?.data.map((d) => [d.year, d.capacityMw]) ?? [],
    );
