...
export class CIIPanel extends Panel {
  ...
  private buildTrendArrow(
    ...
    if (trend === "falling")
      return h("span", { className: "trend-down" }, `↓${Math.abs(change)}`);
  ...
  public renderFromCached(cached: CachedRiskScores): void {
    ...
    this.bindShareButtons();
    console.log(
      `[CIIPanel] Rendered ${scores.length} countries from cached/bootstrap data`,
    );
  }
