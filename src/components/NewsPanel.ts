...
export class NewsPanel extends Panel {
  ...
  private async renderClustersAsync(items: NewsItem[]): Promise<void> {
    ...
    try {
      ...
      console.warn(
        "[NewsPanel] Failed to cluster news, keeping flat list:",
        error,
      );
    }
  }
  ...
  private renderClusterHtmlSafely(
    ...
    try {
      ...
      console.error(
        "[NewsPanel] Failed to render cluster card:",
        error,
        cluster,
      );
      const clusterId =
        typeof cluster?.id === "string" ? cluster.id : "unknown-cluster";
      return `
  ...
  private renderClusterHtml(
    ...
    const newTag = showNewTag
      ? `<span class="new-tag">${t("common.new")}</span>`
      : "";
    const langBadge =
  ...
  private setupContentDelegation(): void {
    this.content.addEventListener("click", (e) => {
      ...
      if (assetBtn) {
        ...
        const assetType = assetBtn.dataset.assetType as
          | RelatedAsset["type"]
          | undefined;
    ...
    this.content.addEventListener("mouseover", (e) => {
      ...
      const context = this.relatedAssetContext.get(
        container.dataset.clusterId ?? "",
      );
      if (context)
