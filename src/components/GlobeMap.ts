...
export class GlobeMap {
  ...
  private showMarkerTooltip(d: GlobeMarker, anchor: HTMLElement): void {
    ...
    if (d._kind === "conflict") {
      html =
        ...
        (d.fatalities
          ? `<br><span style="opacity:.7;">Casualties: ${d.fatalities}</span>`
          : "");
    } else if (d._kind === "hotspot") {
      const sc =
  ...
  private initStaticLayers(): void {
    ...
    this.irradiatorSiteMarkers = (GAMMA_IRRADIATORS as GammaIrradiator[]).map(
      (g) => ({
    ...
    this.waterwayMarkers = (STRATEGIC_WATERWAYS as StrategicWaterway[]).map(
      (w) => ({
