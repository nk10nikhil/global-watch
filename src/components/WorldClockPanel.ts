...
const CITY_REGIONS: { name: string; ids: string[] }[] = [
  ...
  {
    name: "Europe",
    ids: ["london", "paris", "frankfurt", "zurich", "moscow", "istanbul"],
  },
  {
    name: "Middle East & Africa",
    ids: ["riyadh", "dubai", "cairo", "lagos", "johannesburg"],
  },
  {
    name: "Asia-Pacific",
    ids: [
...
const STORAGE_KEY = "GLOBALWATCH-world-clock-cities";
...
export class WorldClockPanel extends Panel {
  ...
  private setupDragHandlers(): void {
    ...
    document.addEventListener("mousemove", (e: MouseEvent) => {
      ...
      rows.forEach((r) =>
        r.classList.remove("wc-drag-over-above", "wc-drag-over-below"),
      );
  ...
  private renderClocks(): void {
    ...
    for (const city of sorted) {
      ...
      if (city.marketOpen !== undefined && city.marketClose !== undefined) {
        const isOpen =
          isWeekday && h >= city.marketOpen && h < city.marketClose;
        statusHtml = isOpen
