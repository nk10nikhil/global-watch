...
const ALERT_HOURS = 6;
const STORAGE_KEY = "GLOBALWATCH-intel-findings";
...
export class IntelligenceFindingsBadge {
  ...
  public update(): void {
    ...
    if (count === 0) {
      ...
      this.badge.title = t(
        "components.intelligenceFindings.reviewRecommended",
        { count: String(count) },
      );
    } else if (count <= LOW_COUNT_THRESHOLD) {
      this.badge.classList.add("status-low");
      this.badge.title = t("components.intelligenceFindings.count", {
        count: String(count),
      });
    } else {
      this.badge.classList.add("status-high");
  ...
  private renderDropdown(): void {
    ...
    if (criticalCount > 0) {
      ...
      statusText = t("components.intelligenceFindings.critical", {
        count: String(criticalCount),
      });
    } else if (highCount > 0) {
      statusClass = "high";
      statusText = t("components.intelligenceFindings.highPriority", {
        count: String(highCount),
      });
    }
  ...
  private getInsight(finding: UnifiedFinding): string {
    ...
    if (alert.type === "cii_spike") {
      ...
      if (cii && cii.change >= 30)
        return t(
          "components.intelligenceFindings.insights.criticalDestabilization",
        );
