...
export class SecurityAdvisoriesPanel extends Panel {
  ...
  private getFiltered(): SecurityAdvisory[] {
    switch (this.activeFilter) {
      ...
      case "health":
        return this.advisories.filter(
          (a) => a.sourceCountry === "EU" || a.sourceCountry === "INT",
        );
      case "US":
      case "AU":
      case "UK":
      case "NZ":
        return this.advisories.filter(
          (a) => a.sourceCountry === this.activeFilter,
        );
      default:
  ...
  private formatTime(date: Date): string {
    ...
    if (minutes < 60)
      return t("components.securityAdvisories.time.minutesAgo", {
        count: String(minutes),
      });
    if (hours < 24)
      return t("components.securityAdvisories.time.hoursAgo", {
        count: String(hours),
      });
    if (days < 7)
      return t("components.securityAdvisories.time.daysAgo", {
        count: String(days),
      });
  ...
  private render(): void {
    if (this.advisories.length === 0) {
      this.setContent(
        `<div class="panel-empty">${t("common.noDataAvailable")}</div>`,
      );
      return;
    }
    ...
    const dntCount = this.advisories.filter(
      (a) => a.level === "do-not-travel",
    ).length;
    const reconsiderCount = this.advisories.filter(
      (a) => a.level === "reconsider",
    ).length;
    const cautionCount = this.advisories.filter(
      (a) => a.level === "caution",
    ).length;
