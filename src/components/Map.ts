...
export class MapComponent {
  ...
  private createLayerToggles(): HTMLElement {
    ...
    const enforceLayerLimit = () => {
      const allBtns = Array.from(
        toggles.querySelectorAll<HTMLButtonElement>(".layer-toggle"),
      );
      ...
      const activeCount = allBtns.filter((b) =>
  ...
  private setupZoomHandlers(): void {
    ...
    const shouldIgnoreInteractionStart = (
  ...
  private renderCables(projection: d3.GeoProjection): void {
    ...
    UNDERSEA_CABLES.forEach((cable) => {
      ...
      const path = cableGroup
        ...
        .attr(
          "class",
          `cable-path ${advisoryClass} ${healthClass} ${highlightClass}`.trim(),
        )
  ...
  private renderOverlays(projection: d3.GeoProjection): void {
    ...
    if (this.state.layers.natural) {
      ...
      const filteredQuakes =
        this.state.timeRange === "all"
          ? this.earthquakes
          : this.earthquakes.filter(
              (eq) => eq.occurredAt >= Date.now() - this.getTimeRangeMs(),
            );
      console.log(
    ...
    if (this.state.layers.stockExchanges) {
      STOCK_EXCHANGES.forEach((exchange) => {
        ...
        if (
          (this.state.zoom >= 2 && exchange.tier === "mega") ||
          this.state.zoom >= 3
        ) {
    ...
    if (this.state.layers.financialCenters) {
      FINANCIAL_CENTERS.forEach((center) => {
        ...
        if (
          (this.state.zoom >= 2 && center.type === "global") ||
          this.state.zoom >= 3
        ) {
          const label = document.createElement("span");
          label.className = "marker-label";
    ...
    if (this.state.layers.centralBanks) {
      CENTRAL_BANKS.forEach((bank) => {
        ...
        if (
          (this.state.zoom >= 2 &&
            (bank.type === "major" || bank.type === "supranational")) ||
          this.state.zoom >= 3
        ) {
          const label = document.createElement("span");
          label.className = "marker-label";
    ...
    if (this.state.layers.commodityHubs) {
      COMMODITY_HUBS.forEach((hub) => {
        ...
        const icon =
          hub.type === "exchange" ? "📦" : hub.type === "port" ? "🚢" : "⛽";
        const div = document.createElement("div");
