...
function expCountdown(exp: Date | null, now: number): string {
  ...
  if (ms <= 0)
    return '<span style="color:#ef4444;font-size:10px">EXPIRED</span>';
  ...
  constructor() {
    ...
    this.content.addEventListener("click", (e) => {
      ...
      if (target.id === "priceSearchBtn" || target.closest("#priceSearchBtn")) {
        this.pricesOrigin = (
          (this.content.querySelector("#priceFromInput") as HTMLInputElement)
            ?.value || "IST"
        ).toUpperCase();
        this.pricesDest = (
          (this.content.querySelector("#priceToInput") as HTMLInputElement)
            ?.value || "LHR"
        ).toUpperCase();
        this.pricesDep =
          (this.content.querySelector("#priceDepInput") as HTMLInputElement)
            ?.value || "";
        this.pricesCurrency =
          (
    ...
    try {
      switch (tab) {
        case "ops":
          ...
          break;
        case "flights":
          this.flightsData = await fetchAirportFlights(
            this.airports[0] ?? "IST",
            "both",
            30,
          );
        ...
        case "news": {
          const entities = [
            ...this.airports,
            ...aviationWatchlist.get().airlines,
          ];
