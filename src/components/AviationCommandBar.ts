...
function parseIntent(raw: string): Intent {
  ...
  if (/^TRACK\s/.test(q)) {
    ...
    if (/^[0-9A-F]{6}$/i.test(token))
      return { type: "TRACK", icao24: token.toLowerCase() };
...
async function executeIntent(intent: Intent): Promise<CommandResult> {
  if (intent.type === "OPS") {
    ...
    if (!summaries.length)
      return { html: '<div class="cmd-empty">No ops data found.</div>' };
    const rows = summaries
      ...
      .join("");
    return {
      html: `<div class="cmd-section"><strong>✈️ Ops Snapshot</strong>${rows}</div>`,
    };
  }
  ...
  if (intent.type === "PRICE_WATCH") {
    ...
    if (!quotes.length)
      return { html: '<div class="cmd-empty">No prices found.</div>' };
    const best = quotes[0]!;
    return {
      html: `<div class="cmd-section">
  ...
  if (intent.type === "NEWS_BRIEF") {
    ...
    if (!items.length)
      return { html: '<div class="cmd-empty">No recent aviation news.</div>' };
    const rows = items
      .map(
        (n) =>
      ...
      .join("");
    return {
      html: `<div class="cmd-section"><strong>📰 Aviation News</strong>${rows}</div>`,
    };
  }
...
export class AviationCommandBar {
  ...
  private async run(raw: string): Promise<void> {
    ...
    if (!resultEl) return;
    resultEl.innerHTML =
      '<div style="color:#9ca3af;font-size:12px">Running…</div>';
