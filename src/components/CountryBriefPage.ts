...
export class CountryBriefPage implements CountryBriefPanel {
  ...
  constructor() {
    ...
    this.overlay.addEventListener("click", (e) => {
      ...
      const linkShareBtn = target.closest(
        ".cb-link-share-btn",
      ...
      const exportOption = target.closest(
        ".cb-export-option",
      ) as HTMLElement | null;
      if (exportOption) {
  ...
  private componentBars(components: CountryScore["components"]): string {
    const items = [
      {
        label: t("modals.countryBrief.components.unrest"),
  ...
  public updateBrief(data: CountryIntelData): void {
    ...
    if (data.error || data.skipped || !data.brief) {
      const msg =
        data.error || data.reason || t("modals.countryBrief.briefUnavailable");
