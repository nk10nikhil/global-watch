...
} from "@/generated/client/globalwatch/natural/v1/service_client";
...
function normalizeNaturalCategory(
...
function toNaturalEvent(
  e: ListNaturalEventsResponse["events"][number],
): NaturalEvent {
  return {
    id: e.id,
    title: e.title,
...
export async function fetchNaturalEvents(_days = 30): Promise<NaturalEvent[]> {
  const hydrated = getHydratedData("naturalEvents") as
