...
function shouldEscalateToCritical(
  lower: string,
  matchCat: EventCategory,
): boolean {
...
async function waitForGap(): Promise<void> {
  ...
  const jitter =
    Math.floor(Math.random() * STAGGER_JITTER_MS * 2) - STAGGER_JITTER_MS;
        ...
        try {
          ...
          if (
            ...
            if (err.statusCode === 401) {
              ...
              consecutive429s++;
              delay = Math.min(
                BASE_PAUSE_MS * Math.pow(2, consecutive429s - 1),
                MAX_PAUSE_MS,
              );
            } else {
              delay = 30_000;
            ...
            if (consecutive429s >= 2 && batchQueue.length > BATCH_SIZE) {
              ...
              console.warn(
                `[Classify] Dropped ${dropped.length} queued items after repeated 429s`,
              );
            }
            batchInFlight = false;
            setTimeout(() => {
