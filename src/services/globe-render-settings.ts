...
const ALLOWED_SCALES = GLOBE_RENDER_SCALE_OPTIONS.filter(
  (o) => !o.disabled,
).map((o) => o.value);
...
export function getGlobeRenderScale(): GlobeRenderScale {
  try {
    ...
    if (raw && ALLOWED_SCALES.includes(raw as GlobeRenderScale))
      return raw as GlobeRenderScale;
  } catch {
    // ignore
  }
  return "auto";
...
export function setGlobeRenderScale(scale: GlobeRenderScale): void {
  ...
  window.dispatchEvent(
    new CustomEvent(EVENT_NAME, { detail: { scale: safeScale } }),
  );
}

export function subscribeGlobeRenderScaleChange(
  cb: (scale: GlobeRenderScale) => void,
): () => void {
...
export function setGlobeTexture(texture: GlobeTexture): void {
  ...
  window.dispatchEvent(
    new CustomEvent(TEXTURE_EVENT_NAME, { detail: { texture } }),
  );
}

export function subscribeGlobeTextureChange(
  cb: (texture: GlobeTexture) => void,
): () => void {
  const handler = (e: Event) => {
    const detail = (e as CustomEvent).detail as
...
export function setGlobeVisualPreset(preset: GlobeVisualPreset): void {
  ...
  window.dispatchEvent(
    new CustomEvent(PRESET_EVENT_NAME, { detail: { preset } }),
  );
}
