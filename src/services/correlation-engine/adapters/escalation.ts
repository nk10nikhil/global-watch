...
export const escalationAdapter: DomainAdapter = {
  ...
  collectSignals(ctx: AppContext): SignalEvidence[] {
    ...
    for (const o of outages) {
      ...
      if (age > windowMs) continue;
      if (o.lat != null && o.lon != null && o.lat === 0 && o.lon === 0)
        continue;
    ...
    for (const c of clusters) {
      if (!c.threat || c.threat.level === "info" || c.threat.level === "low")
        continue;
      ...
      const normalizedCountry = normalizeToCode(
        matchedCountries[0],
        c.lat,
        c.lon,
      );
