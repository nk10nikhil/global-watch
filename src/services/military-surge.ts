...
function calculateBaseline(theaterId: string): {
  ...
  const avgTransport =
    relevant.reduce((sum, h) => sum + h.transportCount, 0) / relevant.length;
  const avgFighter =
    relevant.reduce((sum, h) => sum + h.fighterCount, 0) / relevant.length;
  const avgRecon =
    relevant.reduce((sum, h) => sum + h.reconCount, 0) / relevant.length;
...
export function analyzeFlightsForSurge(
  flights: MilitaryFlight[],
): SurgeAlert[] {
  cleanupOldHistory();
  ...
  for (const [theaterId, theaterFlightList] of theaterFlights) {
    ...
    if (
      transportCount >= baseline.transport * SURGE_THRESHOLD &&
      transportCount >= 5
    ) {
    ...
    if (
      fighterCount >= baseline.fighter * SURGE_THRESHOLD &&
      fighterCount >= 4
    ) {
...
export function detectForeignMilitaryPresence(
  flights: MilitaryFlight[],
...
export function foreignPresenceToSignal(alert: ForeignPresenceAlert): {
  ...
  const newsContext =
    focalPointDetector.getNewsCorrelationContext(relevantCountries);
  ...
  const metadata: Record<string, unknown> = {
    ...
    focalPointContext:
      focalPointContexts.length > 0 ? focalPointContexts : null,
  };
...
export function surgeAlertToSignal(surge: SurgeAlert): {
  ...
  const severity =
    surge.surgeMultiple >= 4
      ? "critical"
      : surge.surgeMultiple >= 3
        ? "high"
        : "medium";
...
export function getTheaterPostureSummaries(
  ...
  for (const theater of POSTURE_THEATERS) {
    ...
    const recentAvg =
      recent.length > 0
        ? recent.reduce((a, b) => a + b.totalMilitary, 0) / recent.length
        : total;
    const olderAvg =
      older.length > 0
        ? older.reduce((a, b) => a + b.totalMilitary, 0) / older.length
        : total;
    const changePercent =
...
export function recalcPostureWithVessels(
  ...
  for (const p of postures) {
    ...
    p.postureLevel =
      combined === 2 ? "critical" : combined === 1 ? "elevated" : "normal";
