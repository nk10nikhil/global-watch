...
function analyzeSentiment(text: string): {
  ...
  const sentiment: SentimentType =
    score < -1 ? "negative" : score > 1 ? "positive" : "neutral";
...
export function calculateVelocity(cluster: ClusteredEvent): VelocityMetrics {
  ...
  const timeSpanMs =
    cluster.lastUpdated.getTime() - cluster.firstSeen.getTime();
...
export function enrichWithVelocity(
...
export async function calculateVelocityWithML(
    ...
    const mlSentiment: SentimentType =
      sentiment.label === "positive"
        ...
        : sentiment.label === "negative"
          ...
          : "neutral";
    const mlScore =
      sentiment.label === "negative" ? -sentiment.score : sentiment.score;
...
export async function enrichWithVelocityML(
  clusters: ClusteredEvent[],
