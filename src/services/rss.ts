...
import {
  ...
  AI_CLASSIFY_MAX_PER_FEED,
} from "./ai-classify-queue";
...
export function getFeedFailures(): Map<
  string,
  { count: number; cooldownUntil: number }
> {
...
function extractImageUrl(item: Element): string | undefined {
  ...
  try {
    ...
    const contentText =
      contentEncoded.length > 0 ? contentEncoded[0]!.textContent || "" : "";
...
export async function fetchFeed(feed: Feed): Promise<NewsItem[]> {
  ...
  try {
    ...
    if (typeof feed.url !== "string") {
      url =
        feed.url[currentLang] ||
        feed.url["en"] ||
        Object.values(feed.url)[0] ||
        "";
    }
    ...
    void setPersistentCache(
      getPersistentFeedKey(feedScope),
...
export async function fetchCategoryFeeds(
  ...
  const filteredFeeds = feeds.filter(
    (feed) => !feed.lang || feed.lang === currentLang,
  );
