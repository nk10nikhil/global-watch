...
} from "@/generated/client/globalwatch/cyber/v1/service_client";
import type {
  CyberThreat,
  CyberThreatType,
  CyberThreatSource,
...
const emptyFallback: ListCyberThreatsResponse = {
  threats: [],
...
function toCyberThreat(proto: ProtoCyberThreat): CyberThreat {
  return {
    ...
    tags: proto.tags,
    firstSeen: proto.firstSeenAt
      ? new Date(proto.firstSeenAt).toISOString()
      : undefined,
    lastSeen: proto.lastSeenAt
      ? new Date(proto.lastSeenAt).toISOString()
      : undefined,
  };
}
...
export async function fetchCyberThreats(
  ...
  const hydrated = getHydratedData("cyberThreats") as
