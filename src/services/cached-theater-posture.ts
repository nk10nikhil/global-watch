...
export function toPostureData(
  resp: GetTheaterPostureResponse,
): CachedTheaterPosture {
...
export async function fetchCachedTheaterPosture(
  ...
  if (breaker.getCached() === null) {
    const hydrated = getHydratedData("theaterPosture") as
  ...
  if (
    !result ||
    !Array.isArray(result.postures) ||
    result.postures.length === 0
  ) {
    return null;
  }
