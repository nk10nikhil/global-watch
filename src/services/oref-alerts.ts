...
const translationCache = new Map<
  string,
  { title: string; data: string[]; desc: string }
>();
...
function alertNeedsTranslation(alert: OrefAlert): boolean {
  return (
    hasHebrew(alert.title) ||
    alert.data.some(hasHebrew) ||
    hasHebrew(alert.desc)
  );
}
...
function parseTranslationResponse(raw: string, alerts: OrefAlert[]): void {
  ...
  for (const alert of alerts) {
    ...
    const entry = {
      title: stripOrefLabels(
        title && !hasHebrew(title) ? title : staticTranslate(alert.title),
      ),
      data: (areas && !areas.some(hasHebrew)
        ? areas
        : alert.data.map((d) =>
            locationTranslator
              ? locationTranslator(staticTranslate(d))
              : staticTranslate(d),
          )
      ).map(stripOrefLabels),
      desc: stripOrefLabels(
        desc && !hasHebrew(desc) ? desc : staticTranslate(alert.desc),
      ),
    };
...
export async function fetchOrefAlerts(
  ...
  if (cachedResponse && now - lastFetchAt < CACHE_TTL) {
    return {
      ...cachedResponse,
      alerts: applyTranslations(cachedResponse.alerts),
    };
  }
...
export function onOrefAlertsUpdate(
  cb: (data: OrefAlertsResponse) => void,
): void {
