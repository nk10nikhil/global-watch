...
function coerceEntry(v: unknown): MarketWatchlistEntry | null {
  ...
  if (v && typeof v === "object") {
    ...
    const display = normalizeName(
      typeof obj.display === "string" ? obj.display : undefined,
    );
    return {
      symbol: sym,
      ...(name ? { name } : {}),
...
export function setMarketWatchlistEntries(
  ...
  window.dispatchEvent(
    new CustomEvent(MARKET_WATCHLIST_EVENT, { detail: { entries: out } }),
  );
}
...
export function resetMarketWatchlist(): void {
  ...
  window.dispatchEvent(
    new CustomEvent(MARKET_WATCHLIST_EVENT, { detail: { entries: [] } }),
  );
}
...
export function parseMarketWatchlistInput(
  text: string,
