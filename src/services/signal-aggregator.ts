...
class SignalAggregator {
  ...
  ingestTheaterPostures(
    ...
    for (const p of postures) {
      ...
      const hasFlight = this.signals.some(
        (s) => s.country === code && s.type === "military_flight",
      );
      ...
      const hasVessel = this.signals.some(
        (s) => s.country === code && s.type === "military_vessel",
      );
