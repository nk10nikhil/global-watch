...
function protoToMarket(m: {
  ...
  return {
    ...
    source:
      m.source === MarketSource.MARKET_SOURCE_KALSHI ? "kalshi" : "polymarket",
...
export async function fetchPredictions(opts?: {
  ...
  const markets = await breaker.execute(async () => {
    ...
    if (
      hydrated &&
      hydrated.fetchedAt &&
      Date.now() - hydrated.fetchedAt < 20 * 60 * 1000
    ) {
      const variant =
        SITE_VARIANT === "tech"
          ? hydrated.tech
...
export async function fetchCountryMarkets(
  ...
  if (hydrated?.geopolitical?.length) {
    ...
    const filtered = hydrated.geopolitical
      .filter(
        (m) => !isExpired(m.endDate) && m.title.toLowerCase().includes(lower),
      )
      .slice(0, 5);
