...
function parseAuLevel(title: string): SecurityAdvisory["level"] {
  ...
  if (
    lower.includes("exercise a high degree of caution") ||
    lower.includes("high degree")
  )
    return "caution";
  return "info";
}
...
function extractTargetCountry(
  title: string,
  feed: AdvisoryFeed,
): string | undefined {
  if (feed.targetCountry) return feed.targetCountry;
  if (feed.sourceCountry === "EU" || feed.sourceCountry === "INT")
    return undefined;
...
function parseFeedXml(text: string, feed: AdvisoryFeed): SecurityAdvisory[] {
...
): Array<Omit<SecurityAdvisory, "pubDate"> & { pubDate: string }> {
...
function fromSerializable(
  items: Array<Omit<SecurityAdvisory, "pubDate"> & { pubDate: string }>,
): SecurityAdvisory[] {
  ...
  const feedResults = await Promise.allSettled(
    ADVISORY_FEEDS.map(async (feed) => {
      try {
        ...
        if (!response.ok) {
          console.warn(
            `[SecurityAdvisories] ${feed.name} HTTP ${response.status}`,
          );
          return [];
        }
...
export async function loadCachedAdvisories(): Promise<
