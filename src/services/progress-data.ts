...
function buildSeedMap(
  seeds: SeedProgressIndicator[],
): Map<string, SeedProgressIndicator> {
...
function resolveFromSeeds(
  seedMap: Map<string, SeedProgressIndicator>,
): ProgressDataSet[] {
  return PROGRESS_INDICATORS.map((indicator) => {
    ...
    return seed?.data?.length
      ? buildDataSet(indicator, seed.data)
      : fallbackDataSet(indicator);
  });
}

async function fetchProgressDataFresh(): Promise<ProgressDataSet[]> {
  // 1. Try bootstrap hydration cache (first page load)
  const hydrated = getHydratedData("progressData") as
    | SeedProgressIndicator[]
    | undefined;
  ...
  try {
    ...
    if (resp.ok) {
      const { data } = (await resp.json()) as {
        data: { progressData?: SeedProgressIndicator[] };
      };
      if (data.progressData?.length)
        return resolveFromSeeds(buildSeedMap(data.progressData));
    }
  } catch {
