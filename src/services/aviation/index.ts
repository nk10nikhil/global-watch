...
import {
  ...
  type CabinClass,
} from "@/generated/client/globalwatch/aviation/v1/service_client";
...
function toDisplayFlight(p: ProtoFlight): FlightInstance {
  return {
    ...
    date: p.date,
    carrier: {
      iata: p.operatingCarrier?.iataCode ?? "",
      name: p.operatingCarrier?.name ?? "",
    },
    origin: { iata: p.origin?.iata ?? "", name: p.origin?.name ?? "" },
    destination: {
      iata: p.destination?.iata ?? "",
...
const breakerPrices = createCircuitBreaker<{
  quotes: PriceQuote[];
  isDemoMode: boolean;
}>({ name: "Flight Prices", cacheTtlMs: 10 * 60 * 1000, persistCache: true });
...
export async function fetchFlightDelays(): Promise<AirportDelayAlert[]> {
  const hydrated = getHydratedData("flightDelays") as
    | { alerts?: ProtoAlert[] }
    | undefined;
...
export async function fetchAirportOpsSummary(
  airports: string[],
): Promise<AirportOpsSummary[]> {
...
export async function fetchAirportFlights(
  ...
  return breakerFlights.execute(async () => {
    const r = await client.listAirportFlights({
      airport,
...
export async function fetchCarrierOps(
  airports: string[],
): Promise<CarrierOps[]> {
...
export async function fetchAviationNews(
  ...
  return breakerNews.execute(async () => {
    const r = await client.listAviationNews({
      entities,
      windowHours,
      maxItems,
    });
