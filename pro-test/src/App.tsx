...
export function renderTurnstileWidgets(): number {
  ...
  let count = 0;
  document
    .querySelectorAll<HTMLElement>(".cf-turnstile:not([data-rendered])")
    .forEach((el) => {
...
function showReferralSuccess(
  ...
  data: { referralCode?: string; position?: number; status?: string },
) {
  if (data.referralCode == null && data.status == null) {
    ...
    if (btn) {
      btn.textContent = t("form.joinWaitlist");
      btn.disabled = false;
    }
    return;
  }
  ...
  if (safeCode) {
    ...
    const shareLinks = [
      ...
      {
        label: t("referral.telegram"),
        href: `https://t.me/share/url?url=${shareUrl}&text=${encodeURIComponent(t("referral.joinWaitlistShare"))}`,
      },
    ];
...
async function submitWaitlist(email: string, formEl: HTMLFormElement) {
  ...
  const honeypot =
    (formEl.querySelector('input[name="website"]') as HTMLInputElement)
      ?.value || "";
  const turnstileWidget = formEl.querySelector(
    ".cf-turnstile",
  ) as HTMLElement | null;
...
const Logo = () => (
  ...
  >
    ...
    <div className="flex flex-col">
      ...
      <span className="text-[9px] text-wm-muted font-mono uppercase tracking-widest leading-none mt-1">
        by Someone.ceo
      </span>
    </div>
  </a>
...
const Navbar = () => (
  <nav
    className="fixed top-0 left-0 right-0 z-50 glass-panel border-b-0 border-x-0 rounded-none"
    aria-label="Main navigation"
  >
    <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
      <Logo />
      <div className="hidden md:flex items-center gap-8 text-sm font-mono text-wm-muted">
        <a href="#tiers" className="hover:text-wm-text transition-colors">
          {t("nav.free")}
        </a>
        <a href="#pro" className="hover:text-wm-green transition-colors">
          {t("nav.pro")}
        </a>
        <a href="#api" className="hover:text-wm-text transition-colors">
          {t("nav.api")}
        </a>
        <a href="#enterprise" className="hover:text-wm-text transition-colors">
          {t("nav.enterprise")}
        </a>
      </div>
      <a
        href="#waitlist"
        className="bg-wm-green text-wm-bg px-4 py-2 rounded-sm font-mono text-xs uppercase tracking-wider font-bold hover:bg-green-400 transition-colors"
      >
        {t("nav.reserveAccess")}
      </a>
    </div>
...
const WiredBadge = () => (
  ...
  >
    {t("wired.asFeaturedIn")}{" "}
    <span className="text-wm-text font-bold">WIRED</span>{" "}
...
const SignalBars = () => {
  ...
  return (
    <div className="relative my-4 md:my-8 -mx-6">
      ...
      <div
        className="flex items-end justify-center gap-[3px] md:gap-1 h-28 md:h-44 relative px-4"
        aria-hidden="true"
      >
        {Array.from({ length: total }).map((_, i) => {
          const distFromCenter = Math.abs(i - center);
          const isSignal = distFromCenter <= signalRadius;
          const signalIntensity = isSignal
            ? 1 - distFromCenter / signalRadius
            : 0;
          ...
          return (
            <motion.div
              ...
              initial={{
                height: isSignal ? peakHeight * 0.3 : noiseBase * 0.5,
                opacity: isSignal ? 0.4 : 0.08,
              }}
              animate={
                isSignal
                  ? {
                      ...
                      opacity: [
                        0.6 + signalIntensity * 0.3,
                        1,
                        0.75 + signalIntensity * 0.2,
                        0.95,
                      ],
                    }
                  : {
    ...
    <div className="max-w-4xl mx-auto text-center relative z-10">
      ...
      >
        ...
        <h1 className="text-6xl md:text-8xl font-display font-bold tracking-tighter leading-[0.95]">
          ...
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-wm-green to-emerald-300 text-glow">
            {t("hero.signalWord")}
          </span>
        </h1>
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              type="submit"
              className="bg-wm-green text-wm-bg px-6 py-3 rounded-sm font-mono text-sm uppercase tracking-wider font-bold hover:bg-green-400 transition-colors flex items-center justify-center gap-2 whitespace-nowrap"
            >
              {t("hero.reserveEarlyAccess")}{" "}
        ...
        <div className="flex items-center justify-center gap-4 mt-4">
          <p className="text-xs text-wm-muted font-mono">
            {t("hero.launchingDate")}
          </p>
          <span className="text-wm-border">|</span>
          <a
            href="https://worldmonitor.app"
            className="text-xs text-wm-green font-mono hover:text-green-300 transition-colors flex items-center gap-1"
          >
    ...
    <div className="max-w-5xl mx-auto">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center mb-12">
        {[
          ...
          <div key={i}>
            <p className="text-3xl md:text-4xl font-display font-bold text-wm-green">
              {stat.value}
            </p>
            <p className="text-xs font-mono text-wm-muted uppercase tracking-widest mt-1">
              {stat.label}
            </p>
          </div>
        ))}
      ...
      <blockquote className="max-w-3xl mx-auto text-center">
        ...
        <footer className="mt-6 flex items-center justify-center gap-3">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 text-wm-muted hover:text-wm-text transition-colors"
          >
            <img
              src={wiredLogo}
              alt="WIRED"
              className="h-5 brightness-0 invert opacity-60 hover:opacity-100 transition-opacity"
            />
          </a>
        </footer>
      </blockquote>
  ...
  <section className="py-24 px-6 max-w-5xl mx-auto" id="tiers">
    ...
    <div className="grid md:grid-cols-2 gap-8">
      <div className="bg-wm-card border border-wm-green p-8 relative border-glow">
        ...
        <a
          href="#waitlist"
          className="block text-center py-2.5 rounded-sm font-mono text-xs uppercase tracking-wider font-bold bg-wm-green text-wm-bg hover:bg-green-400 transition-colors"
        >
          {t("twoPath.proCta")}
        </a>
      </div>
      ...
      <div className="bg-wm-card border border-wm-border p-8">
        ...
        <ul className="space-y-3 mb-8">
          <li className="text-xs font-mono text-wm-green uppercase tracking-wider mb-1">
            {t("twoPath.entF1")}
          </li>
          {[
            t("twoPath.entF2"),
            ...
            <li key={i} className="flex items-start gap-3 text-sm">
              <Check
                className="w-4 h-4 shrink-0 mt-0.5 text-wm-muted"
                aria-hidden="true"
              />
              <span className="text-wm-muted">{f}</span>
            </li>
        ...
        <a
          href="#enterprise"
          className="block text-center py-2.5 rounded-sm font-mono text-xs uppercase tracking-wider font-bold border border-wm-border text-wm-muted hover:text-wm-text hover:border-wm-text transition-colors"
        >
          {t("twoPath.entCta")}
        </a>
      </div>
  ...
  const items = [
    {
      ...
      title: t("whyUpgrade.noiseTitle"),
      desc: t("whyUpgrade.noiseDesc"),
    },
    {
      icon: <TrendingUp className="w-6 h-6" aria-hidden="true" />,
      title: t("whyUpgrade.fasterTitle"),
      desc: t("whyUpgrade.fasterDesc"),
    },
    {
      icon: <SlidersHorizontal className="w-6 h-6" aria-hidden="true" />,
      title: t("whyUpgrade.controlTitle"),
      desc: t("whyUpgrade.controlDesc"),
    },
    {
      icon: <Telescope className="w-6 h-6" aria-hidden="true" />,
      title: t("whyUpgrade.deeperTitle"),
      desc: t("whyUpgrade.deeperDesc"),
    },
  ];
    ...
    <div className="max-w-6xl mx-auto">
      <div className="relative rounded-lg overflow-hidden border border-wm-border shadow-2xl shadow-wm-green/5">
        <div className="bg-wm-card px-4 py-2 border-b border-wm-border flex items-center gap-3">
          ...
          <span className="font-mono text-xs text-wm-muted ml-2">
        ...
        <div className="relative aspect-[16/9] bg-black">
          ...
          <div className="absolute bottom-4 left-0 right-0 text-center pointer-events-auto">
            ...
            >
              {t("livePreview.tryLiveDashboard")}{" "}
              <ArrowRight className="w-4 h-4" aria-hidden="true" />
            </a>
          </div>
        </div>
  ...
  >
    <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-start">
      <div>
        ...
        <p className="text-wm-muted mb-8">{t("proShowcase.subtitle")}</p>
          ...
          <div className="flex gap-4">
            ...
            <div>
              <h3 className="font-bold mb-1">
                {t("proShowcase.equityResearch")}
              </h3>
              <p className="text-sm text-wm-muted">
                {t("proShowcase.equityResearchDesc")}
              </p>
            </div>
          </div>
          <div className="flex gap-4">
            ...
            <div>
              <h3 className="font-bold mb-1">
                {t("proShowcase.geopoliticalAnalysis")}
              </h3>
              <p className="text-sm text-wm-muted">
                {t("proShowcase.geopoliticalAnalysisDesc")}
              </p>
            </div>
          </div>
          <div className="flex gap-4">
            ...
            <div>
              <h3 className="font-bold mb-1">
                {t("proShowcase.economyAnalytics")}
              </h3>
              <p className="text-sm text-wm-muted">
                {t("proShowcase.economyAnalyticsDesc")}
              </p>
            </div>
          </div>
          <div className="flex gap-4">
            ...
            <div>
              <h3 className="font-bold mb-1">
                {t("proShowcase.riskMonitoring")}
              </h3>
              <p className="text-sm text-wm-muted">
                {t("proShowcase.riskMonitoringDesc")}
              </p>
            </div>
          </div>
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-wm-muted">
                {t("proShowcase.orbitalSurveillanceDesc")}
              </p>
            </div>
          </div>
          <div className="flex gap-4">
            ...
            <div>
              <h3 className="font-bold mb-1">
                {t("proShowcase.morningBriefs")}
              </h3>
              <p className="text-sm text-wm-muted">
                {t("proShowcase.morningBriefsDesc")}
              </p>
            </div>
          </div>
          <div className="flex gap-4">
        ...
        <div className="mt-10 pt-8 border-t border-wm-border">
          <p className="font-mono text-xs text-wm-muted uppercase tracking-widest mb-4">
            {t("proShowcase.deliveryLabel")}
          </p>
          <div className="flex gap-6">
            {[
              ...
              <div
                key={i}
                className="flex flex-col items-center gap-1.5 text-wm-muted hover:text-wm-text transition-colors cursor-pointer"
              >
                {ch.icon}
          ...
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-gray-300 font-bold mb-3">
                {t("slackMock.morningBrief")} &middot; Mar 6
              </p>
  ...
  const personas = [
    {
      ...
      desc: t("audience.investorsDesc"),
    },
    {
      icon: <Fuel className="w-6 h-6" aria-hidden="true" />,
      title: t("audience.tradersTitle"),
      desc: t("audience.tradersDesc"),
    },
    {
      icon: <Search className="w-6 h-6" aria-hidden="true" />,
      title: t("audience.researchersTitle"),
      desc: t("audience.researchersDesc"),
    },
    {
      icon: <Globe className="w-6 h-6" aria-hidden="true" />,
      title: t("audience.journalistsTitle"),
      desc: t("audience.journalistsDesc"),
    },
    {
      icon: <Landmark className="w-6 h-6" aria-hidden="true" />,
      title: t("audience.govTitle"),
      desc: t("audience.govDesc"),
    },
    {
      icon: <Building2 className="w-6 h-6" aria-hidden="true" />,
      title: t("audience.teamsTitle"),
      desc: t("audience.teamsDesc"),
    },
  ];
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {personas.map((p, i) => (
            <div
              key={i}
              className="bg-wm-card border border-wm-border p-6 hover:border-wm-green/30 transition-colors"
            >
  ...
  >
    <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="p-6 text-gray-300 overflow-x-auto">
            <pre>
              <code>
                ...
                <br />
                <span className="text-wm-blue">"status"</span>:{" "}
                <span className="text-wm-green">"success"</span>,<br />
                <span className="text-wm-blue">"data"</span>:{" "}
                <span className="text-wm-muted">{"["}</span>
                <br />
                <span className="text-wm-muted">{"{"}</span>
                <br />
                <span className="text-wm-blue">"type"</span>:{" "}
                <span className="text-wm-green">
                  "multi_signal_convergence"
                </span>
                ,<br />
                <span className="text-wm-blue">"signals"</span>:{" "}
                <span className="text-wm-muted">
                  ["military_flights", "ais_dark_ships", "oref_sirens"]
                </span>
                ,<br />
                <span className="text-wm-blue">"confidence"</span>:{" "}
                <span className="text-orange-400">0.92</span>,<br />
                <span className="text-wm-blue">"location"</span>:{" "}
                <span className="text-wm-muted">{"{"}</span>{" "}
                <span className="text-wm-blue">"lat"</span>:{" "}
                <span className="text-orange-400">34.05</span>,{" "}
                <span className="text-wm-blue">"lng"</span>:{" "}
                <span className="text-orange-400">35.12</span>{" "}
                <span className="text-wm-muted">{"}"}</span>
                <br />
                <span className="text-wm-muted">{"}"}</span>
                <br />
      ...
      <div className="order-1 lg:order-2">
        ...
        <p className="text-wm-muted mb-8">{t("apiSection.subtitle")}</p>
        ...
        <div className="grid grid-cols-2 gap-4 mb-8 p-4 bg-wm-card border border-wm-border rounded-sm">
          <div>
            <p className="font-mono text-xs text-wm-muted uppercase tracking-widest mb-2">
              {t("apiSection.starter")}
            </p>
            <p className="text-sm font-bold">{t("apiSection.starterReqs")}</p>
            <p className="text-xs text-wm-muted">
              {t("apiSection.starterWebhooks")}
            </p>
          </div>
          <div>
            <p className="font-mono text-xs text-wm-muted uppercase tracking-widest mb-2">
              {t("apiSection.business")}
            </p>
            <p className="text-sm font-bold">{t("apiSection.businessReqs")}</p>
            <p className="text-xs text-wm-muted">
              {t("apiSection.businessWebhooks")}
            </p>
          </div>
        </div>
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-6">
        <div className="bg-wm-card border border-wm-border p-6">
          ...
          <p className="text-sm text-wm-muted">
            {t("enterpriseShowcase.securityDesc")}
          </p>
        </div>
        <div className="bg-wm-card border border-wm-border p-6">
          ...
          <p className="text-sm text-wm-muted">
            {t("enterpriseShowcase.aiAgentsDesc")}
          </p>
        </div>
        <div className="bg-wm-card border border-wm-border p-6">
          <Layers className="w-8 h-8 text-wm-muted mb-4" aria-hidden="true" />
          <h3 className="font-bold mb-2">
            {t("enterpriseShowcase.dataLayers")}
          </h3>
          <p className="text-sm text-wm-muted">
            {t("enterpriseShowcase.dataLayersDesc")}
          </p>
        </div>
      </div>
      <div className="grid md:grid-cols-3 gap-6 mb-12">
        <div className="bg-wm-card border border-wm-border p-6">
          ...
          <h3 className="font-bold mb-2">
            {t("enterpriseShowcase.connectors")}
          </h3>
          <p className="text-sm text-wm-muted">
            {t("enterpriseShowcase.connectorsDesc")}
          </p>
        </div>
        <div className="bg-wm-card border border-wm-border p-6">
          <PanelTop className="w-8 h-8 text-wm-muted mb-4" aria-hidden="true" />
          <h3 className="font-bold mb-2">
            {t("enterpriseShowcase.whiteLabel")}
          </h3>
          <p className="text-sm text-wm-muted">
            {t("enterpriseShowcase.whiteLabelDesc")}
          </p>
        </div>
        <div className="bg-wm-card border border-wm-border p-6">
          <BarChart3
          ...
          />
          <h3 className="font-bold mb-2">
            {t("enterpriseShowcase.financial")}
          </h3>
          <p className="text-sm text-wm-muted">
            {t("enterpriseShowcase.financialDesc")}
          </p>
        </div>
      </div>
      ...
      <div className="data-grid mb-12">
        <div className="data-cell">
          <h4 className="font-mono text-xs text-wm-muted uppercase tracking-widest mb-2">
            {t("enterpriseShowcase.commodity")}
          </h4>
          <p className="text-sm">{t("enterpriseShowcase.commodityDesc")}</p>
        </div>
        <div className="data-cell">
          <h4 className="font-mono text-xs text-wm-muted uppercase tracking-widest mb-2">
            {t("enterpriseShowcase.government")}
          </h4>
          <p className="text-sm">{t("enterpriseShowcase.governmentDesc")}</p>
        </div>
        <div className="data-cell">
          <h4 className="font-mono text-xs text-wm-muted uppercase tracking-widest mb-2">
            {t("enterpriseShowcase.risk")}
          </h4>
          <p className="text-sm">{t("enterpriseShowcase.riskDesc")}</p>
        </div>
        <div className="data-cell">
          <h4 className="font-mono text-xs text-wm-muted uppercase tracking-widest mb-2">
            {t("enterpriseShowcase.soc")}
          </h4>
...
const PricingTable = () => {
  const rows = [
    {
      feature: t("pricingTable.dataRefresh"),
      free: t("pricingTable.f5_15min"),
      pro: t("pricingTable.fLt60s"),
      api: t("pricingTable.fPerRequest"),
      ent: t("pricingTable.fLiveEdge"),
    },
    {
      feature: t("pricingTable.dashboard"),
      free: t("pricingTable.f50panels"),
      pro: t("pricingTable.f50panels"),
      api: "\u2014",
      ent: t("pricingTable.fWhiteLabel"),
    },
    {
      feature: t("pricingTable.ai"),
      free: t("pricingTable.fBYOK"),
      pro: t("pricingTable.fIncluded"),
      api: "\u2014",
      ent: t("pricingTable.fAgentsPersonas"),
    },
    {
      feature: t("pricingTable.briefsAlerts"),
      free: "\u2014",
      pro: t("pricingTable.fDailyFlash"),
      api: "\u2014",
      ent: t("pricingTable.fTeamDist"),
    },
    {
      feature: t("pricingTable.delivery"),
      free: "\u2014",
      pro: t("pricingTable.fSlackTgWa"),
      api: t("pricingTable.fWebhook"),
      ent: t("pricingTable.fSiemMcp"),
    },
    {
      feature: t("pricingTable.apiRow"),
      free: "\u2014",
      pro: "\u2014",
      api: t("pricingTable.fRestWebhook"),
      ent: t("pricingTable.fMcpBulk"),
    },
    {
      feature: t("pricingTable.infraLayers"),
      free: t("pricingTable.f45"),
      pro: t("pricingTable.f45"),
      api: "\u2014",
      ent: t("pricingTable.fTensOfThousands"),
    },
    {
      feature: t("pricingTable.satellite"),
      free: t("pricingTable.fLiveTracking"),
      pro: t("pricingTable.fPassAlerts"),
      api: "\u2014",
      ent: t("pricingTable.fImagerySar"),
    },
    {
      feature: t("pricingTable.connectorsRow"),
      free: "\u2014",
      pro: "\u2014",
      api: "\u2014",
      ent: t("pricingTable.f100plus"),
    },
    {
      feature: t("pricingTable.deployment"),
      free: t("pricingTable.fCloud"),
      pro: t("pricingTable.fCloud"),
      api: t("pricingTable.fCloud"),
      ent: t("pricingTable.fCloudOnPrem"),
    },
    {
      feature: t("pricingTable.securityRow"),
      free: t("pricingTable.fStandard"),
      pro: t("pricingTable.fStandard"),
      api: t("pricingTable.fKeyAuth"),
      ent: t("pricingTable.fSsoMfa"),
    },
  ];
    ...
    <section className="py-24 px-6 max-w-7xl mx-auto">
      ...
      <div className="hidden md:block">
        ...
        {rows.map((row, i) => (
          <div
            key={i}
            className="grid grid-cols-5 gap-4 py-4 border-b border-wm-border/50 text-sm hover:bg-wm-card/50 transition-colors"
          >
        ...
        {rows.map((row, i) => (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div>
                <span className="text-wm-muted">{t("tiers.free")}:</span>{" "}
                {row.free}
              </div>
              <div>
                <span className="text-wm-green">{t("tiers.pro")}:</span>{" "}
                <span className="text-wm-green">{row.pro}</span>
              </div>
              <div>
                <span className="text-wm-muted">{t("nav.api")}:</span> {row.api}
              </div>
              <div>
                <span className="text-wm-muted">{t("tiers.enterprise")}:</span>{" "}
                {row.ent}
              </div>
            </div>
          </div>
      ...
      <div className="space-y-4">
        {faqs.map((faq, i) => (
          <details
            key={i}
            open={faq.open}
            className="group bg-wm-card border border-wm-border rounded-sm [&_summary::-webkit-details-marker]:hidden"
          >
            <summary className="flex items-center justify-between p-6 cursor-pointer font-medium">
              {faq.q}
              <ChevronDown
                className="w-5 h-5 text-wm-muted group-open:rotate-180 transition-transform"
                aria-hidden="true"
              />
            </summary>
...
const Footer = () => (
  <footer
    className="border-t border-wm-border bg-[#020202] pt-24 pb-12 px-6 text-center"
    id="waitlist"
  >
    <div className="max-w-2xl mx-auto mb-16">
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            type="submit"
            className="bg-wm-green text-wm-bg px-6 py-3 rounded-sm font-mono text-sm uppercase tracking-wider font-bold hover:bg-green-400 transition-colors whitespace-nowrap"
          >
            {t("finalCta.getPro")}
          </button>
        </div>
    ...
    <div className="flex flex-col md:flex-row items-center justify-between max-w-7xl mx-auto pt-8 border-t border-wm-border/50 text-xs text-wm-muted font-mono">
      <div className="flex items-center gap-3 mb-4 md:mb-0">
        ...
        />
        <div className="flex flex-col">
          <span className="font-display font-bold text-sm leading-none tracking-tight text-wm-text">
            WORLD MONITOR
          </span>
      ...
      <div className="flex items-center gap-6">
        <a href="/" className="hover:text-wm-text transition-colors">
          Dashboard
        </a>
        <a
      ...
      <span className="text-[10px] opacity-40 mt-4 md:mt-0">
      ...
      <section className="py-24 px-6 border-t border-wm-border">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="data-grid">
            <div className="data-cell">
              <h3 className="font-mono text-xs text-wm-muted uppercase tracking-widest mb-2">
                {t("enterpriseShowcase.commodity")}
              </h3>
              <p className="text-sm">{t("enterpriseShowcase.commodityDesc")}</p>
            </div>
            <div className="data-cell">
              ...
              <p className="text-sm">
                {t("enterpriseShowcase.governmentDesc")}
              </p>
            </div>
            <div className="data-cell">
              <h3 className="font-mono text-xs text-wm-muted uppercase tracking-widest mb-2">
      ...
      <section className="py-24 px-6 border-t border-wm-border" id="contact">
        <div className="max-w-xl mx-auto">
          <h2 className="font-display text-3xl font-bold mb-2 text-center">
            {t("enterpriseShowcase.contactFormTitle")}
          </h2>
          <p className="text-sm text-wm-muted mb-10 text-center">
            {t("enterpriseShowcase.contactFormSubtitle")}
          </p>
          <form
            className="space-y-4"
            onSubmit={async (e) => {
              ...
              try {
                ...
                if (!res.ok) {
                  ...
                  if (res.status === 422 && errorEl) {
                    errorEl.textContent =
                      data.error || t("enterpriseShowcase.workEmailRequired");
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4">
              <input
                type="text"
                name="name"
                placeholder={t("enterpriseShowcase.namePlaceholder")}
                required
                className="bg-wm-bg border border-wm-border rounded-sm px-4 py-3 text-sm focus:outline-none focus:border-wm-green transition-colors font-mono"
              />
              <input
                type="email"
                name="email"
                placeholder={t("enterpriseShowcase.emailPlaceholder")}
                required
                className="bg-wm-bg border border-wm-border rounded-sm px-4 py-3 text-sm focus:outline-none focus:border-wm-green transition-colors font-mono"
              />
            </div>
            <span
              data-form-error
              className="hidden text-red-400 text-xs font-mono block"
            />
            <div className="grid grid-cols-2 gap-4">
              <input
                type="text"
                name="organization"
                placeholder={t("enterpriseShowcase.orgPlaceholder")}
                required
                className="bg-wm-bg border border-wm-border rounded-sm px-4 py-3 text-sm focus:outline-none focus:border-wm-green transition-colors font-mono"
              />
              <input
                type="tel"
                name="phone"
                placeholder={t("enterpriseShowcase.phonePlaceholder")}
                required
                className="bg-wm-bg border border-wm-border rounded-sm px-4 py-3 text-sm focus:outline-none focus:border-wm-green transition-colors font-mono"
              />
            </div>
            <textarea
              name="message"
              placeholder={t("enterpriseShowcase.messagePlaceholder")}
              rows={4}
              className="w-full bg-wm-bg border border-wm-border rounded-sm px-4 py-3 text-sm focus:outline-none focus:border-wm-green transition-colors font-mono resize-none"
            />
            <div className="cf-turnstile mx-auto" />
            <button
              type="submit"
              className="w-full bg-wm-green text-wm-bg py-3 rounded-sm font-mono text-sm uppercase tracking-wider font-bold hover:bg-green-400 transition-colors"
            >
